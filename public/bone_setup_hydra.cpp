...
// -----------------------------------------------------------------
template <typename T>
class CBoneSetupMemoryPool_Hydra
{
public:
	T *Alloc()
	{
...
CBoneSetupMemoryPool_Hydra<Quaternion> g_QaternionPool;
CBoneSetupMemoryPool_Hydra<Vector> g_VectorPool;
CBoneSetupMemoryPool_Hydra<matrix3x4_t> g_MatrixPool;

// -----------------------------------------------------------------
CBoneCache_Hydra *CBoneCache_Hydra::CreateResource(const bonecacheparams_t_Hydra &params)
{
	...
	int size = (sizeof(CBoneCache_Hydra) + tableSizeStudio + tableSizeCached + matrixSize + 3) & ~3;

	CBoneCache_Hydra *pMem = (CBoneCache_Hydra *)malloc(size);
	Construct(pMem);
	pMem->Init(params, size, studioToCachedIndex, cachedToStudioIndex, cachedBoneCount);
	return pMem;
}

unsigned int CBoneCache_Hydra::EstimatedSize(const bonecacheparams_t_Hydra &params)
{
	// conservative estimate - max size
	return (params.pStudioHdr->numbones() * (sizeof(short) + sizeof(short) + sizeof(matrix3x4_t)) + 3) & ~3;
}

void CBoneCache_Hydra::DestroyResource()
{
	free(this);
}


CBoneCache_Hydra::CBoneCache_Hydra()
{
	m_size = 0;
	m_cachedBoneCount = 0;
}

void CBoneCache_Hydra::Init(const bonecacheparams_t_Hydra &params, unsigned int size, short *pStudioToCached, short *pCachedToStudio, int cachedBoneCount)
{
...
void CBoneCache_Hydra::UpdateBones(const matrix3x4_t *pBoneToWorld, int numbones, float curtime)
{
...
matrix3x4_t *CBoneCache_Hydra::GetCachedBone(int studioIndex)
{
...
void CBoneCache_Hydra::ReadCachedBones(matrix3x4_t *pBoneToWorld)
{
	matrix3x4_t *pBones = BoneArray();
	const short *pCachedToStudio = CachedToStudio();
	for (int i = 0; i < m_cachedBoneCount; i++)
...
void CBoneCache_Hydra::ReadCachedBonePointers(matrix3x4_t **bones, int numbones)
{
...
bool CBoneCache_Hydra::IsValid(float curtime, float dt)
{
...
// private functions
matrix3x4_t *CBoneCache_Hydra::BoneArray()
{
	return (matrix3x4_t *)((char *)(this + 1) + m_matrixOffset);
}

short *CBoneCache_Hydra::StudioToCached()
{
	return (short *)((char *)(this + 1));
}

short *CBoneCache_Hydra::CachedToStudio()
{
	return (short *)((char *)(this + 1) + m_cachedToStudioOffset);
}

// Construct a singleton
static CDataManager<CBoneCache_Hydra, bonecacheparams_t_Hydra, CBoneCache_Hydra *, CThreadFastMutex> g_StudioBoneCache(24 * 1024L);
...
{
	AUTO_LOCK(g_StudioBoneCache.AccessMutex());
	CBoneCache_Hydra *pCache = g_StudioBoneCache.GetResource_NoLock(cacheHandle);
	if (pCache)
	{
	...
	matrix3x4_t *pBoneToWorld)
{
	...
	BuildBoneChain_Hydra(pStudioHdr, rootxform, pos, q, iBone, pBoneToWorld, boneComputed);
	return;
}
...
//-----------------------------------------------------------------------------
void ExtractAnimValue_Hydra(int frame, mstudioanimvalue_t *panimvalue, float scale, float &v1, float &v2)
{
	if (!panimvalue)
	{
		v1 = v2 = 0;
...
void ExtractAnimValue_Hydra(int frame, mstudioanimvalue_t *panimvalue, float scale, float &v1)
{
	if (!panimvalue)
	{
		v1 = 0;
...
// Purpose: return a sub frame rotation for a single bone
//-----------------------------------------------------------------------------
void CalcBoneQuaternion_Hydra(int frame, float s,
	...
	const mstudioanim_t *panim, Quaternion &q)
{
	...
	if (s > 0.001f)
	{
		...
		ExtractAnimValue_Hydra(frame, pValuesPtr->pAnimvalue(0), baseRotScale.x, angle1.x, angle2.x);
		ExtractAnimValue_Hydra(frame, pValuesPtr->pAnimvalue(1), baseRotScale.y, angle1.y, angle2.y);
		ExtractAnimValue_Hydra(frame, pValuesPtr->pAnimvalue(2), baseRotScale.z, angle1.z, angle2.z);
	...
	else
	{
		...
		ExtractAnimValue_Hydra(frame, pValuesPtr->pAnimvalue(0), baseRotScale.x, angle.x);
		ExtractAnimValue_Hydra(frame, pValuesPtr->pAnimvalue(1), baseRotScale.y, angle.y);
		ExtractAnimValue_Hydra(frame, pValuesPtr->pAnimvalue(2), baseRotScale.z, angle.z);
...
inline void CalcBoneQuaternion_Hydra(int frame, float s,
	...
	const mstudioanim_t *panim, Quaternion &q)
{
	if (pLinearBones)
	{
		CalcBoneQuaternion_Hydra(frame, s, pLinearBones->quat(panim->bone), pLinearBones->rot(panim->bone), pLinearBones->rotscale(panim->bone), pLinearBones->flags(panim->bone), pLinearBones->qalignment(panim->bone), panim, q);
	}
	else
	{
		CalcBoneQuaternion_Hydra(frame, s, pBone->quat, pBone->rot, pBone->rotscale, pBone->flags, pBone->qAlignment, panim, q);
	}
}
...
//-----------------------------------------------------------------------------
void CalcBonePosition_Hydra(int frame, float s,
	...
	const mstudioanim_t *panim, Vector &pos)
{
	...
	if (s > 0.001f)
	{
		...
		for (j = 0; j < 3; j++)
		{
			ExtractAnimValue_Hydra(frame, pPosV->pAnimvalue(j), baseBoneScale[j], v1, v2);
			pos[j] = v1 * (1.0 - s) + v2 * s;
		}
	}
	else
	{
		for (j = 0; j < 3; j++)
		{
			ExtractAnimValue_Hydra(frame, pPosV->pAnimvalue(j), baseBoneScale[j], pos[j]);
		}
	}
...
inline void CalcBonePosition_Hydra(int frame, float s,
	const mstudiobone_t *pBone,
	const mstudiolinearbone_t *pLinearBones,
	const mstudioanim_t *panim, Vector &pos)
{
	if (pLinearBones)
	{
		CalcBonePosition_Hydra(frame, s, pLinearBones->pos(panim->bone), pLinearBones->posscale(panim->bone), panim, pos);
	}
	else
	{
		CalcBonePosition_Hydra(frame, s, pBone->pos, pBone->posscale, panim, pos);
	}
}
	...
	matrix3x4_t &mBoneLocal)
{
	...
	{
		CalcBoneQuaternion_Hydra(iLocalFrame, s, pbone, NULL, panim, boneQuat);
		CalcBonePosition_Hydra(iLocalFrame, s, pbone, NULL, panim, bonePos);
	}
...
static void CalcDecompressedAnimation(const mstudiocompressedikerror_t *pCompressed, int iFrame, float fraq, Vector &pos, Quaternion &q)
{
	...
	{
		Vector p1, p2;
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(0), pCompressed->scale[0], p1.x, p2.x);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(1), pCompressed->scale[1], p1.y, p2.y);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(2), pCompressed->scale[2], p1.z, p2.z);
		pos = p1 * (1 - fraq) + p2 * fraq;

		Quaternion			q1, q2;
		RadianEuler			angle1, angle2;
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(3), pCompressed->scale[3], angle1.x, angle2.x);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(4), pCompressed->scale[4], angle1.y, angle2.y);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(5), pCompressed->scale[5], angle1.z, angle2.z);
	...
	{
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(0), pCompressed->scale[0], pos.x);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(1), pCompressed->scale[1], pos.y);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(2), pCompressed->scale[2], pos.z);

		RadianEuler			angle;
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(3), pCompressed->scale[3], angle.x);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(4), pCompressed->scale[4], angle.y);
		ExtractAnimValue_Hydra(iFrame, pCompressed->pAnimvalue(5), pCompressed->scale[5], angle.z);
	...
	)
{
	...
	BuildBoneChain_Hydra(pStudioHdr, rootXform, pos, q, iBone, boneToWorld, boneComputed);
	BuildBoneChain_Hydra(pStudioHdr, rootXform, pos, q, iNewParent, boneToWorld, boneComputed);
	...
	float cycle, int boneMask)
{
	...
	while (panim && panim->bone < 255)
	{
		...
		if (j >= 0 && (pStudioHdr->boneFlags(j) & boneMask))
		{
			...
			{
				CalcBoneQuaternion_Hydra(iLocalFrame, s, &pAnimbone[panim->bone], pAnimLinearBones, panim, q[j]);
				CalcBonePosition_Hydra(iLocalFrame, s, &pAnimbone[panim->bone], pAnimLinearBones, panim, pos[j]);
#ifdef STUDIO_ENABLE_PERF_COUNTERS
...
static void CalcAnimation_Hydra(const CStudioHdr *pStudioHdr, Vector *pos, Quaternion *q,
	...
	float cycle, int boneMask)
{
	...
	for (i = 0; i < pStudioHdr->numbones(); i++, pbone++, pweight++)
	{
		if (panim && panim->bone == i)
		{
			if (*pweight > 0 && (pStudioHdr->boneFlags(i) & boneMask))
			{
				CalcBoneQuaternion_Hydra(iLocalFrame, s, pbone, pLinearBones, panim, q[i]);
				CalcBonePosition_Hydra(iLocalFrame, s, pbone, pLinearBones, panim, pos[i]);
#ifdef STUDIO_ENABLE_PERF_COUNTERS
				pStudioHdr->m_nPerfAnimatedBones++;
				pStudioHdr->m_nPerfUsedBones++;
#endif
...
FORCEINLINE fltx4 QuaternionSM_HydraSIMD(float s, const fltx4 &p, const fltx4 &q)
{
	fltx4 p1, q1, result;
...
// Purpose: qt = p + s * q
//-----------------------------------------------------------------------------
void QuaternionAccumulate_Hydra(const Quaternion &p, float s, const Quaternion &q, Quaternion &qt)
{
	Quaternion q2;
	...
	int boneMask)
{
	...
	CBoneBitList_Hydra srcBoneComputed;

	matrix3x4_t *destBoneToWorld = g_MatrixPool.Alloc();
	CBoneBitList_Hydra destBoneComputed;

	matrix3x4_t *targetBoneToWorld = g_MatrixPool.Alloc();
	CBoneBitList_Hydra targetBoneComputed;
	...
	for (i = 0; i < pStudioHdr->numbones(); i++)
	{
		...
		else if (s2 > 0.0)
		{
			...
			BuildBoneChain_Hydra(pStudioHdr, rootXform, pos1, q1, i, destBoneToWorld, destBoneComputed);
			BuildBoneChain_Hydra(pStudioHdr, rootXform, pos2, q2, i, srcBoneToWorld, srcBoneComputed);
	...
	int boneMask)
{
	...
	{
		WorldSpaceSlerp_Hydra(pStudioHdr, q1, pos1, seqdesc, sequence, q2, pos2, s, boneMask);
		return;
	}
	...
	if (seqdesc.flags & STUDIO_DELTA)
	{
		for (i = 0; i < nBoneCount; i++)
		{
			...
			{
#ifndef _X360
				QuaternionMA_Hydra(q1[i], s2, q2[i], q1[i]);
#else
			...
			{
#ifndef _X360
				QuaternionSM_Hydra(s2, q2[i], q1[i], q1[i]);
#else
				fltx4 q1simd = LoadUnalignedSIMD(q1[i].Base());
				fltx4 q2simd = LoadAlignedSIMD(q2[i]);
				fltx4 result = QuaternionSM_HydraSIMD(s2, q2simd, q1simd);
				StoreUnalignedSIMD(q1[i].Base(), result);
#endif
...
void Calc3WayBlendIndices_Hydra(int i0, int i1, float s0, float s1, const mstudioseqdesc_t &seqdesc, int *pAnimIndices, float *pWeight)
{
	...
	)
{
	...
	Studio_LocalPoseParameter_Hydra(pStudioHdr, poseParameter, seqdesc, sequence, 0, s0, i0);
	Studio_LocalPoseParameter_Hydra(pStudioHdr, poseParameter, seqdesc, sequence, 1, s1, i1);


	if (seqdesc.flags & STUDIO_REALTIME)
	{
		float cps = Studio_CPS_Hydra(pStudioHdr, seqdesc, sequence, poseParameter);
		cycle = flTime * cps;
	...
	if (s0 < 0.001)
	{
		if (s1 < 0.001)
		{
			...
			{
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0, i1), cycle, boneMask);
			}
		}
		else if (s1 > 0.999)
		{
			CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0, i1 + 1), cycle, boneMask);
		}
		else
		{
			CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0, i1), cycle, boneMask);
			CalcAnimation_Hydra(pStudioHdr, pos2, q2, seqdesc, sequence, seqdesc.anim(i0, i1 + 1), cycle, boneMask);
	...
	else if (s0 > 0.999)
	{
		if (s1 < 0.001)
		{
			...
			{
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0 + 1, i1), cycle, boneMask);
			}
		}
		else if (s1 > 0.999)
		{
			CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0 + 1, i1 + 1), cycle, boneMask);
		}
		else
		{
			CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0 + 1, i1), cycle, boneMask);
			CalcAnimation_Hydra(pStudioHdr, pos2, q2, seqdesc, sequence, seqdesc.anim(i0 + 1, i1 + 1), cycle, boneMask);
			BlendBones_Hydra(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, s1, boneMask);
		}
	}
	else
	{
		if (s1 < 0.001)
		{
			...
			{
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0, i1), cycle, boneMask);
				ScaleBones_Hydra(pStudioHdr, q, pos, sequence, 1.0 - s0, boneMask);
			}
			else if (PoseIsAllZeros(pStudioHdr, sequence, seqdesc, i0, i1))
			{
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0 + 1, i1), cycle, boneMask);
				ScaleBones_Hydra(pStudioHdr, q, pos, sequence, s0, boneMask);
			}
			else
			{
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0, i1), cycle, boneMask);
				CalcAnimation_Hydra(pStudioHdr, pos2, q2, seqdesc, sequence, seqdesc.anim(i0 + 1, i1), cycle, boneMask);

				BlendBones_Hydra(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, s0, boneMask);
			}
		}
		else if (s1 > 0.999)
		{
		...
		else if (!anim_3wayblend.GetBool())
		{
			...
			CalcAnimation_Hydra(pStudioHdr, pos2, q2, seqdesc, sequence, seqdesc.anim(i0, i1 + 1), cycle, boneMask);
			CalcAnimation_Hydra(pStudioHdr, pos3, q3, seqdesc, sequence, seqdesc.anim(i0 + 1, i1 + 1), cycle, boneMask);
			BlendBones_Hydra(pStudioHdr, q2, pos2, seqdesc, sequence, q3, pos3, s0, boneMask);
		...
		else
		{
			...
			Calc3WayBlendIndices_Hydra(i0, i1, s0, s1, seqdesc, iAnimIndices, weight);
			...
			*/
			...
			{
				// on diagonal
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, iAnimIndices[0], cycle, boneMask);
				CalcAnimation_Hydra(pStudioHdr, pos2, q2, seqdesc, sequence, iAnimIndices[2], cycle, boneMask);
				BlendBones_Hydra(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, weight[2] / (weight[0] + weight[2]), boneMask);
			}
			else
			{
				CalcAnimation_Hydra(pStudioHdr, pos, q, seqdesc, sequence, iAnimIndices[0], cycle, boneMask);
				CalcAnimation_Hydra(pStudioHdr, pos2, q2, seqdesc, sequence, iAnimIndices[1], cycle, boneMask);
				BlendBones_Hydra(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, weight[1] / (weight[0] + weight[1]), boneMask);

				CalcAnimation_Hydra(pStudioHdr, pos3, q3, seqdesc, sequence, iAnimIndices[2], cycle, boneMask);
				BlendBones_Hydra(pStudioHdr, q, pos, seqdesc, sequence, q3, pos3, weight[2], boneMask);
			}
		}
	}
	...
	)
{
	...
	CalcPose_HydraSingle(pStudioHdr, pos, q, seqdesc, sequence, cycle, poseParameter, boneMask, flTime);
	...
	)
{
	...
	if (seqdesc.flags & STUDIO_LOCAL)
	{
		InitPose_Hydra(pStudioHdr, pos2, q2, boneMask);
	}

	if (CalcPose_HydraSingle(pStudioHdr, pos2, q2, seqdesc, sequence, cycle, poseParameter, boneMask, flTime))
	{
		// this weight is wrong, the IK rules won't composite at the correct intensity
		AddLocalLayers_hydra(pStudioHdr, pIKContext, pos2, q2, seqdesc, sequence, cycle, poseParameter, boneMask, 1.0, flTime);
		SlerpBones_Hydra(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, flWeight, boneMask);
	}
	...
	for (j = 0; j < pStudioHdr->numbonecontrollers(); j++)
	{
		...
		if (pStudioHdr->boneFlags(k) & boneMask)
		{
			...
			case STUDIO_XR:
				...
				QuaternionSM_Hydra(1.0, q0, q[k], q[k]);
				break;
			case STUDIO_YR:
...
//-----------------------------------------------------------------------------
bool Studio_SolveIK_Hydra(mstudioikchain_t *pikchain, Vector &targetFoot, matrix3x4_t *pBoneToWorld)
{
	if (pikchain->pLink(0)->kneeDir.LengthSqr() > 0.0)
	{
		...
		return Studio_SolveIK_Hydra(pikchain->pLink(0)->bone, pikchain->pLink(1)->bone, pikchain->pLink(2)->bone, targetFoot, targetKneePos, targetKneeDir, pBoneToWorld);
	}
	else
	{
		return Studio_SolveIK_Hydra(pikchain->pLink(0)->bone, pikchain->pLink(1)->bone, pikchain->pLink(2)->bone, targetFoot, pBoneToWorld);
	}
}
...
bool Studio_SolveIK_Hydra(int iThigh, int iKnee, int iFoot, Vector &targetFoot, matrix3x4_t *pBoneToWorld)
{
	...
	return Studio_SolveIK_Hydra(iThigh, iKnee, iFoot, targetFoot, worldKnee, ikKneeDir, pBoneToWorld);
}
...
bool Studio_SolveIK_Hydra(int iThigh, int iKnee, int iFoot, Vector &targetFoot, Vector &targetKneePos, Vector &targetKneeDir, matrix3x4_t *pBoneToWorld)
{
	...
	if (ik.solve(l1, l2, ikFoot.Base(), ikTargetKnee.Base(), ikKnee.Base()))
	{
		...
		Studio_AlignIKMatrix_Hydra(mWorldThigh, ikKnee);
		Studio_AlignIKMatrix_Hydra(mWorldKnee, ikFoot - ikKnee);
...
float Studio_IKRuleWeight_Hydra(mstudioikrule_t &ikRule, const mstudioanimdesc_t *panim, float flCycle, int &iFrame, float &fraq)
{
...
float Studio_IKRuleWeight_Hydra(ikcontextikrule_t_Hydra &ikRule, float flCycle)
{
	if (ikRule.end > 1.0f && flCycle < ikRule.start)
	{
		flCycle = flCycle + 1.0f;
...
bool Studio_IKShouldLatch(ikcontextikrule_t_Hydra &ikRule, float flCycle)
{
	if (ikRule.end > 1.0f && flCycle < ikRule.start)
	{
		flCycle = flCycle + 1.0f;
...
float Studio_IKTail(ikcontextikrule_t_Hydra &ikRule, float flCycle)
{
	if (ikRule.end > 1.0f && flCycle < ikRule.start)
	{
		flCycle = flCycle + 1.0f;
...
bool Studio_IKAnimationError_Hydra(const CStudioHdr *pStudioHdr, mstudioikrule_t *pRule, const mstudioanimdesc_t *panim, float flCycle, Vector &pos, Quaternion &q, float &flWeight)
{
	float fraq;
	int iFrame;

	flWeight = Studio_IKRuleWeight_Hydra(*pRule, panim, flCycle, iFrame, fraq);
...
bool Studio_IKSequenceError(const CStudioHdr *pStudioHdr, mstudioseqdesc_t &seqdesc, int iSequence, float flCycle, int iRule, const float poseParameter[], mstudioanimdesc_t *panim[4], float weight[4], ikcontextikrule_t_Hydra &ikRule)
{
	int i;
	...
	ikRule.flWeight = Studio_IKRuleWeight_Hydra(ikRule, flCycle);
	...
	for (i = 0; i < 4; i++)
	{
		if (weight[i])
		{
			...
			case IK_ATTACHMENT:
			{
				int bResult = Studio_IKAnimationError_Hydra(pStudioHdr, pRule, panim[i], flCycle, pos1, q1, w);

				if (bResult)
				{
					ikRule.pos = ikRule.pos + pos1 * weight[i];
					QuaternionAccumulate_Hydra(ikRule.q, weight[i], q1, ikRule.q);
					total += weight[i];
				}
			}
			break;
...
CIKContext_Hydra::CIKContext_Hydra()
{
...
void CIKContext_Hydra::Init(const CStudioHdr *pStudioHdr, const QAngle &angles, const Vector &pos, float flTime, int iFramecounter, int boneMask)
{
...
void CIKContext_Hydra::AddDependencies(mstudioseqdesc_t &seqdesc, int iSequence, float flCycle, const float poseParameters[], float flWeight)
{
	int i;
	...
	ikcontextikrule_t_Hydra ikrule;
	...
	if (seqdesc.flags & STUDIO_REALTIME)
	{
		float cps = Studio_CPS_Hydra(m_pStudioHdr, seqdesc, iSequence, poseParameters);
	...
	Studio_SeqAnims_Hydra(m_pStudioHdr, seqdesc, iSequence, poseParameters, panim, weight);
...
void CIKContext_Hydra::AddAutoplayLocks(Vector pos[], Quaternion q[])
{
	...
	memset(&m_ikLock[ikOffset], 0, sizeof(ikcontextikrule_t_Hydra)*m_pStudioHdr->GetNumIKAutoplayLocks());
	...
	for (int i = 0; i < m_pStudioHdr->GetNumIKAutoplayLocks(); i++)
	{
		...
		ikcontextikrule_t_Hydra &ikrule = m_ikLock[i + ikOffset];
...
void CIKContext_Hydra::AddSequenceLocks(mstudioseqdesc_t &seqdesc, Vector pos[], Quaternion q[])
{
	if (m_pStudioHdr->numikchains() == 0)
	{
		return;
	...
	memset(&m_ikLock[ikOffset], 0, sizeof(ikcontextikrule_t_Hydra) * seqdesc.numiklocks);
...
//-----------------------------------------------------------------------------
void CIKContext_Hydra::BuildBoneChain(
	const Vector pos[],
	const Quaternion q[],
	int	iBone,
	matrix3x4_t *pBoneToWorld,
	CBoneBitList_Hydra &boneComputed)
{
	...
	::BuildBoneChain_Hydra(m_pStudioHdr, m_rootxform, pos, q, iBone, pBoneToWorld, boneComputed);
}
	...
	CBoneBitList_Hydra &boneComputed)
{
	...
	{
		// evil recursive!!!
		BuildBoneChain_Hydra(pStudioHdr, rootxform, pos, q, parent, pBoneToWorld, boneComputed);
...
void CIKTarget_Hydra::SetOwner(int entindex, const Vector &pos, const QAngle &angles)
{
...
void CIKTarget_Hydra::ClearOwner(void)
{
	latched.owner = -1;
}
...
int CIKTarget_Hydra::GetOwner(void)
{
...
void CIKTarget_Hydra::UpdateOwner(int entindex, const Vector &pos, const QAngle &angles)
{
...
void CIKTarget_Hydra::SetPos(const Vector &pos)
{
	est.pos = pos;
}
...
void CIKTarget_Hydra::SetAngles(const QAngle &angles)
{
...
void CIKTarget_Hydra::SetQuaternion(const Quaternion &q)
{
	est.q = q;
}
...
void CIKTarget_Hydra::SetNormal(const Vector &normal)
{
...
void CIKTarget_Hydra::SetPosWithNormalOffset(const Vector &pos, const Vector &normal)
{
...
void CIKTarget_Hydra::SetOnWorld(bool bOnWorld)
{
...
void CIKTarget_Hydra::IKFailed(void)
{
...
void CIKTarget_Hydra::MoveReferenceFrame(Vector &deltaPos, QAngle &deltaAngles)
{
	est.pos -= deltaPos;
...
void CIKContext_Hydra::ClearTargets(void)
{
	int i;
...
void CIKContext_Hydra::UpdateTargets(Vector pos[], Quaternion q[], matrix3x4_t boneToWorld[], CBoneBitList_Hydra &boneComputed)
{
	int i, j;
	...
	for (j = 0; j < m_ikChainRule.Count(); j++)
	{
		...
		{
			ikcontextikrule_t_Hydra *pRule = &m_ikChainRule.Element(j).Element(i);

			// ikchainresult_t_Hydra *pChainRule = &chainRule[ m_ikRule[i].chain ];
				...
				// case IK_SELF:
			{
				...
				CIKTarget_Hydra *pTarget = &m_target[pRule->slot];
		...
		if (pTarget->est.flWeight > 0.0)
		{
			...
			if (pTarget->latched.bHasLatch)
			{
				if (pTarget->est.latched == 1.0)
				{
					...
					QuaternionSM_Hydra(-1, pTarget->est.q, pTarget->latched.q, pTarget->latched.deltaQ);
				...
				else if (pTarget->est.latched > 0.0)
				{
					...
					QuaternionMA_Hydra(pTarget->est.q, 1, pTarget->latched.deltaQ, pTarget->est.q);
...
void CIKContext_Hydra::AutoIKRelease(void)
{
	int i;
...
void CIKContext_Hydra::SolveDependencies(Vector pos[], Quaternion q[], matrix3x4_t boneToWorld[], CBoneBitList_Hydra &boneComputed)
{
	//	ASSERT_NO_REENTRY();
	...
	ikchainresult_t_Hydra chainResult[32]; // allocate!!!

	// init chain rules
	for (i = 0; i < m_pStudioHdr->numikchains(); i++)
	{
		mstudioikchain_t *pchain = m_pStudioHdr->pIKChain(i);
		ikchainresult_t_Hydra *pChainResult = &chainResult[i];
		int bone = pchain->pLink(2)->bone;
	...
	for (j = 0; j < m_ikChainRule.Count(); j++)
	{
		...
		{
			ikcontextikrule_t_Hydra *pRule = &m_ikChainRule.Element(j).Element(i);
			ikchainresult_t_Hydra *pChainResult = &chainResult[pRule->chain];
			pChainResult->target = -1;
	...
	for (i = 0; i < m_target.Count(); i++)
	{
		...
		if (m_target[i].est.flWeight > 0.0)
		{
			...
			ikchainresult_t_Hydra *pChainResult = &chainResult[pTarget->chain];
	...
	for (i = 0; i < m_pStudioHdr->numikchains(); i++)
	{
		...
		if (pChainResult->flWeight > 0.0)
		{
			...
			// FIXME: once per link!
			if (Studio_SolveIK_Hydra(pchain, pChainResult->pos, boneToWorld))
			{
				Vector p3;
				...
				SolveBone_Hydra(m_pStudioHdr, pchain->pLink(2)->bone, boneToWorld, pos, q);
				SolveBone_Hydra(m_pStudioHdr, pchain->pLink(1)->bone, boneToWorld, pos, q);
				SolveBone_Hydra(m_pStudioHdr, pchain->pLink(0)->bone, boneToWorld, pos, q);
			}
			else
			{
				...
				{
					CIKTarget_Hydra *pTarget = &m_target[pChainResult->target];
	...
	// do exact IK solution
	// FIXME: once per link!
	Studio_SolveIK_Hydra(pchain, p3, boneToWorld);
...
void CIKContext_Hydra::SolveAutoplayLocks(
	Vector pos[],
	Quaternion q[]
	)
{
...
void CIKContext_Hydra::SolveSequenceLocks(
	mstudioseqdesc_t &seqdesc,
	Vector pos[],
	Quaternion q[]
	)
...
void CIKContext_Hydra::AddAllLocks(Vector pos[], Quaternion q[])
{
	// skip all array access if no autoplay locks.
	...
	memset(&m_ikLock[ikOffset], 0, sizeof(ikcontextikrule_t_Hydra)*m_pStudioHdr->GetNumIKChains());
...
void CIKContext_Hydra::SolveAllLocks(
	Vector pos[],
	Quaternion q[]
	)
{
...
void CIKContext_Hydra::SolveLock(
	...
	)
{
	...
	{
		Studio_SolveIK_Hydra(pchain->pLink(0)->bone, pchain->pLink(1)->bone, pchain->pLink(2)->bone, p3, m_ikLock[i].kneePos, m_ikLock[i].kneeDir, boneToWorld);
	}
	else
	{
		Studio_SolveIK_Hydra(pchain, p3, boneToWorld);
	...
	)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		mstudioseqdesc_t &seqdesc = ((CStudioHdr *)pStudioHdr)->pSeqdesc(sequenceIndex);
		if (seqdesc.flags & STUDIO_AUTOPLAY)
		{
			float cycle = 0;
			float cps = Studio_CPS_Hydra(pStudioHdr, seqdesc, sequenceIndex, poseParameters);
			cycle = realTime * cps;
			cycle = cycle - (int)cycle;
...
/*
* This is for DoAimAtBone_Hydra below, was just for testing, not needed in general
* but to turn it back on, uncomment this and the section in DoAimAtBone_Hydra() below
*
	...
	)
{
	...
	if (pStudioHdr->boneFlags(iBone) & BONE_ALWAYS_PROCEDURAL)
	{
		...
		case STUDIO_PROC_AXISINTERP:
			DoAxisInterpBone_Hydra(pbones, iBone, bonetoworld);
			return true;

		case STUDIO_PROC_QUATINTERP:
			DoQuatInterpBone_Hydra(pbones, iBone, bonetoworld);
			return true;

		case STUDIO_PROC_AIMATBONE:
			DoAimAtBone_Hydra(pbones, iBone, bonetoworld, NULL);
			return true;

		case STUDIO_PROC_AIMATATTACH:
			DoAimAtBone_Hydra(pbones, iBone, bonetoworld, pStudioHdr);
			return true;
...
// Purpose:
//-----------------------------------------------------------------------------
bool SweepBoxToStudio_Hydra(IPhysicsSurfaceProps *pProps, const Ray_t& ray, CStudioHdr *pStudioHdr, mstudiohitboxset_t *set,
...
// Purpose:
//-----------------------------------------------------------------------------
bool TraceToStudio_Hydra(IPhysicsSurfaceProps *pProps, const Ray_t& ray, CStudioHdr *pStudioHdr, mstudiohitboxset_t *set,
	matrix3x4_t **hitboxbones, int fContentsMask, trace_t &tr)
{
	if (!ray.m_IsRay)
	{
		return SweepBoxToStudio_Hydra(pProps, ray, pStudioHdr, set, hitboxbones, fContentsMask, tr);
	}
...
void Studio_SeqAnims_Hydra(const CStudioHdr *pStudioHdr, mstudioseqdesc_t &seqdesc, int iSequence, const float poseParameter[], mstudioanimdesc_t *panim[4], float *weight)
{
	...
	Studio_LocalPoseParameter_Hydra(pStudioHdr, poseParameter, seqdesc, iSequence, 0, s0, i0);
	Studio_LocalPoseParameter_Hydra(pStudioHdr, poseParameter, seqdesc, iSequence, 1, s1, i1);
...
int Studio_MaxFrame_Hydra(const CStudioHdr *pStudioHdr, int iSequence, const float poseParameter[])
{
	...
	Studio_SeqAnims_Hydra(pStudioHdr, seqdesc, iSequence, poseParameter, panim, weight);
...
{
	mstudioseqdesc_t &seqdesc = ((CStudioHdr *)pStudioHdr)->pSeqdesc(iSequence);
	float cps = Studio_CPS_Hydra(pStudioHdr, seqdesc, iSequence, poseParameter);
...
bool Studio_AnimPosition_Hydra(mstudioanimdesc_t *panim, float flCycle, Vector &vecPos, QAngle &vecAngle)
{
	float	prevframe = 0;
	vecPos.Init();
	vecAngle.Init();
...
bool Studio_AnimVelocity_Hydra(mstudioanimdesc_t *panim, float flCycle, Vector &vecVelocity)
{
	float	prevframe = 0;
...
bool Studio_AnimMovement_Hydra(mstudioanimdesc_t *panim, float flCycleFrom, float flCycleTo, Vector &deltaPos, QAngle &deltaAngle)
{
	...
	Vector startPos;
	QAngle startA;
	Studio_AnimPosition_Hydra(panim, flCycleFrom, startPos, startA);

	Vector endPos;
	QAngle endA;
	Studio_AnimPosition_Hydra(panim, flCycleTo, endPos, endA);
...
float Studio_FindAnimDistance_Hydra(mstudioanimdesc_t *panim, float flDist)
{
	float	prevframe = 0;
...
bool Studio_SeqMovement_Hydra(const CStudioHdr *pStudioHdr, int iSequence, float flCycleFrom, float flCycleTo, const float poseParameter[], Vector &deltaPos, QAngle &deltaAngles)
{
	mstudioanimdesc_t *panim[4];
	float	weight[4];
	...
	for (int i = 0; i < 4; i++)
	{
		if (weight[i])
		{
			...
			if (Studio_AnimMovement_Hydra(panim[i], flCycleFrom, flCycleTo, localPos, localAngles))
			{
				found = true;
...
bool Studio_SeqVelocity_Hydra(const CStudioHdr *pStudioHdr, int iSequence, float flCycle, const float poseParameter[], Vector &vecVelocity)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (weight[i])
		{
			...
			if (Studio_AnimVelocity_Hydra(panim[i], flCycle, vecLocalVelocity))
			{
...
float Studio_FindSeqDistance_Hydra(const CStudioHdr *pStudioHdr, int iSequence, const float poseParameter[], float flDist)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (weight[i])
		{
			float flLocalCycle = Studio_FindAnimDistance_Hydra(panim[i], flDist);
