...
ConVar oc_blood_displays_on_all_normals("oc_blood_displays_on_all_normals", "0", FCVAR_REPLICATED | FCVAR_ARCHIVE);
...
void CBaseEntity::SetEffects( int nEffects )
{
	if ( nEffects != m_fEffects )
	{
		...
		if ( !IsPlayer() )
		{
			if ((nEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT | EF_BATTERYLIGHT | EF_HEALTHLIGHT)) && !(m_fEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT | EF_BATTERYLIGHT | EF_HEALTHLIGHT | EF_FRAGLIGHT | EF_FRAGSLIGHT | EF_FRAGFLIGHT | EF_EGONDISSOLVELIGHT)))
			{
				AddEntityToDarknessCheck( this );
			}
			else if (!(nEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT | EF_BATTERYLIGHT | EF_HEALTHLIGHT)) && (m_fEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT | EF_BATTERYLIGHT | EF_HEALTHLIGHT | EF_FRAGLIGHT | EF_FRAGSLIGHT | EF_FRAGFLIGHT | EF_EGONDISSOLVELIGHT)))
			{
...
void CBaseEntity::DecalTrace(const trace_t *pTrace, char const *decalName)
{
	int index = decalsystem->GetDecalIndexForName(decalName);
	if (index < 0)
		return;
...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	{
		FireBulletsHitScan(info);
	}
	else if (cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1)
	{
		FireBulletsDynamic(info);
	}
}

void CBaseEntity::FireBulletsHitScan(const FireBulletsInfo_t &info)
{
	...
	for (int iShot = 0; iShot < info.m_iShots; iShot++)
	{
		...
		if (info.m_bHoldedAttack)
		{
			CShotManipulator Manipulator2(info.m_vecDirShooting);
...
void CBaseEntity::FireBulletsDynamic(const FireBulletsInfo_t &info)
{
	// Make sure given a valid bullet type
	if (info.m_iAmmoType == -1)
	{
	...
	for (int iShot = 0; iShot < info.m_iShots; iShot++)
	{
		...
		CSimulatedBullet *pBullet = new CSimulatedBullet(info, nDamageType, nAmmoFlags, vecDir, pAttacker, info.m_pAdditionalIgnoreEnt, bTraceHull
#ifndef CLIENT_DLL
			, this
#endif
			);
		...
		for (int iShot = 0; iShot < info.m_iShots; iShot++)
		{
			if (iShot == 0 && info.m_iShots > 1 && (info.m_nFlags & FIRE_BULLETS_FIRST_SHOT_ACCURATE))
			{
				vecDir2 = Manipulator.GetShotDirection();
			}
			else
			{
				vecDir2 = Manipulator.ApplySpread(info.m_vecSpread);
			}

			if (info.m_bHoldedAttack)
			{
				vecDir2 = info.m_vecDirShooting;
			}

			vecEnd = info.m_vecSrc + vecDir2 * info.m_flDistance;
			vecEnd = g_pBulletManager->EndPoint;
			if (IsPlayer() && info.m_iShots > 1 && iShot % 2)
			{
				AI_TraceHull(info.m_vecSrc, vecEnd, Vector(-3, -3, -3), Vector(3, 3, 3), MASK_SHOT, &traceFilter, &tr);//vecEnd
			}
			else
			{
				AI_TraceLine(info.m_vecSrc, vecEnd, MASK_SHOT, &traceFilter, &tr);
		...
		MakeTracer(vecTracerSrc, Tracer, pAmmoDef->TracerType(info.m_iAmmoType));//Tracer
		iSeed++;
...
void CBaseEntity::FireBulletsShotgun(const FireBulletsInfo_t &info)
{
	if (((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 0)) || ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 2)))
	{
		FireBulletsHitScanShotgun(info);
	}
	else if (cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1)
	{
		FireBulletsDynamicShotgun(info);
	}
}


void CBaseEntity::FireBulletsHitScanShotgun(const FireBulletsInfo_t &info)
{
	static int	tracerCount;
	trace_t		tr;
	CAmmoDef*	pAmmoDef = GetAmmoDef();
...
void CBaseEntity::FireBulletsDynamicShotgun(const FireBulletsInfo_t &info)
{
	// Make sure given a valid bullet type
	if (info.m_iAmmoType == -1)
	{
	...
	for (int iShot = 0; iShot < info.m_iShots; iShot++)
	{
		...
		CSimulatedBulletShotgun *pBullet = new CSimulatedBulletShotgun(info, nDamageType, nAmmoFlags, vecDir, pAttacker, info.m_pAdditionalIgnoreEnt, bTraceHull
#ifndef CLIENT_DLL
			, this
#endif
			);

		BulletManagerShotgun()->AddBulletShotgun(pBullet);
		...
		for (int iShot = 0; iShot < info.m_iShots; iShot++)
		{
			...
			vecEnd = info.m_vecSrc + vecDir2 * info.m_flDistance;
			vecEnd = g_pBulletManagerShotgun->EndPoint;
			if (IsPlayer() && info.m_iShots > 1 && iShot % 2)
			{
				AI_TraceHull(info.m_vecSrc, vecEnd, Vector(-3, -3, -3), Vector(3, 3, 3), MASK_SHOT, &traceFilter, &tr);
			}
...
void CBaseEntity::FireBulletsVehicle(const FireBulletsInfo_t &info)
{
	static int	tracerCount;
	trace_t		tr;
	CAmmoDef*	pAmmoDef = GetAmmoDef();
	...
	//if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 0))
	bool bDoImpacts = false;
	bool bDoTracers = false;
	float flCumulativeDamage = 0.0f;
...
/*#ifndef CLIENT_DLL
			if (cvar->FindVar("oc_ragdoll_enable_blooddrips")->GetInt())
			{
				...
				pBlood->direction = -vecOrigin * Approach(1.f, 0.05f, 1);
				pBlood->timeThink = Approach(0.5f, 0.05f, 1);
				pBlood->composeColor = 2;
				pBlood->Spawn();
				pBlood->SetLocalOrigin(ptr->endpos);
				pBlood->SetAbsOrigin(ptr->endpos);
...
void CBaseEntity::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	...
	//DevMsg("pszTracerName: %s \n", pszTracerName);
	//DevMsg("iAttachment: %i \n", iAttachment);
	...
	if (cvar->FindVar("oc_particle_tracers")->GetInt())
	{
		if (GetOwnerEntity() && GetOwnerEntity()->IsNPC())
		{
			...
			{
			case TRACER_LINE:
				UTIL_ParticleTracer("weapon_npc_tracer_smg", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_LINE);//OverCharged
				break;
			case TRACER_LINE_AND_WHIZ:
				UTIL_ParticleTracer("weapon_npc_tracer_ar2", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_LINE_AND_WHIZ);//OverCharged
				break;
			case TRACER_357:
				UTIL_ParticleTracer("weapon_npc_tracer_357", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_357);//OverCharged
				break;
			case TRACER_SMG:
				UTIL_ParticleTracer("weapon_npc_tracer_smg", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_SMG);//OverCharged
				break;
			case TRACER_PISTOL:
				UTIL_ParticleTracer("weapon_npc_tracer_pistol", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_PISTOL);//OverCharged
				break;
			case TRACER_AR2:
				UTIL_ParticleTracer("weapon_npc_tracer_ar2", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_AR2);//OverCharged
				break;
			case TRACER_ABGUN:
				UTIL_ParticleTracer("weapon_npc_tracer_abgun", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_ABGUN);//OverCharged
				break;
			case TRACER_OICW:
				UTIL_ParticleTracer("weapon_npc_tracer_oicw", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_OICW);//OverCharged
				break;
			case TRACER_AK47:
				UTIL_ParticleTracer("weapon_npc_tracer_ak47", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_AK47);//OverCharged
				break;
			case TRACER_SHOTGUN:
				UTIL_ParticleTracer("weapon_npc_tracer_shotgun", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_SHOTGUN);//OverCharged
			case TRACER_SNIPER:
				UTIL_ParticleTracer("weapon_npc_tracer_sniper", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_SNIPER);//OverCharged
				break;
			}
		}
		else
		{
			...
			{
			case TRACER_LINE:
				UTIL_ParticleTracer("weapon_tracer_smg", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_LINE);//OverCharged
				break;
			case TRACER_LINE_AND_WHIZ:
				UTIL_ParticleTracer("weapon_tracer_ar2", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_LINE_AND_WHIZ);//OverCharged
				break;
			case TRACER_357:
				UTIL_ParticleTracer("weapon_tracer_357", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_357);//OverCharged
				break;
			case TRACER_SMG:
				UTIL_ParticleTracer("weapon_tracer_smg", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_SMG);//OverCharged
				break;
			case TRACER_PISTOL:
				UTIL_ParticleTracer("weapon_tracer_pistol", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_PISTOL);//OverCharged
				break;
			case TRACER_AR2:
				UTIL_ParticleTracer("weapon_tracer_ar2", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_AR2);//OverCharged
				break;
			case TRACER_ABGUN:
				UTIL_ParticleTracer("weapon_tracer_abgun", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_ABGUN);//OverCharged
				break;
			case TRACER_OICW:
				UTIL_ParticleTracer("weapon_tracer_oicw", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_OICW);//OverCharged
				break;
			case TRACER_AK47:
				UTIL_ParticleTracer("weapon_tracer_ak47", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_AK47);//OverCharged
				break;
			case TRACER_SHOTGUN:
				UTIL_ParticleTracer("weapon_tracer_shotgun", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_SHOTGUN);//OverCharged
			case TRACER_SNIPER:
				UTIL_ParticleTracer("weapon_tracer_sniper", vNewSrc, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_SNIPER);//OverCharged
				break;
			}
		}
	}
	else
	{
		...
		UTIL_Tracer(vNewSrc, tr.endpos, entindex(), iAttachment, 6000.0f, Whiz, pszTracerName);
...
void CBaseEntity::TraceBleed( float flDamage, const Vector &vecDir, trace_t *ptr, int bitsDamageType )
{
	...
	for ( i = 0 ; i < cCount ; i++ )
	{
		...
		if (!oc_blood_displays_on_all_normals.GetInt())
		{
