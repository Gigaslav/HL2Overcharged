...
static ConVarRef HostPitchLoop("host_timescale");
// Envelope
...
public:
	...
	CSoundPatch()
	{
		...
		HostPitchLoop = cvar->FindVar("host_timescale");
	}
	~CSoundPatch()
	{
		g_SoundPatchCount--;
	...
	void	InitOVR(IRecipientFilter *pFilter, CBaseEntity *pEnt, int channel, const char *pSoundName, soundlevel_t iSoundLevel);
...
void CSoundPatch::InitOVR(IRecipientFilter *pFilter, CBaseEntity *pEnt, int channel, const char *pSoundName,
	soundlevel_t soundlevel)
{
	m_hEnt = pEnt;
	m_entityChannel = channel;
	...
	m_iszSoundName = AllocPooledString(pSoundName);
	m_volume.SetValue(params.volume);
	m_pitch.SetValue(params.pitch);
	m_isPlaying = false;
	m_shutdownTime = 0;
	m_flLastTime = 0;
	m_Filter.Init(pFilter);
...
{
	return m_pitch.Value();// *GP->newPitch;
}
...
bool CSoundPatch::Update( float time, float deltaTime )
{
	...
	if ( m_flags && m_Filter.IsActive() )
	{
		...
		/*if (cvar->FindVar("host_timescale")->GetInt() < 1.0f)
		{

			if (SingleDo < 1)
			{
				SingleDo++;
				ep.m_nPitch = (int)m_pitch.Value() * GP->newPitch;
			}
			else if (SingleDo >= 1)
			{
...
public:
	...
	CSoundPatch		*SoundCreateOVR(IRecipientFilter& filter, int nEntIndex, int channel, const char *pSoundName);
...
{
	if (pCmd != NULL && pCmd->m_pPatch != NULL)
	{
...
CSoundPatch *CSoundControllerImp::SoundCreateOVR(IRecipientFilter& filter, int nEntIndex, int channel, const char *pSoundName)
{
#ifdef CLIENT_DLL
	if ( GameRules() )
	{
	...
	// FIXME: This is done so we don't have to futz with the public interface
	EHANDLE hEnt = (nEntIndex != -1) ? g_pEntityList->GetNetworkableHandle(nEntIndex) : NULL;
	pSound->InitOVR(&filter, hEnt.Get(), channel, pSoundName, SNDLVL_NORM);//CHAN_AUTO
...
CSoundPatch *CSoundControllerImp::SoundCreate( IRecipientFilter& filter, int nEntIndex, const EmitSound_t &es )
{
	...
	pSound->ChangePitch(es.m_nPitch * HostPitchLoop.GetFloat(), 0);
...
float CSoundControllerImp::SoundGetPitch( CSoundPatch *pSound )
{
	...
	return pSound->GetPitch();// *GP->newPitch;
}
