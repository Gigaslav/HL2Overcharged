...
void CSprite::ComputeWorldSpaceSurroundingBox( Vector *pVecWorldMins, Vector *pVecWorldMaxs )
{
	...
	if ( m_bWorldSpaceScale == false )
	{
		...
		float width = 0;
		float height = 0;
		if (modelinfo != NULL && GetModel() != NULL)
		{
			width = modelinfo->GetModelSpriteWidth(GetModel());
			height = modelinfo->GetModelSpriteHeight(GetModel());
			flScale *= MAX(width, height);
		}
		else
			Warning("OVR Error: sprite %s %s cannot be found on disk \n", this->GetClassname(), this->GetModelName());
	}
