...
public:
	...
	CNetworkVar(bool, m_bExpScope);
	CNetworkVar(bool, m_bDefRenderGroup);
	CNetworkVar(bool, m_bIsLeg);
	CNetworkVar(bool, IsNearWall);

	float m_expFactor; //ADDED
	float m_expFactor2; //ADDED
	bool  wasIronSighted;
	float delta;

#ifdef CLIENT_DLL
	float m_flNextLagScale;
	float lagScale;
	bool  alreadyInIronSight;
#endif

	int fov1;
	int fov2;
	bool toggled_do_once;
	...
	CBaseAnimatingOverlay			*pPlayerModel;
	CBaseEntity						*pVial;
	...
	virtual void			CalcViewModelLag(Vector& origin, QAngle& angles, QAngle& original_angles, int state, float lagScale);
	virtual void			CalcViewModelView( CBasePlayer *owner, const Vector& eyePosition, 
								const QAngle& eyeAngles );
	virtual void			AddViewModelBob( CBasePlayer *owner, Vector& eyePosition, QAngle& eyeAngles ) {};
	virtual void			AddViewModelBobRUN(CBasePlayer *owner, Vector& eyePosition, QAngle& eyeAngles) {};
