...
BEGIN_DATADESC( CPropVehicleDriveable )
	...
	DEFINE_FIELD(m_bAllowDetach, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bCanDetach, FIELD_BOOLEAN),
...
void CPropVehicleDriveable::EnterVehicle( CBaseCombatCharacter *pPassenger )
{
	...
	if ( pPlayer != NULL )
	{
		...
		if (pPlayer->grenadeState > 0)
			return;
		if (pPlayer->GetActiveWeapon() && (pPlayer->GetActiveWeapon()->IsIronSighted() || pPlayer->GetActiveWeapon()->IsScopeSighted()))
			return;
...
void CPropVehicleDriveable::GunText(float radius, bool detach, const char *textOn, const char *textOff)
{
	CBasePlayer *pOwner = ToBasePlayer(UTIL_GetLocalPlayer());
	if (pOwner)
	{
		if (detach)
			UTIL_HudHintText(pOwner, textOff);
		else
			UTIL_HudHintText(pOwner, textOn);			
	}
}
bool CPropVehicleDriveable::CheckAttachedGun(const char *attachment, float radius, bool offset)
{
	...
	if (pServerVehicle)
	{
		...
		Vector vecOrigin;
		QAngle ang;
		GetAttachment(iattachment, vecOrigin, ang);

		/*if (offset)
		{
			vecOrigin += vForward * 4.f;
			vecOrigin += vRight * 5.f;
		}*/
		...
		//Need to compare two directions: from tau and from player(*-1)

		if (!pServerVehicle->GetPassenger() && UTIL_GetLocalPlayer() != NULL && UTIL_GetLocalPlayer()->FInViewCone(this) && UTIL_GetLocalPlayer()->FInViewCone(vecOrigin))
		{
			CBaseEntity *ppEnts[16];// = gEntList.FindEntityInSphere(this, vecOrigin, radius);
			int nEntCount = UTIL_EntitiesInSphere(ppEnts, 16, vecOrigin, radius, 0);
			...
			for (int i = 0; i < nEntCount; i++)
			{
				...
				if (ppEnts[i] != NULL)
				{
					...
					{
						CBasePlayer *pOwner = ToBasePlayer(ppEnts[i]);
						if (pOwner)
						{
							//debugoverlay->AddLineOverlay(pOwner->EyePosition(), vecOrigin, 255, 255, 0, false, 0.1f);
							/*trace_t trVehicle;
							UTIL_TraceLine(vecOrigin, vecOrigin + pOwner->EyePosition(), MASK_SHOT, pOwner, COLLISION_GROUP_PLAYER, &trVehicle);
							
							QAngle resAngleVehicle;
							QAngle resAnglePlayer;

							Vector	vForward, vRight, vUp;

							pOwner->EyeVectors(&vForward, &vRight, &vUp);

							Vector playerDir = pOwner->EyePosition() + vForward;// +pOwner->EyeDirection3D();

							//VectorAngles(trVehicle.startpos + trVehicle.endpos, resAngleVehicle);
							
							//VectorAngles(playerDir, resAnglePlayer);

							Vector vehicleDir = trVehicle.startpos + trVehicle.endpos;

							int newVector = VectorCompare(vehicleDir, playerDir);
							
							DevMsg("diff: %i\n", newVector);*/

							/*float angDiff_x = newVector.x;
							float angDiff_y = newVector.y;
							float angDiff_z = newVector.z;*/

							//DevMsg("diff: %.2f\n %.2f\n %.2f\n", angDiff_x, angDiff_y, angDiff_z);
					...
					{
						/*if (!FStrEq(ppEnts[i]->GetClassname(), "prop_vehicle_airboat"))
							return false;*/
					}
				}
				/*else
