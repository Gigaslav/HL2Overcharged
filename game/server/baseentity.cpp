...
//	DEFINE_FIELD( m_nPushEnumCount, FIELD_INTEGER ),
	...
	DEFINE_KEYFIELD(m_bInitStatic, FIELD_BOOLEAN, "freeze"),
...
CBaseEntity * CBaseEntity::CreateNoSpawnInstance(const char *szName, const Vector &vecOrigin, const QAngle &vecAngles)
{
	CBaseEntity *pEntity = CreateEntityByName(szName);
	if (!pEntity)
	{
...
int CBaseEntity::PrecacheModel( const char *name, bool bPreload )
{
	...
	if ( !CBaseEntity::IsPrecacheAllowed() )
	{
		if ( !engine->IsModelPrecached( name ) )
		{
			...
			Warning( "baseentity.cpp: Late precache of %s\n", name );
		}
	}
#if defined( WATCHACCESS )
	else
...
void CC_Ent_Remove( const CCommand& args )
{
	...
	if ( pEntity )
	{
		if (Q_strstr(STRING(MAKE_STRING(pEntity->GetClassname())), "weapon_frag")
			|| Q_strstr(STRING(MAKE_STRING(pEntity->GetClassname())), "weapon_hopwire")
			|| Q_strstr(STRING(MAKE_STRING(pEntity->GetClassname())), "weapon_tripwire")
			|| Q_strstr(STRING(MAKE_STRING(pEntity->GetClassname())), "weapon_molotov")
			|| Q_strstr(STRING(MAKE_STRING(pEntity->GetClassname())), "weapon_smokegrenade")
			|| Q_strstr(STRING(MAKE_STRING(pEntity->GetClassname())), "weapon_flashbang"))
		{
			CBaseCombatWeapon *pGren = static_cast<CBaseCombatWeapon*>(pEntity);
			UTIL_GetCommandClient()->RemoveGrenadeFromSlot(pGren);
		}
	...
	else 
	{
		...
		while ( (ent = gEntList.NextEnt(ent)) != NULL )
		{
			if (  (ent->GetEntityName() != NULL_STRING	&& FStrEq(args[1], STRING(ent->GetEntityName())))	|| 
				  ...
				  (ent->GetClassname()!=NULL && FStrEq(args[1], ent->GetClassname())))
			{
				if (Q_strstr(STRING(MAKE_STRING(ent->GetClassname())), "weapon_frag")
					|| Q_strstr(STRING(MAKE_STRING(ent->GetClassname())), "weapon_hopwire")
					|| Q_strstr(STRING(MAKE_STRING(ent->GetClassname())), "weapon_tripwire")
					|| Q_strstr(STRING(MAKE_STRING(ent->GetClassname())), "weapon_molotov")
					|| Q_strstr(STRING(MAKE_STRING(ent->GetClassname())), "weapon_smokegrenade")
					|| Q_strstr(STRING(MAKE_STRING(ent->GetClassname())), "weapon_flashbang"))
				{
					CBaseCombatWeapon *pGren = static_cast<CBaseCombatWeapon*>(ent);
					UTIL_GetCommandClient()->RemoveGrenadeFromSlot(pGren);
				}
...
void CBaseEntity::SetLocalAngles( const QAngle& angles )
{
	...
	if ( !IsEntityQAngleReasonable( angles ) )
	{
		if ( CheckEmitReasonablePhysicsSpew() )
		{
			//Warning( "Bad SetLocalAngles(%f,%f,%f) on %s\n", angles.x, angles.y, angles.z, GetDebugName() );//OverCharged
		}
		Assert( false );
		return;
	}
...
void CBaseEntity::SetLocalVelocity( const Vector &inVecVelocity )
{
	...
	{
		case -1:
			//Warning( "Discarding SetLocalVelocity(%f,%f,%f) on %s\n", vecVelocity.x, vecVelocity.y, vecVelocity.z, GetDebugName() );//OverCharged
			Assert( false );
			return;
		case 0:
			if ( CheckEmitReasonablePhysicsSpew() )
			{
				//Warning( "Clamping SetLocalVelocity(%f,%f,%f) on %s\n", inVecVelocity.x, inVecVelocity.y, inVecVelocity.z, GetDebugName() );//OverCharged
			}
			break;
	}
...
void CC_Ent_Create( const CCommand& args )
{
	...
	if (entity)
	{
		...
		if ( tr.fraction != 1.0 )
		{
			//DevMsg("args[1] %s \n", args[1]);
			//DevMsg("args[2] %s \n", args[2]);
			if (!Q_stricmp(args[2], "plrang") && args[3] != NULL)
			{
				QAngle plrAng = entity->GetAbsAngles();
				if (!Q_stricmp(args[3], "x"))
					plrAng.x = pPlayer->GetAbsAngles().x;
				if (!Q_stricmp(args[3], "y"))
					plrAng.y = pPlayer->GetAbsAngles().y;
				if (!Q_stricmp(args[3], "z"))
					plrAng.z = pPlayer->GetAbsAngles().z;

				tr.endpos.z += 12;
				entity->Teleport(&tr.endpos, &plrAng, NULL);
				UTIL_DropToFloor(entity, MASK_SOLID);
			}
			else if (!Q_stricmp(args[2], "plrrevang") && args[3] != NULL)
			{
				QAngle plrAng = entity->GetAbsAngles();
				if (!Q_stricmp(args[3], "x"))
					plrAng.x = pPlayer->GetAbsAngles().x + 180;
				if (!Q_stricmp(args[3], "y"))
					plrAng.y = pPlayer->GetAbsAngles().y + 180;
				if (!Q_stricmp(args[3], "z"))
					plrAng.z = pPlayer->GetAbsAngles().z + 180;
