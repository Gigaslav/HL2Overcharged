...
#include "mapadd.h"//OverCharged
#include "OverCharged/map_parser.h"//OverCharged
#include "OverCharged/luamanager.h"//OverCharged
...
ConVar oc_mapadd_enabled("oc_mapadd_enabled", "0", FCVAR_ARCHIVE);

#include <iostream>
#include <vector>
//CUtlVector<string_t> passedMapsList;
std::vector < std::string > passedMapsList;
...
bool CServerGameDLL::LevelInit( const char *pMapName, char const *pMapEntities, char const *pOldLevel, char const *pLandmarkName, bool loadGame, bool background )
{
	...
	while (pEntity)
	{
		DevMsg("pEntity founded: %s \n", pEntity->GetClassname());

		char Name2[64];
		Q_snprintf(Name2, sizeof(Name2), "%s", pEntity->GetClassname());
		char Name1[64];
		Q_snprintf(Name1, sizeof(Name1), "%s", bmName);
		if (Q_strcmp(Name1, Name2) == 0)
		{
			DevMsg("bullet_manager founded: %s \n", pEntity->GetClassname());
			UTIL_Remove(pEntity);
		}

		char Name0[64];
		Q_snprintf(Name0, sizeof(Name0), "%s", bmsName);
		if (Q_strcmp(Name0, Name2) == 0)
		{
			DevMsg("bullet_manager_shotgun founded: %s \n", pEntity->GetClassname());
			UTIL_Remove(pEntity);
		}

		pEntity = gEntList.NextEnt(pEntity);
	}
	g_pBulletManager = (CBulletManager*)CBaseEntity::Create(bmName, vec3_origin, vec3_angle);
	g_pBulletManagerShotgun = (CBulletManagerShotgun*)CBaseEntity::Create(bmsName, vec3_origin, vec3_angle);


	//MapAdd logic below
	if (gpGlobals->eLoadType == MapLoad_NewGame)
	{
		passedMapsList.clear();
	}
	else if (gpGlobals->eLoadType == MapLoad_LoadGame)
	{
		std::string mapName = pMapName;
		int i = 0;
		for (; i < (int)passedMapsList.size(); ++i)
		{
			std::string mapNameOld = passedMapsList.at(i);
			if (mapNameOld.compare(mapName) == 0)
				break;
		}

		while ((int)passedMapsList.size() - 1 > i)
			passedMapsList.pop_back();
	}

	std::string mapName = pMapName;

	bool simularMap = false;
	for (int i = 0; i < (int)passedMapsList.size(); ++i)
	{
		std::string mapNameOld = passedMapsList.at(i);
		if (mapNameOld.compare(mapName) == 0)
		{
	...
	if (!simularMap)
	{
		passedMapsList.push_back(mapName);
	}

	if (!simularMap && gpGlobals->eLoadType != MapLoad_LoadGame)
	{
		//Pre Init
		if (GetMapScriptParser())
		{
			GetMapScriptParser()->LevelInitParsing();
		}
	}
...
static TITLECOMMENT gTitleComments[] =
{
	...
	{ "d2_lostcoast", "#LC_Chapter1_Title" },
#endif
};
...
void CServerGameClients::ClientActive( edict_t *pEdict, bool bLoadGame )
{
	...
	//Post Init
	if (GetMapScriptParser() && gpGlobals->eLoadType == MapLoad_NewGame)
		GetMapScriptParser()->LevelPostInitParsing();
