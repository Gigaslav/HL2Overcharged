...
public:
	...
	void Spawn( void );
	void Precache( void );
	void CreateBloodStream(trace_t *ptr);

	void ConstrainWeapon(CBaseCombatWeapon *pWeapon, const char* pAttachmentName, const char* pWeaponName);
	void ConstraintedWeaponThink();
private:
	void DispatchBloodOverlay(const CTakeDamageInfo &info, int BloodColor);

	CBloodEmitter *pBlood = NULL;
	//CBaseEntity *pBlood = NULL;
	QAngle origAngle;
	Vector originPos;
	Vector direction;
	int hitbox;
	IPhysicsConstraint *m_pConstraintBloodStream;
	//CBaseCombatWeapon *pDroppedWeapon;
	CNetworkHandle(CBaseCombatWeapon, pDroppedWeapon);
	IPhysicsConstraint *m_pConstraint;
	bool constrainted;
	float m_flNextDropConstrainted;
public:
...
CBaseEntity *CreateServerRagdoll( CBaseAnimating *pAnimating, int forceBone, const CTakeDamageInfo &info, int collisionGroup, bool bUseLRURetirement = false, int bloodColor = -1 );
CBaseEntity *CreateServerRagdoll(CBaseAnimating *pAnimating, int forceBone, const CTakeDamageInfo &info, int collisionGroup, CBaseCombatWeapon *pWeapon, const char* pAttachmentName, const char* pWeaponName, bool constraintWeapon = false, bool bUseLRURetirement = false, int bloodColor = -1);
...
CBaseAnimating *CreateServerRagdollSubmodel(CBaseAnimating *pOwner, const char *pModelName, const Vector &position, const QAngle &angles, int collisionGroup, int bloodColor = -1);
