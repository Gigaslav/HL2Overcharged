...
{
	int pitch = m_dpv.pitch/* * GP->newPitch*/;
	int vol = m_dpv.vol;
	int flags = 0;
	...
	if (m_dpv.spinup || m_dpv.spindown)
	{
		...
		{
			pitch = m_dpv.pitchrun/* * GP->newPitch*/;
			m_dpv.spinup = 0;				// done with ramp up
		}

		if (pitch < m_dpv.pitchstart)
		{
			pitch = m_dpv.pitchstart/* * GP->newPitch*/;
	...
	if (flags && fChanged) 
	{
		...
		if (pSoundSource)
		{
			UTIL_EmitAmbientSound(pSoundSource->GetSoundSourceIndex(), pSoundSource->GetAbsOrigin(), 
				STRING(m_iszSound), (vol * 0.01), m_iSoundLevel, flags, pitch/*  * GP->newPitch*/);
		}
	}
...
void CAmbientGeneric::InitModulationParms(void)
{
	...
	if (m_dpv.spinup)
		m_dpv.pitch = m_dpv.pitchstart/* * GP->newPitch*/;
	else
		m_dpv.pitch = m_dpv.pitchrun/* * GP->newPitch*/;
...
void CAmbientGeneric::SendSound( SoundFlags_t flags)
{
	...
	if ( pSoundSource )
	{
		...
		{
			UTIL_EmitAmbientSound(pSoundSource->GetSoundSourceIndex(), pSoundSource->GetAbsOrigin(), szSoundFile, 
				(m_dpv.vol * 0.01), m_iSoundLevel, flags, m_dpv.pitch/* * GP->newPitch*/);
		}
	}	
	else
	{
					  ...
					  float volume, soundlevel_t soundlevel, int flags, int pitch)
{
	...
	if (ipick > 0 && name)
	{
		...
		CBaseEntity::EmitSentenceByIndex(filter, ENTINDEX(entity), CHAN_VOICE, sentenceIndex, volume, soundlevel, flags, pitch * GP->newPitch());
...
void SENTENCEG_PlaySentenceIndex( edict_t *entity, int iSentenceIndex, float volume, soundlevel_t soundlevel, int flags, int pitch )
{
	...
	{
		CPASAttenuationFilter filter( GetContainingEntity( entity ), soundlevel );
		CBaseEntity::EmitSentenceByIndex(filter, ENTINDEX(entity), CHAN_VOICE, iSentenceIndex, volume, soundlevel, flags, pitch * GP->newPitch());
	}
}
...
void UTIL_EmitSoundSuit(edict_t *entity, const char *sample)
{
	...
	if (random->RandomInt(0,1))
		pitch = (random->RandomInt(0, 6) + 98)/* * GP->newPitch*/;
