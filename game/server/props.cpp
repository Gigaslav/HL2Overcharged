...
/*            /////////////////////////////////////////////////////////////////OverCharged///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			{
				DispatchParticleEffect("grenade_explosion_01", WorldSpaceCenter(), vec3_angle);
//				DispatchParticleEffect("weapon_grenade_attachments", WorldSpaceCenter(), vec3_angle);
				...
				{
					DispatchParticleEffect("grenade_explosion_01", ftr.endpos, vec3_angle);
					FireSystem_StartFire(ftr.endpos, 256, 0.0f, 8.0f, (SF_FIRE_START_ON | SF_FIRE_START_FULL | SF_FIRE_SMOKELESS | SF_FIRE_NO_GLOW | SF_FIRE_DONT_DROP), (CBaseEntity*)this, FIRE_NATURAL );
					CPASFilter filter(GetAbsOrigin());
					te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 255, 95, 55, 4, 150, 0.4, 0);//OverCharged
				}
			}
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
		}
...
LINK_ENTITY_TO_CLASS(prop_physics_multiplayer, CPhysicsProp); //OVERCHARGED: Fix for CS:S and HL2DM maps
...
void CPhysicsProp::Spawn( )
{
	...
	//OVERCHARGED: We need to do the same for this
...
BEGIN_DATADESC(CBasePropDoor)
	...
	DEFINE_KEYFIELD(m_SoundKickOpen, FIELD_SOUNDNAME, "soundkick"),		// Overcharged player kick
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "Kickable", InputKickable),		// Overcharged player kick
	...
	DEFINE_OUTPUT(m_OnBroken, "OnBroken"), // BriJee: Overcharged added
...
void CBasePropDoor::CalcDoorSounds()
{
	...
	UTIL_ValidateSoundName(m_SoundKickOpen, "d1_trainstation_03.breakin_doorkick");	// overcharged player kick
	...
	PrecacheScriptSound(STRING(m_SoundKickOpen));		// overcharged player kick
...
void CBasePropDoor::TakeKick(const CTakeDamageInfo &info)
{
	if (info.GetAttacker() && info.GetAttacker()->IsPlayer() && (info.GetDamageType() & DMG_CLUB))
	{
		if (IsDoorClosed() || (IsDoorOpen() && HasSpawnFlags(SF_DOOR_USE_CLOSES)))
		{
			if (m_bLocked)
			...
			else
			{
				...
				else
				{
					if (VPhysicsGetObject())
					{
						...
						if (pPlayer)
						{
							//Vector impactVel = pPlayer->GetAutoaimVector(0);
							Vector vecLocal = VPhysicsGetObject()->GetMassCenterLocalSpace();
							Vector vecWorld;
							VPhysicsGetObject()->LocalToWorld(&vecWorld, vecLocal);

							Vector impactPoint = info.GetDamagePosition();
							Vector impactVel = info.GetDamageForce();
							VectorScale(info.GetDamageForce(), 0.1f, impactVel);
							//VectorScale(impactVel, 15000.f, impactVel);

							UpdateAreaPortals(true);

							m_OnOpen.FireOutput(this, this);

							BreakDoors((vecWorld + impactVel), Vector(0, 0, 145));

							CSingleUserRecipientFilter filter(pPlayer);						
							EmitSound(filter, entindex(), STRING(m_SoundKickOpen), &impactPoint, 0.f, NULL);						
						}
					}
				}
...
void CBasePropDoor::Kick(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	// If we're blocked while closing, open away from our blocker. This will
	// liberate whatever bit of detritus is stuck in us.
	if (IsDoorBlocked() && IsDoorClosing())
	{
		m_hActivator = pActivator;
		DoorKicked(m_hActivator);
		CBasePlayer *pPlayer = ToBasePlayer(pActivator);
		if (pPlayer)
		{
			CSingleUserRecipientFilter filter(pPlayer);

			EmitSound(filter, entindex(), STRING(m_SoundKickOpen), &GetAbsOrigin(), 0.f, NULL);
		}
		return;
	}
...
void CBasePropDoor::InputKickable(inputdata_t &inputdata)		// overcharged player kick
{
...
void CBasePropDoor::DoorKicked(CBaseEntity *pOpenAwayFrom)
{
	// Don't bother if we're already doing this
	if (IsDoorOpen() || IsDoorOpening())
		return;
	...
	if (HasSlaves())
	{
		...
		for (int i = 0; i < numDoors; i++)
		{
			...
			if (pLinkedDoor != NULL)
			{
				// If the door isn't already moving, get it moving
				pLinkedDoor->m_hActivator = m_hActivator;
				pLinkedDoor->DoorKicked(pOpenAwayFrom);
			}
		}
	}
}
...
bool CBasePropDoor::AreWeLocked(void)		//overcharged player kick
{
	if (GetMaster())
	{
...
public:
	...
	void	BeginKicked(CBaseEntity *pOpenAwayFrom);
...
void CPropDoorRotating::BeginKicked(CBaseEntity *pOpenAwayFrom)
{
	// Determine the direction to open.
	QAngle angOpen = m_angRotationOpenForward;
	doorCheck_e eDirCheck = DOOR_CHECK_FORWARD;
	...
	AngularMove(angOpen, m_flSpeed * 5.f);
}
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
