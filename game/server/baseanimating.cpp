...
bool CBaseAnimating::IsValidRagdollName()
{
	//bool canBRagdoll = BaseClass::CanBecomeRagdoll();

	/*IPhysicsObject *pPhys = VPhysicsGetObject();
	if (!pPhys)
	return false;*/

	bool invalidClassName = FClassnameIs(this, "npc_rollermine") ||
		//FClassnameIs(this, "npc_manhack") ||
		FClassnameIs(this, "npc_turret_floor") ||
		FClassnameIs(this, "npc_scanner") ||
		FClassnameIs(this, "npc_hydra");

	return !invalidClassName;
}

bool CBaseAnimating::BecomeRagdoll(const CTakeDamageInfo &info, const Vector &forceVector)
{
	if (CanBecomeRagdoll() && IsValidRagdollName())
	{
...
float CBaseAnimating::GetSequenceCycleRate( CStudioHdr *pStudioHdr, int iSequence )
{
	...
	//pStudioHdr->pAnimdesc(GetSequence()).fps = 5;
	if (t > 0.0f)
	{
		return 1.0f / t;
	}
...
/*void CBaseAnimating::SetSequenceCycleRate(CStudioHdr *pStudioHdr, int iSequence, int fps)
{
	//float t = SequenceDuration(pStudioHdr, iSequence);
	pStudioHdr->pAnimdesc(GetSequence()).fps = fps;
}*/
...
void CBaseAnimating::HandleAnimEvent( animevent_t *pEvent )
{
	if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER))
	{
		...
		{
			if ((GetFlags() & FL_ONFIRE) || cvar->FindVar("oc_ragdoll_serverside")->GetInt())
			{
				CTakeDamageInfo dmgInfo(this, NULL, 0.f, DMG_NEVERGIB);
				dmgInfo.SetDamageForce(vec3_origin);

				BecomeRagdoll(dmgInfo, vec3_origin);
			}
			else
...
float CBaseAnimating::SetNonThresholdPoseParameter(CStudioHdr *pStudioHdr, const char *szName, float flValue)
{
	...
	return SetNonThresholdPoseParameter(pStudioHdr, poseParam, flValue);
}
float CBaseAnimating::SetNonThresholdPoseParameter(CStudioHdr *pStudioHdr, int iParameter, float flValue)
{
	if (!pStudioHdr)
	{
		return flValue;
	...
	/*if (pStudioHdr->pBone(iParameter - 1))
	{
		pStudioHdr->pBone(iParameter - 1)->rotscale = Vector(1,1,1);
		pStudioHdr->pBone(iParameter - 1)->rot.x = flValue;
		pStudioHdr->pBone(iParameter - 1)->quat.x = flValue;
		pStudioHdr->pBone(iParameter - 1)->qAlignment.x = flValue;
	}*/
	/*pStudioHdr->pBone(iParameter)->rotscale = Vector(1, 1, 1);
	pStudioHdr->pBone(iParameter)->rot.x = flValue;
	pStudioHdr->pBone(iParameter)->quat.x = flValue;
	pStudioHdr->pBone(iParameter)->qAlignment.x = flValue;*/

	if (iParameter >= 0)
	{
		float flNewValue;
		flValue = Studio_SetPoseParameter(pStudioHdr, iParameter, flValue, flNewValue);
		m_flPoseParameter.Set(iParameter, flValue);
	}
...
#ifndef CLIENT_DLL
void CBaseAnimating::SetBoneTransform(int iBone, const Vector &in)
{
	CStudioHdr *pStudioHdr = GetModelPtr();
	...
	//matrix3x4_t& pBoneToWorld = GetBoneTransform(iBone, pBoneToWorld);
	...
	MatrixSetTranslation(in, pBoneToWorld);*/
}
#endif
//=========================================================
//=========================================================
...
void CBaseAnimating::DoMuzzleFlashOVR(int attachmentIndex, int flashType, int r, int g, int b, const char *name)
{
}
//-----------------------------------------------------------------------------
// Purpose: 
...
bool CBaseAnimating::Dissolve_Egon(const char *pMaterialName, float flStartTime, bool bNPCOnly, int nDissolveType, Vector vDissolverOrigin, int iMagnitude)
{
	// Right now this prevents stuff we don't want to catch on fire from catching on fire.
	if (bNPCOnly && !(GetFlags() & FL_NPC))
		return false;
	...
	bool bRagdollCreated = false;
	CEntityDissolveEgon *pDissolve = CEntityDissolveEgon::Create(this, pMaterialName, flStartTime, nDissolveType, &bRagdollCreated);
	if (pDissolve)
	{
		SetEffectEntity(pDissolve);
...
/*void CBaseAnimating::CalcFirstPersonDeathViewServer(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	Msg("CalcFirstPersonDeathViewServer \n");
	...
	if (m_lifeState != LIFE_ALIVE)
	{
		...
		Msg("m_lifeState != LIFE_ALIVE \n");

		CBaseEntity *pRagdoll = NULL;

		CTakeDamageInfo newinfo;
		newinfo.SetDamageForce(vec3_origin);
		newinfo.SetDamageType(pPlayer->GetDamageType());
		
		if (pPlayer && !pPlayer->IsDormant())
		{
			//pPlayer->GetRagdollInitBoneArrays(boneDelta0, boneDelta1, currentBones, boneDt);

			pRagdoll = CreateServerRagdoll(this, m_nForceBone, newinfo, COLLISION_GROUP_INTERACTIVE_DEBRIS, true);
		}
		else
		{
			pRagdoll = CreateServerRagdoll(this, m_nForceBone, newinfo, COLLISION_GROUP_INTERACTIVE_DEBRIS, true);
		...
		if (pRagdoll)
		{
			...
			pPlayer->SetParent(pRagdoll);

			Msg("Parent: %s \n", pPlayer->GetParent()->GetClassname());

			int m_iEyeAttachment = LookupAttachment("eyes");
