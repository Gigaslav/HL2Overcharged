...
#include "OverCharged/hl2_player_body.h"
...
public:
	...
	virtual void			CreateViewModel(int viewmodelindex = 0, bool DefRenderGroup = false);
	CNetworkVar(bool, m_bDefRenderGroup);

	//virtual void	        CreateHandModel(int viewmodelindex = 1, int iOtherVm = 0);			// BriJee OVR: C_ arms aka custom hands method
	CBaseViewModel			*GetViewModel(int viewmodelindex = 0, bool bObserverOK = true, bool DefRenderGroup = false);
...
#endif
	CBaseViewModel			*pViewModelWeapon;
	CPlayerModel			*pPlayerModel;
	void					ReadDataFromFile(IFileSystem *filesystem);
	void					DropGarbage(void);
	bool					CanDropGarbage;
	bool					IsSighted();
	void					OffSights();
	float					m_flSwingEventAnimTime;
	CUtlVector<string_t>	garbage;
	CUtlVector<float>		garbageIndex;
...
#ifndef CLIENT_DLL
	virtual Vector			Weapon_ShootPosition(int attachment);
	virtual Vector			Weapon_ShootPosition(const char *attachment);
#endif
...
public:
	...
	CNetworkVar(bool,		m_bIsRunning);
	//CNetworkVar(bool,		m_bIsSighted);
	bool					GetIsInSwing();
	bool					throwingStateGate;
	int						throwingState;
	int						grenadeState = 0;
	float					m_flNextGrenadeThrow;
	float					sequenceDuration = 0.f;
	float					m_flNextGrenadeSelect = 0.f;
	int						grenType;
	CBaseCombatWeapon		*pCurWeapon = NULL;
	CBaseCombatWeapon		*pCurGrenade = NULL;
	//CUtlVector<CBaseCombatWeapon*> pAllGrenades;
	//CNetworkArray(CBaseCombatWeaponHandle, pAllGrenades, MAX_WEAPONS);
	CUtlVector<CBaseCombatWeapon*> pAllGrenades;
...
private:
	...
	void				CheckCamAttachment(void);

	void				SlowMoHandle();
	void				SelectionSlowMoHandle();

	float				m_flNextSlowMoTick;
	int					m_iSlowMoPercentage;
	int					ChangedBulletsStatment;
public:
	bool				HasGrenade(CBaseCombatWeapon *pGren);
	bool				GetAndRemoveGrenade(CBaseCombatWeapon *pGren);
	CBaseCombatWeapon*  GetGrenade(CBaseCombatWeapon *pGren);
	int					GetGrenadesCount();

	bool				m_bSlowMoIsEnabledTemp;
	CNetworkVar(bool, m_bSlowMoIsEnabled);
	CNetworkVar(bool, m_bNightvisionEnabled);

	void				EnableSlowMo(bool forced = false);
	void				IncreaseSlowMoPercentage();
	void				DecreaseSlowMoPercentage();
	void				DisableSlowMo(bool forced = false);
	void				SetSlowMoIsEnabled(bool ibVal) { m_bSlowMoIsEnabledTemp = m_bSlowMoIsEnabled = ibVal; };
	bool				GetSlowMoIsEnabled() { return m_bSlowMoIsEnabled; }
	bool				IsInSelectionSlowMo();

private:
	void				AddGrenade(CBaseCombatWeapon *pGren);
	int					NextGrenadeSlot(bool removeFromSlot = false);
	void				SelectGrenadeSlot(CUtlVector<CBaseCombatWeapon*> *pGrenadesArray, int slot = 0);
	void				NextGrenadeInSlot(int iSlot = 0);
	void				SwitchNextGrenade(int iSlot = 0);

public:
	void				RemoveGrenadeFromSlot(CBaseCombatWeapon *pWeapon);
	void				AddGrenadeToSlot(CBaseCombatWeapon *pWeapon);
	void				RefreshGrenadeNameInSlot(CBaseCombatWeapon *pWeapon);
	void				RefreshGrenadeAmmoInSlot(CBaseCombatWeapon *pWeapon);
	void				UpdateGrenadeInfo(void);
	bool				clearDecals;
	...
	//bool m_bIsInSelectionMode;
	CNetworkVar(bool, m_bIsInSelectionMode);
private:
	bool m_bCleanUpWeapons;

	float m_flSelectionAlpha;

	void EnableCheats(bool &cheatsWasEnabled);
	void DisableCheats(bool &cheatsWasEnabled);
public:
	CNetworkVar(int, m_iBloodOverlayDetailFrame);
	CNetworkVar(float, m_flNextBloodDryDisappear);

	// FIXME: Make these protected or private!
	// This player's data that should only be replicated to 
	//  the player and not to other players.
	CNetworkVarEmbedded(CPlayerLocalData, m_Local);
	CNetworkVar(bool, AllowCamAttachment);
	CNetworkVar(bool, SpecialBodyDraw);
	bool		CheatsWasEnabled;
#if defined USES_ECON_ITEMS
	CNetworkVarEmbedded(CAttributeList, m_AttributeList);
#endif
	void SetupBloodOverlay(bool bVal);
	void SetupBloodOverlayFrame(int iFrame);
	void SetupBloodOverlayDelay(float flDelay);

	void SetupWeaponBloodOverlay(bool bVal, bool bAllWeapons = false);
	void SetupWeaponBloodOverlayFrame(int iFrame);

	CBaseCombatWeapon *GetWeaponByClassName(const char *sClassName);
...
protected:
	...
	void					SetPlayerInUseEnt(CBaseEntity *pUseEntity);
	virtual	void			Internal_HandleMapEvent(inputdata_t &inputdata){}

	void					DispatchBloodOverlay(const CTakeDamageInfo &info);
	...
	CNetworkVar(float, vec_x);		// Time our FOV change started
	CNetworkVar(float, vec_y);		// Time our FOV change started
	CNetworkVar(float, vec_z);		// Time our FOV change started
...
inline void CBasePlayer::SetUseEntity(CBaseEntity *pUseEntity)
{
	...
	this->SetPlayerInUseEnt(pUseEntity);
}
