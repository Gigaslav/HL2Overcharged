...
enum Class_T
{
	...
	CLASS_BULLSEYE,
	//CLASS_BULLSQUID,
	CLASS_BULLSQUID,	// BriJee: Overcharged Bullsquid is back
	...
	CLASS_HEADCRAB,
	CLASS_SNARK,
	CLASS_HOUNDEYE,		// BriJee: Overcharged houndeye is back
	CLASS_MANHACK,
	CLASS_METROPOLICE,		
	CLASS_MILITARY,		
	CLASS_SCANNER,		
	...
	CLASS_ALIENGRUNT,		// BriJee: Overcharged added
	CLASS_ALIENCONTROLLER,
	CLASS_BEE,
	CLASS_BEE_PLR,
	CLASS_HGRUNT,		
	CLASS_RACEX,		// BriJee OVR: Race X addition
	CLASS_COMBINEGUARD,
	CLASS_CREMATOR,
	CLASS_HYDRA,
	NUM_AI_CLASSES
...
public:
	...
	virtual void			DecalTrace( const trace_t *pTrace, char const *decalName);
...
public:
	...
	virtual void FireBulletsHitScan(const FireBulletsInfo_t &info);
	virtual void FireBulletsDynamic(const FireBulletsInfo_t &info);

	virtual void FireBulletsShotgun(const FireBulletsInfo_t &info);
	virtual void FireBulletsHitScanShotgun(const FireBulletsInfo_t &info);
	virtual void FireBulletsDynamicShotgun(const FireBulletsInfo_t &info);
	...
	void FireBulletsShotgun(int cShots, const Vector &vecSrc, const Vector &vecDirShooting,
		const Vector &vecSpread, float flDistance, int iAmmoType, int iTracerFreq = 4,
		int firingEntID = -1, int attachmentID = -1, int iDamage = 0,
		CBaseEntity *pAttacker = NULL, bool bFirstShotAccurate = false, bool bPrimaryAttack = true);
	void FireBulletsVehicle(int cShots, const Vector &vecSrc, const Vector &vecDirShooting,
		const Vector &vecSpread, float flDistance, int iAmmoType, int iTracerFreq = 4,
		int firingEntID = -1, int attachmentID = -1, int iDamage = 0,
		CBaseEntity *pAttacker = NULL, bool bFirstShotAccurate = false, bool bPrimaryAttack = true);
...
public:
	...
	CBaseEntity *CreateNoSpawnInstance(const char *szName, const Vector &vecOrigin, const QAngle &vecAngles);
...
// Bullet firing (legacy)...
//-----------------------------------------------------------------------------
inline void CBaseEntity::FireBulletsShotgun(int cShots, const Vector &vecSrc,
	const Vector &vecDirShooting, const Vector &vecSpread, float flDistance,
	int iAmmoType, int iTracerFreq, int firingEntID, int attachmentID,
	int iDamage, CBaseEntity *pAttacker, bool bFirstShotAccurate, bool bPrimaryAttack)
{
...
// Bullet firing (legacy)...
//-----------------------------------------------------------------------------
inline void CBaseEntity::FireBulletsVehicle(int cShots, const Vector &vecSrc,
	const Vector &vecDirShooting, const Vector &vecSpread, float flDistance,
	int iAmmoType, int iTracerFreq, int firingEntID, int attachmentID,
	int iDamage, CBaseEntity *pAttacker, bool bFirstShotAccurate, bool bPrimaryAttack)
{
