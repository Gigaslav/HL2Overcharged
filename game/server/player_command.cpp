...
void CPlayerMove::SetupMove( CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	...
	if (cvar->FindVar("oc_weapon_free_aim")->GetInt() == 1)
	{
		...
		move->m_vecViewAngles = pPlayer->EyeAngles();// ucmd->viewangles;
	}
	else if (cvar->FindVar("oc_weapon_free_aim")->GetInt() == 0)
	{
		move->m_vecViewAngles = ucmd->viewangles;
	}
...
void CPlayerMove::FinishMove( CBasePlayer *player, CUserCmd *ucmd, CMoveData *move )
{
	...
	player->SetBodyPitch(vecFinal.x);*/
	//int m_nBodyPitchPoseParam = player->LookupPoseParameter("body_pitch");
	//player->SetNonThresholdPoseParameter(m_nBodyPitchPoseParam, pitch);
