...
void CAI_BaseNPC::SetSchedule(CAI_Schedule *pNewSchedule, bool DontClearGoal) //( CAI_Schedule *pNewSchedule )
{
	...
	if (!DontClearGoal) // BriJee: Overcharged FIX from Human Error codebase
...
int CAI_BaseNPC::SelectIdleSchedule()
{
	...
	{
		//return SCHED_ALERT_FACE_BESTSOUND;//Повернуться лицом к источнику звука
		return SCHED_INVESTIGATE_SOUND;//Двигаться к источнику звука
	}
...
{
	/*if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && cvar->FindVar("oc_ragdoll_serverside_interrupt")->GetBool() == false)
	{
		//DevMsg("BecomeRagdoll AI_BaseNpc_schedule \n");
		CTakeDamageInfo info;
	...
	if (CanBecomeServersideRagdoll() && ((GetFlags() & FL_ONFIRE) || cvar->FindVar("oc_ragdoll_serverside")->GetInt()))
	{
		...
		if (BecomeRagdoll(dmgInfo, vec3_origin))
		{
			CleanupOnDeath();
			return SCHED_DIE_RAGDOLL;
		}
