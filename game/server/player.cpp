		...
		if (pWeapon)
		{
			...
			if (cvar->FindVar("oc_ironsight_type")->GetInt() == 0)
			{
				...
				if (pWeapon->IsIronSighted() &&
					...
					shotgunState))//Если не возле стены, не бежим, не перезаряжаемся и не одеваем глушитель, не осматриваем оружие и не перезаряжаем дробовик
				{
					if (!IsInAVehicle())//Если не в транспорте
