...
BEGIN_DATADESC( CNPCMaker )
	...
	DEFINE_KEYFIELD( m_CustomModels,		FIELD_STRING,	"models" ),
	DEFINE_KEYFIELD( m_CustomSkins,			FIELD_STRING,	"skins" ),
...
void CNPCMaker::MakeNPC( void )
{
	...
	if (Q_strlen(STRING(m_CustomModels)))
	{
		CUtlVector<char*, CUtlMemory<char*> > arr;
		Q_SplitString(STRING(m_CustomModels), "||", arr);

		int i = random->RandomInt(0, arr.Count() - 1);

		pent->PrecacheModel(arr.Element(i));
		pent->SetModel(arr.Element(i));
	}
	...
	if (Q_strlen(STRING(m_CustomSkins)))
	{
		CUtlVector<char*, CUtlMemory<char*> > arr;
		Q_SplitString(STRING(m_CustomSkins), "||", arr);

		int i = random->RandomInt(0, arr.Count() - 1);

		int iSkin = atoi(arr.Element(i));
