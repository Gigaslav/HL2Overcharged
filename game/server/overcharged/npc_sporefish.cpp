...
#include "npc_sporefish.h"

ConVar sk_sporefish_health ( "sk_sporefish_health", "0" );
ConVar sk_sporefish_shake ( "sk_sporefish_shake", "0" );
...
LINK_ENTITY_TO_CLASS(npc_sporefish, CNPC_SporeFish);

BEGIN_DATADESC(CNPC_SporeFish)
...
AI_BEGIN_CUSTOM_NPC(monster_ichthyosaur, CNPC_SporeFish)
...
//=========================================================
void CNPC_SporeFish::Precache()
{
	PrecacheModel("models/sporefish.mdl");
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SporeFish::Spawn(void)
{
	Precache( );

	SetModel( "models/sporefish.mdl");
	...
	m_iHealth			= sk_sporefish_health.GetFloat();
	...
	SetTouch(&CNPC_SporeFish::BiteTouch);
...
//=========================================================
//=========================================================
int CNPC_SporeFish::TranslateSchedule(int scheduleType)
{
	switch	( scheduleType )
	{
...
int CNPC_SporeFish::SelectSchedule()
{
	switch(m_NPCState)
	{
	case NPC_STATE_IDLE:
...
bool CNPC_SporeFish::OverrideMove(float flInterval)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_SporeFish::MoveExecute_Alive(float flInterval)
{
...
void CNPC_SporeFish::InputStartCombat(inputdata_t &input)
{
	m_bOnAttack = true;
}

void CNPC_SporeFish::InputEndCombat(inputdata_t &input)
{
	m_bOnAttack = false;
}
...
// schedule.
//=========================================================
void CNPC_SporeFish::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
...
void CNPC_SporeFish::RunTask(const Task_t *pTask)
{
...
//			pev->deadflag = DEAD_DEAD;
	...
	case TASK_ICHTHYOSAUR_FLOAT:
		...
		/*CTakeDamageInfo info(this, GetEnemy(), 10, GetDamageType());
		Event_Killed(info);*/
...
void CNPC_SporeFish::Event_Killed(const CTakeDamageInfo &info)
{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_SporeFish::MeleeAttack1Conditions(float flDot, float flDist)
{
...
//=========================================================
int CNPC_SporeFish::RangeAttack1Conditions(float flDot, float flDist)
{
...
void CNPC_SporeFish::BiteTouch(CBaseEntity *pOther)
{
...
void CNPC_SporeFish::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	CheckPlrTouch(pActivator);
}

void CNPC_SporeFish::CheckPlrTouch(CBaseEntity *pOther)
{
	if (pOther && pOther->IsPlayer())
	{
		...
		if (!pPlayer->HasNamedPlayerItem("weapon_spore_launcher"))
			pPlayer->GiveNamedItem("weapon_spore_launcher");
		else
		{
			CBaseCombatWeapon *pWeapon = pPlayer->GetWeaponByClassName("weapon_spore_launcher");
			if (pWeapon)
			{
				int iAmmoType = pWeapon->m_iPrimaryAmmoType;
				auto ammoText = pWeapon->GetWpnData().szAmmo1;

				if (!g_pGameRules->CanHaveAmmo(pPlayer, iAmmoType))
					return;
				else
					pPlayer->GiveAmmo(10, ammoText);
			}
		}
...
//=========================================================
void CNPC_SporeFish::HandleAnimEvent(animevent_t *pEvent)
{
	...
	case ICHTHYOSAUR_AE_SHAKE_LEFT:
		{
			...
			if (hEnemy != NULL && FVisible( hEnemy ))
			{
				...
				if (DotProduct( vecShootDir, vForward ) > 0.707)
				{
					...
					CTakeDamageInfo info( this, this, sk_sporefish_shake.GetInt(), DMG_SLASH );
...
// relationship table.
//=========================================================
Class_T CNPC_SporeFish::Classify(void)
{
	return	CLASS_RACEX;
}

void CNPC_SporeFish::NPCThink(void)
{
	// blink the eye
...
//-----------------------------------------------------------------------------
float CNPC_SporeFish::GetGroundSpeed(void)
{
	if ( GetIdealActivity() == ACT_GLIDE )
...
Vector CNPC_SporeFish::DoProbe(const Vector &Probe)
{
...
bool CNPC_SporeFish::ProbeZ(const Vector &position, const Vector &probe, float *pFraction)
{
...
//-----------------------------------------------------------------------------
bool CNPC_SporeFish::FVisible(CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker)
{
...
void CNPC_SporeFish::IdleSound(void)
{ 
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ichthyosaur.Idle" );
}

void CNPC_SporeFish::AlertSound(void)
{ 
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ichthyosaur.Alert" );
}

void CNPC_SporeFish::AttackSound(void)
{ 
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ichthyosaur.Attack" );
}

void CNPC_SporeFish::BiteSound(void)
{ 
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ichthyosaur.Bite" );
}

void CNPC_SporeFish::DeathSound(const CTakeDamageInfo &info)
{ 
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ichthyosaur.Die" );
}

void CNPC_SporeFish::PainSound(const CTakeDamageInfo &info)
{ 
	CPASAttenuationFilter filter( this );
...
//-----------------------------------------------------------------------------
void CNPC_SporeFish::GatherEnemyConditions(CBaseEntity *pEnemy)
{
	// Do the base class
