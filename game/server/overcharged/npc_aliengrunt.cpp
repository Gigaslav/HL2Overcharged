...
ConVar	aliengrunt_ragdoll_grab_delay("oc_aliengrunt_ragdoll_grab_delay", "4");
ConVar	aliengrunt_shoot_bees_delay("oc_aliengrunt_shoot_bees_delay", "1");
...
static ConVar aliengrunt_always_ragdoll_grab("oc_aliengrunt_always_ragdoll_grab","0");
#endif
...
ConVar	aliengrunt_charge_delay_max("oc_aliengrunt_charge_delay_max", "6");
ConVar	aliengrunt_charge_delay_min("oc_aliengrunt_charge_delay", "2");
...
BEGIN_DATADESC( CNPC_AlienGrunt )
	...
	DEFINE_FIELD(m_flDamageTime, FIELD_TIME),	// BJ: Armor hit react
...
void CNPC_AlienGrunt::GatherConditions( void )
{
	...
	{	
		...
		while ( pSquadmate && !bFound)
		{
			if ( pSquadmate != this )
			{
				if ( pSquadmate->Classify() == CLASS_ALIENGRUNT && 
