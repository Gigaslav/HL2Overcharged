...
ConVar phys_gunautoweld("phys_gunautoweld", "0" ); // With a nice and descriptive name, and set to OFF by default. 
ConVar phys_gunattract("phys_gunattract", "1" ); // Now we add this. 
...
public:
	...
	void Precache()
	{
		SetModelName( MAKE_STRING( "models/weapons/w_bugbait.mdl" ) ); // Pellet model
...
class CWeaponGravityGun : public CBaseCombatWeapon //класс самого физгана
{
	DECLARE_DATADESC();
	DECLARE_ACTTABLE();
...
private:
	...
	bool m_shouldPassPF; // This is our custom boolean, just at the bottom of the variables in the Private space. 
};
...
acttable_t CWeaponGravityGun::m_acttable[] =						// Light Kill : Physgun act's over here
{
	...
	{ ACT_IDLE_RELAXED,				ACT_IDLE_SHOTGUN_RELAXED,		false },	// L1ght 15 : Anims update
...
void CWeaponGravityGun::EffectUpdate( void )
{
	...
	if ( tr.m_pEnt )		// L1ght 15 : Don't do anything with NPC, no more crashes
	{
	if (tr.m_pEnt->IsNPC())	
		return;
	}
...
{
	// We added this:
	if ( phys_gunattract.GetBool() == false )
    radius = 4.0f;
	// End of added.
	int nearPellet = -1;
	int objectPellet = -1;
...
void CWeaponGravityGun::AttachObject( CBaseEntity *pObject, const Vector& start, const Vector &end, float distance )
{
	...
	if ( pPhysics && pObject->GetMoveType() == MOVETYPE_VPHYSICS || pObject->GetMoveType() == MOVETYPE_STEP )
	// Notice that we added " || pObject->GetMoveType() == MOVETYPE_STEP" to the IF statement.
{
    //============== Added code ==============//
    if ( pObject->IsNPC() && !pObject->IsEFlagSet( EFL_NO_MEGAPHYSCANNON_RAGDOLL ) )
    {
      ...
      CBaseEntity *pRagdoll = CreateServerRagdoll( pObject->MyNPCPointer(), 0, info, 
		  COLLISION_GROUP_INTERACTIVE_DEBRIS, true, pObject->MyNPCPointer()->BloodColor());
...
void CWeaponGravityGun::SecondaryAttack( void )
{
	...
	if (/*(pOwner->m_nButtons & IN_RUN) || */(pOwner->m_nButtons & IN_SPEED) && ((pOwner->m_nButtons & IN_FORWARD) || (pOwner->m_nButtons & IN_BACK) || (pOwner->m_nButtons & IN_MOVELEFT) || (pOwner->m_nButtons & IN_MOVERIGHT)))
	{
		return;
	}
...
// Obviously, in an absolute showoff of their unlimited knowledge of the English Language, they thought that it
// would be awesome to say "glue" and not "Weld" -even with the rain of sparks that you get when two pellets
// are touching. Good going.
 
// Anyways, i'll explain this thing: This is the WeaponIdle function that is called when our weapon is... Idle! What
// this does is check if when you stop pressing the Attack button there are two pellets touching, and if that is the
// case, weld them.
 
      if ( pObject && m_glueTouching && phys_gunautoweld.GetBool() == false ) // What we are going to do
// is adding a third check to the IF statement, so we only weld in the WeaponIdle function when AutoWeld is OFF.
      {
...
void CWeaponGravityGun::ItemPostFrame( void )
{
	...
	//============== Added code ==============//
	CBaseEntity *pObject = m_hObject;
	 if ( m_shouldPassPF == true && ( pOwner->m_nButtons & IN_ATTACK ) )
	{
		WeaponIdle( );
		return;
	}
	 else if ( pObject && m_glueTouching && phys_gunautoweld.GetBool() == true && m_shouldPassPF == false )
	{
		CGravityPellet *pPellet = m_activePellets[m_pelletAttract].pellet;
		if ( pPellet->MakeConstraint( pObject ) )
    {
	...
	else if ( pOwner->m_afButtonPressed & IN_USE )  // Hence we can add this.
    {
		RemoveSinglePellet();     // And call our function in here.
    }
	// -----------------------
	//  No buttons down
	//  Нет нажатых кнопок
...
void CWeaponGravityGun::RemoveSinglePellet( void )
{
  CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
  if ( pOwner->GetAmmoCount(m_iSecondaryAmmoType) == MAX_PELLETS )
    return;
  ...
  for ( int i = 0; i < m_pelletCount; i++ )
  {
    if ( pHit == m_activePellets[i].pellet )
    {
      CGravityPellet *pPellet = dynamic_cast<CGravityPellet*>(pHit);    
      Vector forward;
      AngleVectors( pPellet->GetAbsAngles(), &forward );
      g_pEffects->Dust( pPellet->GetAbsOrigin(), forward, 32, 30 );
      ...
      pelletlist_t tmpPellet = m_activePellets[m_pelletCount-1];
      UTIL_Remove( pPellet );
      m_activePellets[i] = tmpPellet;
 
      m_pelletCount -= 1;
 
      int newAmmo = pOwner->GetAmmoCount( m_iSecondaryAmmoType ) + 1;
      pOwner->SetAmmoCount( newAmmo, m_iSecondaryAmmoType );
      continue;
    }
  }
}