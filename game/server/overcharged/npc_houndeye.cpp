...
void CNPC_Houndeye::Spawn()
{
	...
	//======= L1ght 15 : 2017 random Wings feature addition
	if (random->RandomInt(0,3) == 3)
	{
		SetBodygroup(1,1);		// Model 2, SubModel 2
	}
	//=====end
		...
		if ( pEntity->m_takedamage != DAMAGE_NO || pPhysicsObject)
		{
			...
			if ( !FVisible( pEntity ) )
			{
				if ( pEntity->IsPlayer() )
				{
					...
					&& !FClassnameIs(pEntity, "npc_sporegenerator") && !FClassnameIs(pEntity, "npc_xentree"))	// BJ: OK this works with spores, trees.
				{
			...
			// BJ: Extra attention to new fauna, they should react to shockwaves
			// THIS NOT DAMAGING! Use for xenlights later.
			/*
			if (FVisible(pEntity))	
			{
				if (!FClassnameIs(pEntity, "npc_sporegenerator"))
				{
					DevMsg("Houndeye found sporegenerator entity \n");
				flDamageAdjuster *= 2;
				}

				//if (!FClassnameIs(pEntity, "npc_xenlight"))
					//pEntity->Touch();
			}
			*/
			...
			if (pEntity->m_takedamage != DAMAGE_NO)
			{
				...
				//RadiusDamage(CTakeDamageInfo(this, this, 125 * 10, DMG_BLAST), GetAbsOrigin(), 256, CLASS_NONE, NULL);	// BJ: testing
			}
			// ------------------------------
			//  Apply physics foces
			// ------------------------------
