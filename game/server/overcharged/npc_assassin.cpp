...
ConVar oc_npc_assassin_model( "oc_npc_assassin_model", "models/assassin_female.mdl", FCVAR_ARCHIVE, "Set the main model for assassin." );
ConVar oc_npc_assassin_model2("oc_npc_assassin_model2", "models/assassin_alien.mdl", FCVAR_ARCHIVE, "Set the alt model for assassin.");
ConVar sk_assassin_health( "sk_assassin_health", "0", FCVAR_REPLICATED, "Assassin Health." );
...
BEGIN_DATADESC( CNPC_Assassin )
	...
	DEFINE_FIELD( m_bHaveWeapons,		FIELD_BOOLEAN ),	// L1ght 15 : Have weapons on spawn or not
	DEFINE_FIELD( m_bPistolFiring,		FIELD_BOOLEAN ),	// L1ght 15 : Propper firing
	DEFINE_FIELD( m_iPistolShots,		FIELD_INTEGER ),	// L1ght 15: Shots count. if > 5 start jump
	DEFINE_KEYFIELD( m_iSkin, FIELD_INTEGER, "skin" ),		// L1ght 15 : Alt skins
END_DATADESC()
...
void CNPC_Assassin::Precache( void )
{
	...
	CBaseEntity::PrecacheModel (oc_npc_assassin_model.GetString());
	CBaseEntity::PrecacheModel (oc_npc_assassin_model2.GetString());
	...
	PrecacheScriptSound( "NPC_Alyx.FootstepLeft" );
	PrecacheParticleSystem("weapon_muzzle_flash_pistol_npc");
...
void CNPC_Assassin::Spawn( void )
{
	...
	if ( m_iSkin == 1 || !m_iSkin )	
	{
		SetModel( oc_npc_assassin_model.GetString() );
	}
	else if ( m_iSkin == 2 )
	{
		SetModel( oc_npc_assassin_model2.GetString() );
	}
	...
	//KeyValue( "additionalequipment", "weapon_pistol" );
	//GiveNamedItem( "weapon_pistol" ); // L1ght 15 : Give em someone a weapon!
	...
	m_bPistolFiring = false;	// L1ght 15 : Propper firing
	m_iPistolShots = 0;

	if (GetActiveWeapon())		// L1ght 15 : Have weapons on spawn or not
	{
		//SetBodygroup( 1, 0 );	// turn guns off
		m_bHaveWeapons = true;
	}
	else
	{
		SetBodygroup( 1, 1 );	// turn guns on
...
void CNPC_Assassin::FirePistol( int hand )
{
	...
	if (cvar->FindVar("oc_particle_muzzleflashes")->GetInt())
		DispatchParticleEffect("weapon_muzzle_flash_pistol_npc", muzzlePos, muzzleAngle);
	else
	{
		CEffectData data;
		data.m_nEntIndex = entindex();
		data.m_nAttachmentIndex = LookupAttachment(handName);
		data.m_flScale = 1.0f;
		data.m_fFlags = 2;
		DispatchEffect("MuzzleFlash", data);
	}
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Weapon_Pistol.NPC_Single" ); //NPC_Assassin.ShootPistol
}
...
void CNPC_Assassin::HandleAnimEvent( animevent_t *pEvent )
{
	...
	*/
	...
	switch( pEvent->event )					// L1ght 15 : Propper firing
	{
	...
	case ASSASSIN_AE_KICK_HIT:
	{
		...
		{
			EmitSound( "Zombie.AttackMiss" );	//fix
	...
	case NPC_STATE_COMBAT:
		{
			...
			if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
			{
				...
				if (m_bPistolFiring)		// L1ght 15 : Propper firing
				{
					FirePistol(0);
					m_bPistolFiring = false;
				}
				else
				{
					FirePistol(1);
					m_bPistolFiring = true;
				}

				m_iPistolShots = m_iPistolShots + 1;

				if (m_iPistolShots > 4 )
				{
					m_iPistolShots = 0;

					// L1ght 15: Don't stay. Move around.
					if ( random->RandomInt( 1, 4 ) == 2 )
						return SCHED_ASSASSIN_LUNGE;	//rand2
					else
						return SCHED_ASSASSIN_EVADE;
				}
					return SCHED_RANGE_ATTACK1;
			...
			{
				if (GetActiveWeapon())						// L1ght 15: Drop weapon without ammo
				{
					if (GetActiveWeapon()->m_iClip1 == 0)
					{
					Weapon_Drop( GetActiveWeapon() );	// drop a weapon
					SetBodygroup( 1, 1 );	// turn guns on
					return ACT_JUMP;	// play random animation?
					}
				}
...
int CNPC_Assassin::TranslateSchedule ( int scheduleType )	// L1ght 15 : Must search near enemy if exist
{
	switch	( scheduleType )
	{
...
void CNPC_Assassin::PrescheduleThink( void )
{
	if ( GetActivity() == ACT_RUN || GetActivity() == ACT_WALK)
	{
		...
		if (iStep)
		{
			EmitSound( filter, entindex(), "NPC_Alyx.FootstepLeft" );	//"NPC_Assassin.Footstep" ); //fix
		}
	}
}
...
int CNPC_Assassin::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
	...
	case TASK_ASSASSIN_GET_PATH_TO_VANTAGE_POINT:
		{
			...
			//CAI_Hint *pHint = CAI_HintManager::FindHint( this, GetEnemy()->GetAbsOrigin(), &hint );		//fix
...
void CNPC_Assassin::SetEyeState( eyeState_t state )
{
	...
	case ASSASSIN_EYE_DEAD: //Fade out slowly
		m_pEyeSprite->SetColor( 255, 0, 0 );
		m_pEyeSprite->SetScale( 0.0f, 0.0f );
		m_pEyeSprite->SetBrightness( 0, 0.0f );

		m_pEyeTrail->SetColor( 255, 0, 0 );
		m_pEyeTrail->SetScale( 0.0f );
...
void CNPC_Assassin::BuildScheduleTestBits( void )
{
	...
	/*if ( IsCurSchedule( SCHED_RANGE_ATTACK1 ) )						// L1ght 15 : bug bug looping fire animation. disabled.
	{
...
void CNPC_Assassin::Event_Killed( const CTakeDamageInfo &info )
{
	...
	// BriJee : Temp disabled. Need base class gibs!
	/*if ((info.GetDamageType() & (DMG_BLAST)) && (cvar->FindVar("aa_more_gibs")->GetInt() == 1))
	{
				//AddFlag(FL_ONFIRE);
	...
	if (!m_bHaveWeapons)
	{
	Vector vecStartL, vecStartR;
	GetAttachment( "lefthand", vecStartL );
	CBaseEntity *pLPistol = DropItem( "weapon_pistol", vecStartL, RandomAngle(0,360) );

	GetAttachment( "righthand", vecStartR );
	CBaseEntity *pRPistol = DropItem( "weapon_pistol", vecStartR, RandomAngle(0,360) );

	// In the Citadel we need to dissolve this
	if ( PlayerHasMegaPhysCannon() )
	{
		CBaseCombatWeapon *pWeaponLPistol = static_cast<CBaseCombatWeapon *>(pLPistol);
		pWeaponLPistol->Dissolve( NULL, gpGlobals->curtime, false, ENTITY_DISSOLVE_NORMAL );

		CBaseCombatWeapon *pWeaponRPistol = static_cast<CBaseCombatWeapon *>(pRPistol);
		pWeaponRPistol->Dissolve( NULL, gpGlobals->curtime, false, ENTITY_DISSOLVE_NORMAL );
	}
	}
