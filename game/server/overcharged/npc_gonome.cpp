...
//
// Purpose:		Gonome. Opposing Force big monster recreation.
//				
//				
// $NoKeywords: $
//=============================================================================//
...
#include "NPCEvent.h"
#include "animation.h"
#include "overcharged/npc_gonome.h"
#include "gib.h"
#include "soundent.h"
...
#include "particle_parse.h"	// particles
...
#define		GONOME_AE_FOOTSTEP	( 7 )

LINK_ENTITY_TO_CLASS( npc_gonome, CNPC_Gonome );	// BriJee: Gonome ent definition
...
LINK_ENTITY_TO_CLASS( gonome_spit, CGonomeSpit );
...
void CGonomeSpit::Precache( void )
{
	...
	//PrecacheModel("models/gibs/hgibs_scapula.mdl");		// BriJee: Projectile Gib model
	//PrecacheModel("models/weapons/hunter_flechette.mdl");

	PrecacheScriptSound( "Npc_GonomeSpit.SpitTouch1" );
	PrecacheScriptSound( "Npc_GonomeSpit.SpitHit1" );
	PrecacheScriptSound( "Npc_GonomeSpit.SpitHit2" );

	PrecacheScriptSound( "Zombie.AttackHit" );
	PrecacheScriptSound( "Zombie.AttackMiss" );

	PrecacheParticleSystem("blood_impact_red_01_chunk");	//BriJee: Main projectile particle
...
void CGonomeSpit:: Spawn( void )
{
	...
	SetClassname( "gonome_spit" );
	
	SetSolid( SOLID_BBOX );

	//m_nRenderMode = kRenderTransAlpha;
	//SetRenderColorA( 255 );
	//SetModel( "models/gibs/hgibs_scapula.mdl" );		// BriJee: Projectile Gib model
	//SetModel("models/weapons/hunter_flechette.mdl");
	SetModel("");

	SetSprite( CSprite::SpriteCreate( /*"sprites/gonomespit.vmt"*/ NULL, GetAbsOrigin(), true ) );	// BriJee: NULL sprite name, we will attach particle on it
	...
	if (pParticle != NULL)
	{
		...
		//pParticle->KeyValue("effect_name", "blood_impact_red_01_chunk");
		pParticle->KeyValue("effect_name", "blood_spit_trail");						// BJ: wtf? blood_trail is unknown, found blood_spit_trail in particles
...
{
	CGonomeSpit *pSpit = CREATE_ENTITY( CGonomeSpit, "gonome_spit" );
	pSpit->Spawn();
...
void CGonomeSpit::Touch ( CBaseEntity *pOther )
{
	...
	EmitSound( "Npc_GonomeSpit.SpitTouch1" );

	switch ( random->RandomInt( 0, 1 ) )
	{
	case 0:
		EmitSound( "Npc_GonomeSpit.SpitHit1" );
		break;
	case 1:
		EmitSound( "Npc_GonomeSpit.SpitHit2" );
		break;
	}
	...
	else
	{
		...
		CalculateBulletDamageForce( &info, GetAmmoDef()->Index("Pistol"), GetAbsVelocity(), GetAbsOrigin() );	// BriJee: Ammotype fixup for HL2
...
void CNPC_Gonome::Spawn()
{
	...
	SetModel("models/zombie/gonome.mdl");
	SetBodygroup(ZOMBIE_BODYGROUP_HEADCRAB, 1);	// BriJee: Bodygroup on spawn.

	SetHullType(HULL_MEDIUM_TALL);//HULL_HUMAN);
	SetHullSizeNormal();
...
void CNPC_Gonome::Precache()
{
	...
	PrecacheModel("models/zombie/gonome.mdl");
	
	//PrecacheModel("sprites/gonomespit.vmt");

	PrecacheScriptSound( "Npc_Gonome.Idle" );
	PrecacheScriptSound( "Npc_Gonome.Pain" );
	PrecacheScriptSound( "Npc_Gonome.Alert" );
	PrecacheScriptSound( "Npc_Gonome.Die" );
	PrecacheScriptSound( "Npc_Gonome.Attack" );
	PrecacheScriptSound( "Npc_Gonome.Bite" );
	PrecacheScriptSound( "Npc_Gonome.Growl" );

	PrecacheScriptSound("Npc_Gonome.Step1");	// new
	PrecacheScriptSound("Npc_Gonome.Step2");
}
...
// BJ: Gonome HEADSHOT damage code
void CNPC_Gonome::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	CTakeDamageInfo ainfo = info;
	float flDamage = ainfo.GetDamage();

	// BJ Hitgroup 1 - head
	if (ptr->hitgroup == 1 && (ainfo.GetDamageType() & (DMG_BULLET | DMG_SLASH | DMG_CLUB)))	// only bullet, melee damage filter
	{
		flDamage += 50;			// BJ: x2 damage in head
		if (flDamage <= 0)
			flDamage = 0.1;
...
void CNPC_Gonome::IdleSound( void )
{
	...
	EmitSound( filter, entindex(), "Npc_Gonome.Idle" );	
}
...
void CNPC_Gonome::PainSound( const CTakeDamageInfo &info )
{
	...
	EmitSound( filter, entindex(), "Npc_Gonome.Pain" );	
}
...
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Npc_Gonome.Alert" );	
}
...
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Npc_Gonome.Die" );	
}
...
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Npc_Gonome.Attack" );	
}
...
void CNPC_Gonome::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		case GONOME_AE_FOOTSTEP:
		{
			...
			if (i)
				EmitSound(filter, entindex(), "Npc_Gonome.Step1");
			else
				EmitSound(filter, entindex(), "Npc_Gonome.Step2");
		}
		break;
		...
		// SOUND HERE!
			...
			if ( pHurt )
			{
				...
				if (pHurt->GetFlags() & (FL_NPC | FL_CLIENT))
					pHurt->ViewPunch(QAngle(random->RandomInt(-20, 20), random->RandomInt(-20, 20), random->RandomInt(-20, 20)));
		...
		{
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), "Npc_Gonome.Bite" );	
			...
			if ( pHurt ) 
			{
				...
				pHurt->SetAbsVelocity( pHurt->GetAbsVelocity() + random->RandomInt(1,-1) * (right * 200) );
				//pHurt->SetAbsVelocity( pHurt->GetAbsVelocity() + (up * 100) );
			}
...
void CNPC_Gonome::RemoveIgnoredConditions( void )
{
	...
	if ( GetEnemy() != NULL )
	{
		...
		if ( FClassnameIs( GetEnemy(), "npc_citizen" ) )	// BriJee: Citizen is priority target!
			 ClearCondition( COND_SMELL );
	}
...
int CNPC_Gonome::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	/*if ( info.GetDamageType() == DMG_BULLET ) //OverMike отбрасывало нпс от выстрела
	{
...
void CNPC_Gonome::StartTask ( const Task_t *pTask )
{
	...
	case TASK_MELEE_ATTACK2:
		{
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), "Npc_Gonome.Growl" );		
...
void CNPC_Gonome::Event_Killed(const CTakeDamageInfo &info)
{
	if (!cvar->FindVar("oc_npc_zombies_dont_less_headcrab")->GetBool())
	{
		if ((info.GetDamageType() & (DMG_BLAST)) || (info.GetDamage() >= (m_iMaxHealth * 0.75f)))	// BriJee: NEW headcrab release solution
		{
			Vector vCrabPos;
			GetAttachment("headcrab", vCrabPos);

			CBaseEntity *pHCrab = NULL;
			pHCrab = CBaseEntity::Create("npc_headcrab", /*GetLocalOrigin()*/ vCrabPos, QAngle(0, 0, 0), this);

			if (pHCrab)
			{
				pHCrab->SetGroundEntity(NULL);
				pHCrab->AddSpawnFlags(SF_NPC_FALL_TO_GROUND /*SF_HEADCRAB_START_HANGING*/);
			}

			SetBodygroup(ZOMBIE_BODYGROUP_HEADCRAB, 0);			// BriJee: Bodygroup 0 (remove crab)
		}
	}
