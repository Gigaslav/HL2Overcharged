...
//	DECLARE_DATADESC();
	...
	Class_T  Classify(void) { return CLASS_NONE; }	// BriJee: Temp class name
...
{
	PrecacheModel("models/cockroach.mdl");

	PrecacheScriptSound( "CockRoach.Walk" );
	PrecacheScriptSound( "CockRoach.Die" );
	PrecacheScriptSound( "CockRoach.Smash" );
}
...
void CNPC_Roach::PickNewDest ( int iCondition )
{
	...
	if ( random->RandomInt( 0, 9 ) == 1 )
	{
		...
		EmitSound( filter, entindex(), "CockRoach.Walk" );
	}
}
...
void CNPC_Roach::Event_Killed( const CTakeDamageInfo &info )
{
	...
	{
		EmitSound( filter, entindex(), "CockRoach.Die" );
	}
	else
	{
		EmitSound( filter, entindex(), "CockRoach.Smash" );
	}
