...
//
// Purpose: Gargantua - Big monster from Xen
//			
//
// $Workfile:     $
// $Date:         $
...
#include	"AI_Route.h"
#include	"AI_Memory.h"
#include	"overcharged/npc_gargantua.h"
#include	"soundent.h"
#include	"game.h"
#include	"NPCEvent.h"
...
#include	"particle_parse.h"	// BJ: Fire / Stomp particles
...
#define GARG_DAMAGE					( DMG_ENERGYBEAM | DMG_CRUSH | /*DMG_MISSILEDEFENSE |*/ DMG_BLAST )		// BriJee OVR: Remove unused flag
#define GARG_EYE_SPRITE_NAME		"sprites/redglow3.vmt" //"sprites/gargeye1.vmt"
//#define GARG_BEAM_SPRITE_NAME		"sprites/laserbeam.vmt" //"sprites/xbeam3.vmt"		// BriJee OVR: No need old beam sprites, use particles now :)
//#define GARG_BEAM_SPRITE2			"sprites/laserbeam.vmt" //"sprites/xbeam3.vmt"
//#define GARG_STOMP_SPRITE_NAME		"sprites/redglow2.vmt" //"sprites/gargeye1.vmt"
#define GARG_FLAME_LENGTH			330
#define GARG_GIB_MODEL				"models/gibs/helicopter_brokenpiece_02.mdl"		//"models/metalplategibs.mdl"
#define GARG_GIB_MODEL2				"models/gibs/helicopter_brokenpiece_03.mdl"		// BriJee OVR: More gibs added
#define GARG_GIB_MODEL3				"models/gibs/Strider_gib1.mdl"
...
private:
	...
	//float m_flDelayDispatch;
	bool		bSpawnParticleOnce;		// BJ: Stomp particle
...
void CStomp::Precache()
{
	...
	PrecacheScriptSound("gargantua.Stomp");
...
void CStomp::Spawn(void)
{
	...
	EmitSound(filter, entindex(), "gargantua.Stomp");

	//m_flDelayDispatch = 0.0f;
}
...
void CStomp::Think(void)
{
	...
	if (tr.m_pEnt)
	{
		...
		CTakeDamageInfo info(this, this, 50, DMG_SONIC /*| DMG_DISSOLVE*/);
	...
	while (gpGlobals->curtime - m_flDmgTime > STOMP_INTERVAL)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			CSprite *pSprite = CSprite::SpriteCreate(/*GARG_STOMP_SPRITE_NAME*/ NULL, GetAbsOrigin(), TRUE);	// BriJee OVR: NULL Sprite should make spark effects
			if (pSprite)
			{
			...
			/*if (gpGlobals->curtime > m_flDelayDispatch)		// BJ: Old method pre frame spawn-remove
			{
				DispatchParticleEffect("gargantua_stomp1", PATTACH_ABSORIGIN_FOLLOW, pSprite);
				m_flDelayDispatch = gpGlobals->curtime + 0.05f;
			}*/


			if (!bSpawnParticleOnce)
			{
				bSpawnParticleOnce = true;

				// BJ: Valve method attach effect with info_particle_system once
				CParticleSystem *pParticleStomp = (CParticleSystem *)CreateEntityByName("info_particle_system");
				if (pParticleStomp != NULL)
				{
					pParticleStomp->KeyValue("start_active", "1");
					pParticleStomp->KeyValue("effect_name", "gargantua_stomp1");
					pParticleStomp->SetParent(this);
					//pParticle->SetParentAttachment("SetParentAttachment", "attach_glow2", true);
					pParticleStomp->SetLocalOrigin(Vector(0, 0, -39));	// BJ: PREFECT VALUES !!!
					//pParticleStomp->SetLocalOrigin(vec3_origin);
					DispatchSpawn(pParticleStomp);
					if (gpGlobals->curtime > 1.5f)
						pParticleStomp->Activate();

					DevMsg("Garg stomp particle created \n");
				}// end
		...
		if (m_flScale <= 0)
		{
			...
			UTIL_Remove(this);
			CPASAttenuationFilter filter(this);
			StopSound(entindex(), CHAN_STATIC, "gargantua.Stomp");
		}
	}
}
...
void CNPC_Gargantua::Precache()
{
	...
	gGargGibModel2 = PrecacheModel(GARG_GIB_MODEL2);
	gGargGibModel3 = PrecacheModel(GARG_GIB_MODEL3);

	PrecacheParticleSystem("flamethrower");
	PrecacheParticleSystem("Explosion_2");

	PrecacheScriptSound("gargantua.AttackHit");
	PrecacheScriptSound("gargantua.AttackMiss");
	PrecacheScriptSound("gargantua.Footstep");
	PrecacheScriptSound("gargantua.Breath");
	PrecacheScriptSound("gargantua.Attack");
	PrecacheScriptSound("gargantua.Pain");
	PrecacheScriptSound("gargantua.BeamAttackOn");
	PrecacheScriptSound("gargantua.BeamAttackRun");
	PrecacheScriptSound("gargantua.BeamAttackOff");
	PrecacheScriptSound("gargantua.StompSound");
}


Class_T  CNPC_Gargantua::Classify(void)
{
	return CLASS_ALIENGRUNT;	//CLASS_ZOMBIE; //CLASS_ALIEN_MONSTER;	// BriJee OVR: Garg don't hurt zombies so use this class
}
...
void CNPC_Gargantua::HandleAnimEvent(animevent_t *pEvent)
{
	...
	case GARG_AE_SLASH_LEFT:
	{
		...
		if (pHurt)
		{
			...
			EmitSound(filter, entindex(), "gargantua.AttackHit");
		}
		else // Play a random attack miss sound
		{
			EmitSound(filter, entindex(), "gargantua.AttackMiss");
		}
	}
	break;
	...
	case GARG_AE_LEFT_FOOT:
		...
		EmitSound(filter, entindex(), "gargantua.Footstep");
		break;
	...
	case GARG_AE_BREATHE:
		EmitSound(filter, entindex(), "gargantua.Breath");
		break;
...
int CNPC_Gargantua::TranslateSchedule(int scheduleType)
{
	...
	if (FlameIsOn())
		FlameDestroy();
	//DevMsg("scheduleType %i", scheduleType);

	if (GetHealth() <= 0)
		return SCHED_GARG_DIE;
...
void CNPC_Gargantua::StartTask(const Task_t *pTask)
{
	...
	case TASK_SOUND_ATTACK:
		...
		{
			CPASAttenuationFilter filter(this);
			EmitSound(filter, entindex(), "gargantua.Attack");
		}
...
void CNPC_Gargantua::RunTask(const Task_t *pTask)
{
	...
	case TASK_DIE:
		...
		if (gpGlobals->curtime > m_flWaitFinished)
		{
			...
			DispatchParticleEffect("Explosion_2", GetAbsOrigin(), QAngle(-90, 0, 0));	// BriJee OVR: Big explosion effect

			int i;

			int parts = modelinfo->GetModelFrameCount(modelinfo->GetModel(gGargGibModel));
			int parts2 = modelinfo->GetModelFrameCount(modelinfo->GetModel(gGargGibModel2));
			int parts3 = modelinfo->GetModelFrameCount(modelinfo->GetModel(gGargGibModel3));
			...
			for (i = 0; i < 10; i++)
			{
				...
				if (random->RandomInt(1, 3) == 1)
				{
					pGib->Spawn(GARG_GIB_MODEL2);
				}
				else
				{
					pGib->Spawn(GARG_GIB_MODEL3);
				}
				int bodyPart = 0;
				...
				if (parts2 > 1)
					bodyPart = random->RandomInt(0, parts2 - 1);

				if (parts3 > 1)
					bodyPart = random->RandomInt(0, parts3 - 1);
...
void CNPC_Gargantua::FlameCreate(void)
{
	...
	for (i = 0; i < 4; i++)
	{
		/*if (i < 2)
			...
			m_pFlame[i] = CBeam::BeamCreate(GARG_BEAM_SPRITE2, 14.0);*/		// BriJee OVR: No more sprites!

		if (i < 2)
			m_pFlame[i] = CBeam::BeamCreate(NULL, 24.0);
		else
			m_pFlame[i] = CBeam::BeamCreate(NULL, 14.0);
	...
	EmitSound(filter4, entindex(), "gargantua.BeamAttackOn");
	EmitSound(filter4, entindex(), "gargantua.BeamAttackRun");
}
...
void CNPC_Gargantua::FlameUpdate(void)
{
	...
	if (!DispatchParticleOnce)		// BriJee OVR: Dispatch once.
	{
		DispatchParticleEffect("flamethrower", PATTACH_POINT_FOLLOW, this, "1", false);	// BriJee OVR: Flame particles
		DispatchParticleEffect("flamethrower", PATTACH_POINT_FOLLOW, this, "2", false);
		DispatchParticleOnce = true;
	}
	...
	for (i = 0; i < 2; i++)
	{
		if (m_pFlame[i])
		{
			...
			{
				//g_pEffects->Sparks(trace.endpos, 1, 1, &trace.plane.normal);		// BJ: Cleanup Sparks + Decals on flame / out of sync with particle
				streaks = TRUE;
			...
			FlameDamage(vecStart, trace.endpos, this, this, sk_gargantua_dmg_fire.GetFloat(), CLASS_ZOMBIE, DMG_BURN);
...
void CNPC_Gargantua::FlameDamage(Vector vecStart, Vector vecEnd, CBaseEntity *pevInflictor, CBaseEntity *pevAttacker, float flDamage, int iClassIgnore, int bitsDamageType)
{
	...
	while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(), searchRadius)) != NULL)
	{
		...
		if (pEntity->m_takedamage != DAMAGE_NO)
		{
			...
			//DevMsg("Gard check for enemies 1 \n");
			//if (DispatchParticleOnce)		// BriJee OVR: Ignite enemy NPCs in radius.
			//{

				//CAI_BaseNPC		*pNpcGet;
				//pNpcGet = (CAI_BaseNPC*)GetEntityName(pEntity);

				//DevMsg("Gard check for enemies 2 \n");
				//CBaseCombatCharacter *pBCC;
				//pBCC = pEntity->MyCombatCharacterPointer();

			if (random->RandomInt(0, 3) == 3)	// BJ: 1 to 3 randomize ignite a bit
			{


				if (FClassnameIs(pEntity, "npc_advisor_fighter") || FClassnameIs(pEntity, "npc_citizen") || FClassnameIs(pEntity, "npc_combine_s") || FClassnameIs(pEntity, "npc_crow") ||
					FClassnameIs(pEntity, "npc_hunter") || FClassnameIs(pEntity, "npc_metropolice") || FClassnameIs(pEntity, "npc_stalker") || FClassnameIs(pEntity, "npc_pigeon") ||
					FClassnameIs(pEntity, "npc_hgrunt") || FClassnameIs(pEntity, "npc_pitdrone") || FClassnameIs(pEntity, "npc_shocktrooper") || FClassnameIs(pEntity, "npc_seagull") ||
					FClassnameIs(pEntity, "npc_voltigore") || FClassnameIs(pEntity, "npc_voltigore_baby") || FClassnameIs(pEntity, "npc_cremator") || FClassnameIs(pEntity, "npc_assassin") ||
					FClassnameIs(pEntity, "npc_zombie") || FClassnameIs(pEntity, "npc_zombie_torso") || FClassnameIs(pEntity, "npc_zombine") || FClassnameIs(pEntity, "npc_fastzombie") ||
					FClassnameIs(pEntity, "npc_fastzombie_torso") || FClassnameIs(pEntity, "npc_poisonzombie"))
				{

					CAI_BaseNPC *pNpc = static_cast<CAI_BaseNPC *>(pEntity);
					pNpc->IgniteLifetime(20);									// BJ: DONT USE Ignite, use IgniteLifetime. It ignore limitations for (combine_s, metropolice) only

					//if (pBCC && !pBCC->IsOnFire())
					//{

					//pBCC->Ignite(20);
					DevMsg("Garg ignite %s \n", pNpc->GetClassname());

					//pEntity->GetEnemy->ignite(20);
					//pEntity->Ignite(30);
					//tr.m_pEnt->Ignite();
					//pEntity->GetBaseEntity->Ignite(20);
					//}
				}
				//}
			}
...
{
	StopParticleEffects(this);	// BriJee OVR: Clear all particle effects
	DispatchParticleOnce = false;

	int i;

	CPASAttenuationFilter filter4(this);
	EmitSound(filter4, entindex(), "gargantua.BeamAttackOff");
...
void CNPC_Gargantua::StompAttack(void)
{
	...
	CPASAttenuationFilter filter(this);
	EmitSound(filter, entindex(), "gargantua.StompSound");
...
bool CNPC_Gargantua::BecomeRagdoll(const CTakeDamageInfo &info, const Vector &forceVector)
{
	return true;//BaseClass::BecomeRagdoll(info, forceVector);//because we dont have the model's ragdoll
}
...
void CNPC_Gargantua::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	...
	if (subInfo.GetDamageType() & (GARG_DAMAGE | DMG_BLAST))
	{
		...
		{
			CPASAttenuationFilter filter(this);
			EmitSound(filter, entindex(), "gargantua.Pain");
