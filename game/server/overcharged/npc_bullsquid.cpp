...
//#define		BSQUID_AE_ROAR		( 4 ) // Light Kill : Valve you forget this function
#define		BSQUID_AE_TAILWHIP	( 4 )// Light Kill : Now time for true attack
...
void CNPC_Bullsquid::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		case BSQUID_AE_SPIT:
		{
			...
			int idx = random->RandomInt(5, 12);
			for (int i = 0; i < idx; i++)
			{
				CGrenadeBullsquid *pGrenade = (CGrenadeBullsquid*) CreateEntityByName( "grenade_bullsquid" );
				pGrenade->SetCanMakeDecal(i);
			...
			{
				pGrenade->SetAbsVelocity( ( vecToss + RandomVector( -0.065f, 0.065f ) ) * flVelocity );
#ifdef HL2_EPISODIC//OverCharged
			...
			//EmitSound( "NPC_Bullsquid.PoisonShoot" );
			AttackSound();
			}//end
		}
		break;
		...
		case BSQUID_AE_TAILWHIP:// Light Kill : Seriously Valve why you remove this ?
			{
			EmitSound( "NPC_Bullsquid.TailWhip" );
		...
		case BSQUID_AE_THROW:
			{
				...
				if ( pHurt )
				{
					...
					if ( pHurt->IsPlayer())
					{
						Vector forward, up;
						AngleVectors( GetLocalAngles(), &forward, NULL, &up );
						pHurt->ApplyAbsVelocityImpulse( forward * 150 + up * 150 );
					}
					...
					else
					{
						...
						if (pVictim)
						{
							if ( pVictim->DispatchInteraction( g_interactionBullsquidThrow, NULL, this ) )
							{
								Vector forward, up;
								AngleVectors( GetLocalAngles(), &forward, NULL, &up );
								pVictim->ApplyAbsVelocityImpulse( forward * 150 + up * 100 );
							}
						}
					}
				}
...
Vector VecCheckThrowToleranceUpd( CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed, float flTolerance )
{
...
bool CNPC_Bullsquid::GetSpitVector( const Vector &vecStartPos, const Vector &vecTarget, Vector *vecOut )
{
	...
	Vector vecToss = VecCheckThrowToleranceUpd( this, vecStartPos, vecTarget, /*sk_bullsquid_spit_speed.GetFloat()*/ 600, (10.0f*12.0f) );

	// If this failed then try a little faster (flattens the arc)
	if ( vecToss == vec3_origin )
	{
		vecToss = VecCheckThrowToleranceUpd( this, vecStartPos, vecTarget, /*sk_bullsquid_spit_speed.GetFloat()*/ 600 * 1.5f, (10.0f*12.0f) );
		if ( vecToss == vec3_origin )
			return false;
	}
...
int CNPC_Bullsquid::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	{
		return ( COND_CAN_MELEE_ATTACK1 ); //1
	}
...
int CNPC_Bullsquid::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if (inputInfo.GetAttacker() && !FClassnameIs(inputInfo.GetAttacker(), "npc_headcrab"))
	{
	...
	{
		int MaxDripsGreen = RandomInt(4, 7);
