...
public:
	...
	/*AutoClass()
	{
		//allWeapons.AddToHead(*CBaseCombatWeapon);
	}*/

	

	/*T* SelectClass(const char *nameOfClass)
	{
		T* curClass = nullptr;
		if (allWeapons.Count > 0)
		{
			...
			{
				if (Q_stricmp(MAKE_STRING(allWeapons.Element(i)), nameOfClass) == 0)
					curClass = *allWeapons.Element(i);
				return curClass;
			}
		}
...
public:
	...
	{
		if (Q_stristr("weapon_", pClassName))
		{
			//T newClass = SelectClass(pClassName);
	...
	CUtlVector<CBaseEntity*> allWeapons;*/
...
bool UTIL_GetIsPrecachedModel(CBaseEntity *pEntity, const char *pModelName);

//void			UTIL_SetPitch(float pitch);
//float				UTIL_GetPitch();
