...
DEFINE_FIELD( m_flFractionalBoost, FIELD_FLOAT ),
	...
	DEFINE_FIELD( m_bSpawnNoRagdoll, FIELD_BOOLEAN ),
...
CBaseCombatWeapon *CBaseCombatCharacter::GetWeapon(const char * sWeapon)
{
	CBaseCombatWeapon *pWeapon = NULL;

	for (int i = 0; i < m_hMyWeapons.Count(); i++)
	{
		if (m_hMyWeapons[i] != NULL)
		{
			if (FStrEq(m_hMyWeapons[i].Get()->GetClassname(), sWeapon))
				pWeapon = m_hMyWeapons[i].Get();
		}
	}
...
Activity CBaseCombatCharacter::Weapon_TranslateActivity( Activity baseAct, bool *pRequired )
{
	...
	bool isGrenadeClosed = false;

	CBasePlayer *pPlayer = ToBasePlayer(this);

	if (pPlayer && pPlayer->IsPlayer() && m_hActiveWeapon)
	{
		if (cvar->FindVar("oc_player_allow_fast_gren_throw")->GetInt() ? pPlayer->grenadeState <= 0 && m_hActiveWeapon->thisType == TYPE_GRENADE : false)
			isGrenadeClosed = true;
	}

	if (m_hActiveWeapon && !isGrenadeClosed)
	{
...
bool CBaseCombatCharacter::BecomeRagdollBoogie( CBaseEntity *pKiller, const Vector &forceVector, float duration, int flags )
{
	...
	if (createGradollConstrainted)
		createGradollConstrainted = (bool)(pDroppedWeapon != NULL);

	const char *pAttachmentName = createGradollConstrainted ? "anim_attachment_RH" : "NULL";
	const char *pWeaponName = createGradollConstrainted ? pDroppedWeapon->GetClassname() : "NULL";

	CBaseEntity *pRagdoll = createGradollConstrainted ? CreateServerRagdoll(this, 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, pDroppedWeapon, pAttachmentName, pWeaponName, true, true, BloodColor()) :
		CreateServerRagdoll(this, 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, true, BloodColor());
...
bool CBaseCombatCharacter::BecomeRagdoll( const CTakeDamageInfo &info, const Vector &forceVector )
{
	...
	//DevMsg("BecomeRagdoll basecombatCharacter \n");
...
// UNDONE: Put in a real sound cue here, don't do this bogus hack anymore
		...
		CBaseEntity *pRagdoll = createGradollConstrainted ? 
			CreateServerRagdoll(this, m_nForceBone, info2, COLLISION_GROUP_INTERACTIVE_DEBRIS, pDroppedWeapon, pAttachmentName, pWeaponName, true, true, BloodColor()) :
			CreateServerRagdoll(this, m_nForceBone, info2, COLLISION_GROUP_INTERACTIVE_DEBRIS, true, BloodColor());
	...
	{
		CBaseEntity *pRagdoll = createGradollConstrainted ?
			CreateServerRagdoll(this, m_nForceBone, newinfo, COLLISION_GROUP_DEBRIS, pDroppedWeapon, pAttachmentName, pWeaponName, true, true, BloodColor()) :
			CreateServerRagdoll(this, m_nForceBone, newinfo, COLLISION_GROUP_DEBRIS, false, BloodColor());
	...
	if (CanBecomeServerRagdoll())
	{
		...
		CBaseEntity *pRagdoll = createGradollConstrainted ?
			CreateServerRagdoll(this, m_nForceBone, newinfo, e_CGT, pDroppedWeapon, pAttachmentName, pWeaponName, true, true, BloodColor()) :
			CreateServerRagdoll(this, m_nForceBone, newinfo, e_CGT, true, BloodColor());
		...
		if (info.GetDamageType() & DMG_GAUSS)
			CRagdollBoogie::Create(pRagdoll, 200, gpGlobals->curtime, 1.4f, SF_RAGDOLL_BOOGIE_ELECTRICAL);
	...
	if (!IsValidRagdollName())
		return true;

	if (hl2_episodic.GetBool() && Classify() == CLASS_PLAYER_ALLY_VITAL)
	{
		if (createGradollConstrainted)
			CreateServerRagdoll(this, m_nForceBone, newinfo, COLLISION_GROUP_INTERACTIVE_DEBRIS, pDroppedWeapon, pAttachmentName, pWeaponName, true, true, BloodColor());
		else
			CreateServerRagdoll(this, m_nForceBone, newinfo, COLLISION_GROUP_INTERACTIVE_DEBRIS, true, BloodColor());
...
void CBaseCombatCharacter::SetupServerRagdoll(CBaseEntity *pRagdoll)
{
...
bool CBaseCombatCharacter::CanBecomeServerRagdoll()
{
	...
	bool invalidClassName = FClassnameIs(this, "npc_hydra") || FClassnameIs(this, "npc_antlionguard");

	bool isServerSide = IsOnFire() || cvar->FindVar("oc_ragdoll_serverside")->GetInt();

	if (((m_bForceServerRagdoll == true || ((bMegaPhyscannonActive == true) && !IsPlayer())) || isServerSide) && !invalidClassName)
		return true;
...
void CBaseCombatCharacter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (ShouldGib(info) == false && /*!FClassnameIs(this, "npc_rollermine")&&*/ !m_bSpawnNoRagdoll)
	{
		...
		if ( (info.GetDamageType() & DMG_DISSOLVE) && CanBecomeRagdoll() )
		{
			...
			if (pActiveWeapon && !cvar->FindVar("oc_weapon_disable_dissolve")->GetBool())
			{
				pActiveWeapon->Dissolve(NULL, gpGlobals->curtime, false, nDissolveType);
			}
		}
		else if ((info.GetDamageType() & DMG_DISSOLVE_EGON_ELECTRICAL) && CanBecomeRagdoll())
		{

			int nDissolveType = ENTITY_DISSOLVE_ELECTRICAL;


			bRagdollCreated = Dissolve_Egon(NULL, gpGlobals->curtime, false, nDissolveType);
		...
		else if ((info.GetDamageType() & DMG_DISSOLVE_EGON) && CanBecomeRagdoll())
		{
...
#ifdef HL2_DLL
		else if ( PlayerHasMegaPhysCannon() && !cvar->FindVar("oc_weapon_disable_dissolve")->GetBool())
		{
			if (pActiveWeapon)
			{
				pActiveWeapon->Dissolve(NULL, gpGlobals->curtime, false, ENTITY_DISSOLVE_NORMAL);
			}
		}
#endif
...
void CBaseCombatCharacter::ThrowDirForWeaponStrip(const Vector &vecForward, Vector *pVecThrowDir)
{
	// Nowhere in particular; just drop it.
	VMatrix zRot;
	MatrixBuildRotateZ(zRot, random->RandomFloat(-60.0f, 60.0f));
...
void CBaseCombatCharacter::Weapon_DropAll( bool bDisallowWeaponPickup )
{
	...
	for (int i=0; i<MAX_WEAPONS; ++i) 
	{
		...
		if (pWeapon->thisType == TYPE_GRENADE)
		{
			CBasePlayer *pPlayer = ToBasePlayer(this);

			pPlayer->RemoveGrenadeFromSlot(pWeapon);

			if (pPlayer->HasGrenade(pWeapon))
				pPlayer->GetAndRemoveGrenade(pWeapon);

			//Old Grenades
			/*if (pPlayer->pAllGrenades.HasElement(pWeapon))
				pPlayer->pAllGrenades.FindAndRemove(pWeapon);*/

			pPlayer->UpdateGrenadeInfo();
		}
...
#if 0
	CBasePlayer *pPlayer = ToBasePlayer(this);
	for (int i = 0; i < pPlayer->pAllGrenades.Count(); ++i)
	{
		CBaseCombatWeapon *pGren = (CBaseCombatWeapon*)(pPlayer->pAllGrenades.Element(i));
		if (!pGren)
			continue;

		const char *classname = typeid(pPlayer->pAllGrenades.Element(i)).name();// pGren->m_iClassname.ToCStr();

		if (FStrEq(typeid(pGren).name(), "CBaseAnimating"))
		//if (!classname || classname && !Q_strlen(classname))
		{
			pPlayer->RemoveGrenadeFromSlot(pGren);

			if (pPlayer->pAllGrenades.HasElement(pGren))
				pPlayer->pAllGrenades.FindAndRemove(pGren);

			pGren->SetRemoveable(false);
			Weapon_Detach(pGren);
		...
		{
			CBaseCombatWeapon *pWeapon = m_hMyWeapons[j];
			if (!pWeapon)
				continue;
		...
		DropWeaponForWeaponStrip(pGren, vecForward, gunAngles, flDiameter);
		...
		ThrowDirForWeaponStrip(vecForward, &vecThrow);
		...
		if (tr.startsolid || tr.allsolid || (tr.fraction < 1.0f && tr.m_pEnt != pGren))
		{
			//FIXME: Throw towards a known safe spot?
			vecThrow.Negate();
			VectorMA(vecOrigin, flDiameter, vecThrow, vecOffsetOrigin);
		...
		pGren->SetAbsOrigin(vecOrigin);

		pGren->SetAbsAngles(gunAngles);
		pGren->Drop(vecThrow);
		pGren->SetRemoveable(false);
		Weapon_Detach(pGren);*/
		...
		if (bDisallowWeaponPickup)
		{
			pGren->RemoveSolidFlags(FSOLID_TRIGGER);

			IPhysicsObject *pObj = pGren->VPhysicsGetObject();
...
void CBaseCombatCharacter::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget /* = NULL */, const Vector *pVelocity /* = NULL */ )
{
	...
	if ( IsPlayer() )
	{
		...
		if (pWeapon->UsesClipsForAmmo1())
		{
			pWeapon->m_iClip1 = pWeapon->m_iClip1;//pWeapon->GetDefaultClip1();
		}
		if (pWeapon->UsesClipsForAmmo2())
		{
			pWeapon->m_iClip2 = pWeapon->m_iClip2;//pWeapon->GetDefaultClip2();
		}

		pWeapon->AddSpawnFlags(FL_GRAPHED);
		...
		//pWeapon->SetPrimaryAmmoCount(GetActiveWeapon()->m_iClip1);
		QAngle gunAngles;
		VectorAngles( BodyDirection2D(), gunAngles );
...
void CBaseCombatCharacter::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	else if (pWeapon->GetDefaultClip1() >  pWeapon->GetMaxClip1() )
	{
		if (pWeapon->HasSpawnFlags(FL_GRAPHED))
		{
			pWeapon->m_iClip1 = pWeapon->m_iClip1;
		}
		else
		{
	...
	else if ( pWeapon->GetDefaultClip2() > pWeapon->GetMaxClip2() )
	{
		if (pWeapon->HasSpawnFlags(FL_GRAPHED))
		{
			pWeapon->m_iClip2 = pWeapon->m_iClip2;
			//GiveAmmo((pWeapon->m_iClip2 - pWeapon->m_iClip2), pWeapon->m_iSecondaryAmmoType);
		}
		else
		{
	...
	CBasePlayer *pPlayer = ToBasePlayer(this);
	if (pPlayer && pPlayer->IsPlayer() && pWeapon)
	{
		if (cvar->FindVar("oc_player_allow_fast_gren_throw")->GetInt() ? pWeapon->thisType == TYPE_GRENADE : false)
		{
			pPlayer->RefreshGrenadeNameInSlot(pWeapon);

			pPlayer->RefreshGrenadeAmmoInSlot(pWeapon);
		}
	}
...
bool CBaseCombatCharacter::Weapon_CanUse( CBaseCombatWeapon *pWeapon )
{
	...
	if (pPlayer && pPlayer->IsPlayer() && pWeapon)
	{
		if (cvar->FindVar("oc_player_allow_fast_gren_throw")->GetInt() ? pPlayer->grenadeState <= 0 && pWeapon->thisType == TYPE_GRENADE : false)
			return false;
	}
...
// Output :
//-----------------------------------------------------------------------------
void CBaseCombatCharacter::Weapon_HandleAnimEvent(animevent_t *pEvent, bool resetLoopingSound)
{
	// UNDONE: Some check to make sure that pEvent->pSource is a weapon I'm holding?
	if ( m_hActiveWeapon )
	{
		// UNDONE: NPCS looping sound turning on
		/*if (m_hActiveWeapon->GetWpnData().allowLoopSound)
		{
			if (resetLoopingSound)
			{
				if (m_hActiveWeapon->m_bIsFiring)
				{
					m_hActiveWeapon->StopLoopSound();
				}
			}
			else
			{
				FireNPCPrimaryAttack(pEvent);
			}
		}
		else*/
...
void CBaseCombatCharacter::NPCPrimaryAttack(animevent_t *pEvent)
{
	Vector vecShootOrigin, vecShootDir;
	QAngle angDiscard;

	if (m_hActiveWeapon->GetWpnData().allowLoopSound && !m_hActiveWeapon->m_bIsFiring)
	{
		m_hActiveWeapon->m_bIsFiring = true;
		m_hActiveWeapon->ReinitializeLoopSound(m_hActiveWeapon->GetWpnData().ShootLoopingSound);
		m_hActiveWeapon->StartLoopSound();
	}
	
	// Support old style attachment point firing
	if ((pEvent->options == NULL) || (pEvent->options[0] == '\0') || (!GetAttachment(pEvent->options, vecShootOrigin, angDiscard)))
	{
	...
	FireBullets(1, vecShootOrigin, vecShootDir, GetAttackSpread(m_hActiveWeapon),
		MAX_TRACE_LENGTH, m_hActiveWeapon->m_iPrimaryAmmoType, 1, entindex(), 0);

	DoMuzzleFlash();
	m_hActiveWeapon->m_iClip1 = m_hActiveWeapon->m_iClip1 - 1;
}
...
void CBaseCombatCharacter::CharacterEmitSound(const char *m_sName, const Vector &m_vOrigin, int m_eChannel)
{
	CPASAttenuationFilter filter(this);

	EmitSound_t ep;
	ep.m_nChannel = m_eChannel;// CHAN_WEAPON;
...
void CBaseCombatCharacter::DispatchBloodOverlay(const CTakeDamageInfo &info)
{
	...
	if (cvar->FindVar("oc_player_bloodoverlay")->GetInt() && i <= 3 && (info.GetDamageType() & (DMG_BULLET | DMG_AIRBOAT |DMG_SLASH | DMG_DISSOLVE | DMG_GAUSS | DMG_BLAST | DMG_CLUB | DMG_BUCKSHOT)))
	{
		...
		if (pPlayer && !this->IsPlayer() && (this->BloodColor() != BLOOD_COLOR_MECH))
		{
			trace_t tr;
			...
			//UTIL_TraceLine(info.GetDamagePosition(), pPlayer->GetAbsOrigin(), MASK_PLAYERSOLID, NULL, COLLISION_GROUP_PLAYER, &tr);

			/*AI_TraceLine(PlrPos, info.GetDamagePosition(), MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr);

			AI_TraceLine(tr.endpos, info.GetDamageForce(), MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr);*/

			//DevMsg("DistTo: %.2f \n", PlrPos.DistTo(info.GetDamagePosition()));

			//if (pPlayer->GetAbsOrigin().DistTo(info.GetDamagePosition()/*this->GetAbsOrigin()*/) < 100.0f)
			...
			//if (PlrPos.DistTo(tr.endpos) <= 100 && isFlesh)
			if (PlrPos.DistTo(info.GetDamagePosition()) <= 90)
			{
...
{
	if (FClassnameIs(this, "npc_sporegenerator"))
		return 0;
...
//-----------------------------------------------------------------------------
int CBaseCombatCharacter::GiveAmmo(int iCount, int iAmmoIndex, bool bSuppressSound, bool refreshGrenade)
{
	if (iCount <= 0)
		return 0;
	...
	{
		if (pPlayer->pCurGrenade && pPlayer->pCurGrenade->m_iPrimaryAmmoType == iAmmoIndex)
		{
			refreshGrenade = true;
		}
	}
	...
	if (refreshGrenade)
	{
		if (pPlayer && pPlayer->IsPlayer())
		{
			pPlayer->RefreshGrenadeAmmoInSlot(pPlayer->pCurGrenade);
		}
	}
...
int CBaseCombatCharacter::GiveAmmo( int iCount, const char *szName, bool bSuppressSound )
{
	...
	bool refreshGrenade = false;

	CBasePlayer *pPlayer = ToBasePlayer(this);

	if (pPlayer && pPlayer->IsPlayer())
	{
		if (pPlayer->pCurGrenade && pPlayer->pCurGrenade->m_iPrimaryAmmoType == iAmmoType)
		{
			refreshGrenade = true;
		}
	}
	...
	return GiveAmmo(iCount, iAmmoType, bSuppressSound, refreshGrenade);
}
...
void CBaseCombatCharacter::DoMuzzleFlashOVR(int attachmentIndex, int flashType, int r, int g, int b, const char *name)
{
	BaseClass::DoMuzzleFlashOVR(attachmentIndex, flashType, r, g, b, name);
}
//-----------------------------------------------------------------------------
