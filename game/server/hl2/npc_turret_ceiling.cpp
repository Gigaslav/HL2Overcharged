...
int CNPC_CeilingTurret::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	if ( m_iHealth <= 0 )
	{
		...
		ExplosionCreate( GetAbsOrigin(), GetLocalAngles(), this, 100, 100, false, true );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CeilingTurret::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr, int iTracerType)
{
#if 1
	DoMuzzleFlash();
