...
int CCombineDropshipContainer::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	/*if ((info.GetDamageType() & (DMG_BLAST | DMG_AIRBOAT | DMG_DISSOLVE | DMG_SHOCK | DMG_GAUSS | DMG_PHYSGUN)) == 0)
		return 0;*/

	if ((info.GetDamageType() & (DMG_AIRBOAT | DMG_GAUSS | DMG_ENERGYBEAM | DMG_BLAST)) == 0)
		return 0;
	...
	if ( nPrevHealth != GetHealth() )
	{
		...
		if ( ShouldTriggerDamageEffect( nPrevHealth, MAX_EXPLOSIONS ) )
		{
			ExplosionCreate( dmgInfo.GetDamagePosition(), vec3_angle, this, 1000, 500.0f, 
				SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE, true, 0);
...
void CCombineDropshipContainer::AddSmokeTrail( const Vector &vecPos )
{
	...
	ExplosionCreate( vecPos, vec3_angle, this, 1000, 500.0f, SF_ENVEXPLOSION_NODAMAGE | 
		SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE, true, 0 );
...
int CNPC_CombineDropship::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo ) 
{
	...
	{
		if ( (inputInfo.GetDamageType() & (DMG_AIRBOAT | DMG_GAUSS | DMG_ENERGYBEAM | DMG_BLAST)) || (m_iCrateType == CRATE_SOLDIER) )
		{
...
bool CNPC_CombineDropship::FireCannonRound( void )
{
	...
	// Fire the bullet
	int ammoType = GetAmmoDef()->Index("AirboatGun"); 
