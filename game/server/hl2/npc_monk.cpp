...
int CNPC_Monk::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
	/*if (cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() == 0 && inputInfo.GetAttacker()->IsPlayer())
		return 0;*/
...
{
	if (info.GetAttacker() && info.GetAttacker()->ClassMatches("npc_headcrab_black") || info.GetAttacker()->ClassMatches("npc_headcrab_poison"))
		return false;
	/*if (cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() == 0 && info.GetAttacker()->IsPlayer())
		return false;*/
