...
//#ifdef HL2_EPISODIC////////////	Данный препроцессор не хотел работать, вот я его и отключил:)
...
void CFuncTank::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else if ( m_hController != pPlayer && useType != USE_OFF )
	{
		...
		//UTIL_HudHintText(this, "#OC_PlayerRemoveGun");

		//this->AddEffects(EF_DIMLIGHT);
	...
	{
		if (!pPlayer->HasNamedPlayerItem("weapon_ar3"))
		{
			if (pPlayer->m_nButtons & IN_ATTACK2)//OverCharged BG Detach
			{
				//pPlayer->SetArmorValue(pPlayer->ArmorValue() + 100);
				this->IsTransparent();
				this->AddEFlags(EF_NODRAW);
				InterruptUse = true;
				//this->SetSize(Vector(0, 0, 0), Vector(0, 0, 0));
				this->GetMoveParent()->Remove();
				this->GetParent()->Remove();
				this->Remove();
				//cvar->FindVar("oc_BG_ammo_count")->SetValue(cvar->FindVar("sk_max_BG_primary_clip")->GetInt());
				pPlayer->GiveNamedItem("weapon_ar3");
				StopControl();
				//static CWeaponBunkerGun *pWeapon2;
				//pPlayer->Weapon_Switch(pWeapon2);
			}
		}
		/*else if (pPlayer->HasNamedPlayerItem("weapon_ar3"))
		{
			if (pPlayer->m_nButtons & IN_ATTACK2)//OverCharged BG Detach
			{
				pPlayer->SetArmorValue(pPlayer->ArmorValue() + 100);
		...
		/*else if (pPlayer->HasNamedPlayerItem("weapon_ar3"))
		{
			if (pPlayer->m_nButtons & IN_ATTACK2)//OverCharged BG Detach
			{
				//static CWeaponBunkerGun *pWeapon;
				//pWeapon->m_iClip1 = cvar->FindVar("sk_max_BG_primary_clip")->GetInt();
				//pPlayer->Weapon_Drop(pWeapon, &pPlayer->GetAbsOrigin(), NULL);
				//UTIL_Remove(pWeapon);
				pPlayer->RemovePlayerItem();
				CBaseEntity *pVial = NULL; // Null
				pVial = CBaseEntity::Create("item_empty_bunkergun", pPlayer->Weapon_ShootPosition(), pPlayer->GetAbsAngles(), pPlayer); // Creates
				...
				if (pPhysicsObject)
				{
					...
					pPhysicsObject->SetVelocity(&pPlayer->GetBaseVelocity(), NULL);// Throw vial
	...
	{
		if (!pPlayer->HasNamedPlayerItem("weapon_airboatgun"))
		{
			if (pPlayer->m_nButtons & IN_ATTACK2)//OverCharged BG Detach
			{
...
void CFuncTank::DoMuzzleFlash( void )
{
	...
	if ( m_bUsePoseParameters && GetParent() )
	{
		CBaseAnimating *pAnim = GetParent()->GetBaseAnimating();
		//pAnim->DoMuzzleFlash();
		//DispatchParticleEffect("ar2_normal_npc", PATTACH_POINT_FOLLOW, pAnim, "muzzle", false);
		...
		else
		{
			...
			else
			{
				...
				data.m_nAttachmentIndex = m_nBarrelAttachment;
				data.m_nEntIndex = pAnim->entindex();
				DispatchEffect("Ar2MuzzleFlashParticle", data);*/
				DispatchParticleEffect("ar2_normal_npc", PATTACH_POINT_FOLLOW, pAnim, "muzzle", false);
			}
		}
	}
...
void CFuncTank::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if ( m_iEffectHandling == EH_AR2 )
	{
		DoMuzzleFlash();
		//QAngle Ang;
		//VectorAngles(barrelEnd, Ang);
		//DispatchParticleEffect("abgun_normal2", PATTACH_POINT_FOLLOW, this, "muzzle", false);
		//DispatchParticleEffect("abgun_normal2", barrelEnd, Ang, this);
		// Play the AR2 sound
...
void CFuncTankGun::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	//DevMsg("Fire\n");
	//DevMsg("bulletCount: %.i \n", bulletCount);
	if ((m_iAmmoType != -1) && (m_iEffectHandling != EH_AR2))
	{
		for (i = 0; i < bulletCount; i++)
		{
			info.m_iAmmoType = m_iAmmoType;
...
{
	UTIL_ParticleTracer("weapon_npc_tracer_ar2", vecTracerSrc, tr.endpos, 0, TRACER_DONT_USE_ATTACHMENT, iTracerType == TRACER_LINE_AND_WHIZ);//OverCharged
}*/
...
public:
	...
	//virtual int GetTracerAttachment(){ return m_nGunBarrelAttachment; }
...
void CFuncTankAirboatGun::ControllerPostFrame( void )
{
	...
	Vector vecOrigin;
	QAngle Ang;
	m_hAirboatGunModel->GetAttachment(m_nGunBarrelAttachment, vecOrigin, Ang);
	Vec = vecOrigin;
	QAn = Ang;
	}
...
void CFuncTankAirboatGun::DoMuzzleFlash( void )
{
	if ( m_hAirboatGunModel && (m_nGunBarrelAttachment != 0) )
	{
		...
		DispatchParticleEffect("abgun_normal2", PATTACH_POINT_FOLLOW, this, m_nGunBarrelAttachment, false);*/
		if (!cvar->FindVar("oc_particle_muzzleflashes")->GetInt())
		{
			CEffectData data;
			data.m_nAttachmentIndex = m_nGunBarrelAttachment;
...
//---------------------------------------------------------
//---------------------------------------------------------
/*void CFuncTankAirboatGun::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr, int iTracerType)
{
	const char *pszTracerName = GetTracerType();

	Vector vNewSrc = vecTracerSrc;

	int iAttachment = m_nGunBarrelAttachment;
	Vector vecOrigin;
	QAngle Ang;
	m_hAirboatGunModel->GetAttachment(m_nGunBarrelAttachment, vecOrigin, Ang);

	if (cvar->FindVar("oc_particle_tracers")->GetInt())
		UTIL_ParticleTracer("weapon_npc_tracer_ar2", vecOrigin, tr.endpos, entindex(), iAttachment, iTracerType == TRACER_LINE_AND_WHIZ);//OverCharged
	else		
		UTIL_Tracer(vecOrigin, tr.endpos, entindex(), iAttachment, 6000.0f, true, pszTracerName);
}*/
//-----------------------------------------------------------------------------
...
void CFuncTankAirboatGun::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	//DispatchParticleEffect("abgun_normal3", Vec, QAn, this);
