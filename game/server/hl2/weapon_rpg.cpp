...
ConVar oc_weapon_rpg_laser_x("oc_weapon_rpg_laser_x", "0");
ConVar oc_weapon_rpg_laser_y("oc_weapon_rpg_laser_y", "0");
ConVar oc_weapon_rpg_laser_z("oc_weapon_rpg_laser_z", "0");
...
void CMissile::CreateSmokeTrail( void )
{
	...
	DispatchParticleEffect("weapon_rpg_fireSmoke", PATTACH_ABSORIGIN_FOLLOW, this);
	DispatchParticleEffect("weapon_rpg_fireJet", PATTACH_ABSORIGIN_FOLLOW, this);
...
void CAPCMissile::DoExplosion( void )
{
	...
	{
#ifdef HL2_EPISODIC
		ExplosionCreate( GetAbsOrigin(), GetAbsAngles(), this, APC_MISSILE_DAMAGE, 100, true, false, 20000 );
#else
...
BEGIN_DATADESC( CWeaponRPG )
	...
	DEFINE_FIELD( fired,				FIELD_BOOLEAN ),
	DEFINE_FIELD( secondAttack,			FIELD_BOOLEAN ),
	DEFINE_UTLVECTOR(allMissiles,		FIELD_CLASSPTR),
...
{
	{ ACT_HL2MP_IDLE,                    ACT_HL2MP_IDLE_RPG,                    false },	// Light Kill : MP animstate for singleplayer
    ...
    //{ ACT_RANGE_ATTACK1,                ACT_RANGE_ATTACK_RPG,                false },		//END
...
void CWeaponRPG::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
		case EVENT_WEAPON_SMG1:
		{
			...
			allMissiles.AddToTail(m_hMissile);
...
/*	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	...
	if (pPlayer)
	{
		switch (pPlayer->GetActiveWeapon()->FlashType())
		{
		case 1:
			pPlayer->AddEffects(EF_MAR2);
			break;
		case 2:
			pPlayer->AddEffects(EF_M);
			break;
		case 3:
			pPlayer->AddEffects(EF_ML);
			break;
		default:
			break;
		}
	}
	MuzzleFlashTime = 0.0f;//При каждом выстреле, переменная ставится на 0.*/
	// Can't have an active missile out
	if (m_hMissile != NULL && m_bGuiding)
		return;
...
void CWeaponRPG::ItemPostFrame( void )
{
	...
	if (pOwner && !IsGuiding())
	{
		if (fired && IsViewModelSequenceFinished())
		{
			Reload();
			fired = false;
		}
	...
	//}
	if (!fired && m_flNextPrimaryAttack < gpGlobals->curtime)
		m_bInReload = false;


	if (!((pPlayer->m_nButtons & IN_ATTACK) || (pPlayer->m_nButtons & IN_ATTACK2) || (/*CanReload() &&*/ pPlayer->m_nButtons & IN_RELOAD) || (pPlayer->m_nButtons & IN_FIREMODE)))	// BriJee OVR: IN_FIREMODE added
	{
		// no fire buttons down or reloading
		if (!ReloadOrSwitchWeapons() && (m_bInReload == false) && m_bReloadComplete == 0 && m_flNextPrimaryAttack <= gpGlobals->curtime)
		{
			//DevMsg("WeaponIdle \n");
			WeaponIdle();
		}
...
void CWeaponRPG::UpdateNPCLaserPosition( const Vector &vecTarget )
{
	...
	UpdateNpcLaserPosition(muzzlePoint, vecDir);
...
void CWeaponRPG::UpdateNpcLaserPosition( Vector vecMuzzlePos, Vector vecEndPos )
{
...
{
	/*CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	pPlayer->GetViewModel()->GetAttachmentLocal*/

	/*Vector vecMuzzlePos2; Vector vecEndPos2;
	QAngle angDir;
	Vector vAttachPos, vecRes, vecX, vecY, vecZ;
	GetAttachment("muzzle", vAttachPos, angDir);
	AngleVectors(angDir, &vecX, &vecY, &vecZ);
	vAttachPos.x = vAttachPos.x + oc_weapon_rpg_laser_x.GetFloat();
	vAttachPos.y = vAttachPos.y + oc_weapon_rpg_laser_y.GetFloat();
	vAttachPos.z = vAttachPos.z + oc_weapon_rpg_laser_z.GetFloat();
	vecRes = vecX * MAX_TRACE_LENGTH + vecY + vecZ;*/


	//vecRes = vAttachPos;
	//vecRes += vecX * MAX_TRACE_LENGTH;



	//vecRes += vecY;
	//vecRes += vecZ;
	//vecMuzzlePos2 = vAttachPos;
	//vecEndPos2 = vecRes;
	...
	if (vecMuzzlePos == vec3_origin || vecEndPos == vec3_origin)
	{
		...
		//vecMuzzlePos = vAttachPos;// pPlayer->Weapon_ShootPosition();
		vecMuzzlePos = pPlayer->Weapon_ShootPosition();
		Vector	forward;
		...
		//vecEndPos = vecMuzzlePos + (forward * MAX_TRACE_LENGTH) + (vecY * 10) + (vecZ * 10);
		//vecEndPos = vAttachPos + vecRes;
	...
	UTIL_TraceLine( vecMuzzlePos, vecEndPos, (MASK_SHOT & ~CONTENTS_WINDOW), this, COLLISION_GROUP_NONE, &tr );
#endif
	//debugoverlay->AddLineOverlay(vecMuzzlePos, vecEndPos, 255, 255, 0, false, 0.01);
...
void CWeaponRPG::NotifyRocketDied( void )
{
	...
	allMissiles.Remove(allMissiles.Count()-1);
...
CLaserDot *CLaserDot::Create( const Vector &origin, CBaseEntity *pOwner, bool bVisibleDot )
{
	...
	//pLaserDot->AddEffects(EF_NODRAW);
