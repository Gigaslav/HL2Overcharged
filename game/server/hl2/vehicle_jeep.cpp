...
ConVar  oc_jeep_checking_sphere_radius("oc_jeep_checking_sphere_radius", "23", FCVAR_ARCHIVE);
...
/*class CJeepEntity : public CBaseEntity
{
	DECLARE_CLASS(CJeepEntity, CBaseEntity);
public:
	CJeepEntity *TriggerCreate(CBaseEntity *pOwner, const Vector &position);// , const Vector &min, const Vector &max);
};
LINK_ENTITY_TO_CLASS(jeep_entity, CJeepEntity);

CJeepEntity *CJeepEntity::TriggerCreate(CBaseEntity *pOwner, const Vector &position)//, const Vector &min, const Vector &max)
{
	CJeepEntity *pTrigger = CREATE_ENTITY(CJeepEntity, "jeep_entity");
	...
	UTIL_SetSize(pTrigger, Vector(-10, -10, -10), Vector(10, 10, 10));
	NDebugOverlay::Box(position, Vector(-10, -10, -10), Vector(10, 10, 10), 0, 255, 0, true, 10.1f);
	return pTrigger;
}

CJeepEntity	*m_jpTrigger;*/
//=============================================================================
//
// Jeep water data.
//
...
/*DEFINE_FIELD(jfoundedPlayer, FIELD_BOOLEAN),
DEFINE_FIELD(jcount, FIELD_INTEGER),
DEFINE_FIELD(jcanTakeOffTheGun, FIELD_BOOLEAN),*/
...
DEFINE_FIELD(HasTakenGauss, FIELD_BOOLEAN),
...
void CPropJeep::Precache(void)
{
	...
	PrecacheScriptSound("Weapon_Gauss.Special2");
...
void CPropJeep::Spawn(void)
{
	...
	if (Q_strstr(STRING(GetModelName()), "buggy.mdl"))
	{
		m_bHasGun = true;
		m_bAllowDetach = true;
	}
...
{
	//Draw our beam
	DrawBeam(tr.startpos, tr.endpos, 1);
...
void CPropJeep::AimGunAt(Vector *endPos, float flInterval)
{
	...
	GetAttachment(LookupAttachment("gun_ref"), gunMatrix);//2,3
	...
	GetAttachment("Muzzle", vecMuzzle, vecMuzzleAng);//29
	...
	/*Vector Gun;
	QAngle Muz;
	int tau2 = LookupAttachment("gun_ref");
	this->GetAttachment(tau2, Gun, Muz);
	CShotgunBullet *pBolt1 = CShotgunBullet::BoltCreate(vecOrigin, pPlayer->EyeAngles());// , pOwner);
	//pBolt1->SetLocalVelocity((info.m_vecDirShooting + pPlayer->GetActiveWeapon()->GetBulletSpread()) * 300000);
	pBolt1->Spawn();

	pBolt1->SetParent(this, attachment);*/
}
...
void CPropJeep::Think(void)
{
	...
	//if (m_bAllowDetach && CheckAttachedGun("taucannon", oc_jeep_checking_sphere_radius.GetFloat(), true))
	//	GunText(oc_jeep_checking_sphere_radius.GetFloat(), m_bHasGun, "Press 'use' button to mount back the tau cannon", "Press 'use' button to pick up the tau cannon");

	// BJ: Goddamn localisation strings
	if (m_bAllowDetach && CheckAttachedGun("taucannon", oc_jeep_checking_sphere_radius.GetFloat(), true))
		GunText(oc_jeep_checking_sphere_radius.GetFloat(), m_bHasGun, "#OVR_MountTau", "#OVR_DismountTau");

	/*if (pWDetach != NULL)
	{
		pWDetach->SetOwnerEntity(this);

		pWDetach->LegacyThink(this, "weapon_gauss", "taucannon", "Press 'use' button to mount back the tau cannon", "Press 'use' button to tear off the tau cannon", oc_jeep_checking_sphere_radius.GetFloat(), HasTakenGauss);
	}
	else
	{
		{
			pWDetach = new CEntityWeaponDetach(this, "taucannon", Vector(-10, -10, -10), Vector(10, 10, 10));
		}
	}*/
...
void CPropJeep::DrawBeam(const Vector &startPos, const Vector &endPos, float width)
{
	...
	/*if (cvar->FindVar("oc_particle_tracers")->GetInt())
		UTIL_ParticleTracer("weapon_tracer_gauss", startPos, endPos, 0, TRACER_DONT_USE_ATTACHMENT, 1);//OverCharged
	else
		UTIL_Tracer(startPos, endPos, entindex(), TRACER_DONT_USE_ATTACHMENT, 6000.0f, true, "GaussTracer");*/
	...
	if (attachment)
	{
		...
		{
			DispatchParticleEffect("gauss_normal2_vehicle", PATTACH_POINT_FOLLOW, this, attachment, true);//OverCharged
			
			//DispatchParticleEffect("gauss_normal", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
			//DispatchParticleEffect("gauss_vehicle", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
		}

	}
	/*CPASFilter filter(m_vecGunOrigin);
	te->DynamicLight(filter, 0.0, &m_vecGunOrigin, random->RandomInt(225, 255), random->RandomInt(195, 155), random->RandomInt(195, 155), 4, 310, random->RandomInt(0.013, 0.13), 0);*/

	CEffectData data;
	data.m_vOrigin = m_vecGunOrigin;
	DispatchEffect("GaussLight", data);

	float life = cvar->FindVar("oc_weapon_gauss_beam_life")->GetFloat();
	...
	m_flNextSoundTime = gpGlobals->curtime + random->RandomFloat(0.5f, 1.1f);
...
void CPropJeep::FireCannon(void)
{
	...
	//EmitSound("Weapon_Gauss.Special2");
	...
	//DispatchParticleEffect("gauss_sparks01", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
	//DispatchParticleEffect("muzzle_sparks01", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
	//DispatchParticleEffect("gauss_zap01", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
...
void CPropJeep::FireChargedCannon(void)
{
	...
	{
		DispatchParticleEffect("gauss_normal2", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
		DispatchParticleEffect("gauss_vehicle2", PATTACH_POINT_FOLLOW, this, attachment, true);//OverCharged
	}*/
	if (m_hPlayer)
	{
	...
	/*CPASFilter filter(m_vecGunOrigin);
	te->DynamicLight(filter, 0.0, &m_vecGunOrigin, random->RandomInt(225, 255), random->RandomInt(195, 155), random->RandomInt(195, 155), 4, 310, random->RandomInt(0.013, 0.13), 70);*/
	...
	//Show the effect
	DrawBeam(m_vecGunOrigin, tr.endpos, 1.5);
...
void CPropJeep::RandomZap(bool hardSound)
{
	int attachment = LookupAttachment("Muzzle");
	if (attachment)
	{
		DispatchParticleEffect("gauss_zap_vehicle", PATTACH_POINT_FOLLOW, this, attachment, false);
	}
...
void CPropJeep::ChargeCannon(void)
{
	...
	/*CPASFilter filter(m_vecGunOrigin);
	te->DynamicLight(filter, 0.0, &m_vecGunOrigin, random->RandomInt(225, 255), random->RandomInt(195, 155), random->RandomInt(195, 155), 1, 310, random->RandomInt(0.019, 0.23), 50);//OverCharged
	*/
	//Don't fire again if it's been too soon
	if (m_flCannonTime > gpGlobals->curtime)
		return;
	...
	{
		if (cvar->FindVar("oc_particle_misc")->GetInt())
			DispatchParticleEffect("gauss_charge_jeep", PATTACH_POINT_FOLLOW, this, attachment, false);//OverCharged
...
void CPropJeep::UseAmmoCrate(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
...
void CPropJeep::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (!CheckAttachedGun("taucannon", oc_jeep_checking_sphere_radius.GetFloat(), true))
	{
		if (Q_strstr(STRING(GetModelName()), "buggy.mdl"))
			UseAmmoCrate(pActivator, pCaller, useType, value);

		return BaseClass::Use(pActivator, pCaller, useType, value);
	}
	else
		m_bCanDetach = m_bAllowDetach;
	...
	if (m_bCanDetach)
	{
		...
		int attachment = LookupAttachment("Muzzle");
		int tau = LookupAttachment("taucannon");
		if (attachment)
		{
			// Find out if the player's looking at our ammocrate hitbox 
			Vector vecForward;
			...
			Vector vecOrigin, vecOrigin2;// = this->WorldSpaceCenter();
			QAngle angDir, angDir2;

			Vector vecStart = pPlayer->EyePosition();
			UTIL_TraceLine(vecStart, vecStart + vecForward * 1024, MASK_SOLID | CONTENTS_DEBRIS | CONTENTS_HITBOX, pPlayer, COLLISION_GROUP_NONE, &tr);

			this->GetAttachment(attachment, vecOrigin, angDir);
			UTIL_TraceLine(vecStart, vecOrigin, MASK_ALL, pPlayer, COLLISION_GROUP_NONE, &tr2);
			this->GetAttachment(tau, vecOrigin2, angDir2);
			...
			CWeaponTau *pTau = static_cast<CWeaponTau*>(pPlayer->HasNamedPlayerItem("weapon_tau"));
			...
			if (!HasTakenGauss && !pPlayer->HasNamedPlayerItem("weapon_tau"))
			{
				HasTakenGauss = true;
				pPlayer->Weapon_SetLast(pPlayer->GetActiveWeapon());
			...
			else if (HasTakenGauss && pTau)//FClassnameIs(pPlayer->GetActiveWeapon(), "weapon_tau"))
			{
				HasTakenGauss = false;
				UTIL_Remove(pTau);
				//bool bRemove = pPlayer->RemoveWeapon((CBaseCombatWeapon*)pTau);

				SetBodygroup(1, true);
				m_bHasGun = true;
				if (FClassnameIs(pPlayer->Weapon_GetLast(), "weapon_tau"))
					pPlayer->SwitchToNextBestWeapon(pPlayer->Weapon_GetLast());
				else
					pPlayer->Weapon_Switch(pPlayer->Weapon_GetLast());

				EmitSound("Jeep.GaussReCharge");

				g_pEffects->Sparks(vecOrigin2);
	...
	/*if (pWDetach != NULL && !pWDetach->canTakeOffTheGun)
	{
...
void CPropJeep::HeadlightTurnOn(void)
{
	...
	if (pPlayer)
	{
		...
		EmitSound(filter, pPlayer->entindex(), "Airboat_headlight_on");
	}
...
void CPropJeep::HeadlightTurnOff(void)
{
	...
	{
		CSingleUserRecipientFilter filter(pPlayer);
		EmitSound(filter, pPlayer->entindex(), "Airboat_headlight_off");
	}
