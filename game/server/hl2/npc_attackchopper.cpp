...
private:
	...
	/*bool SF_HELICOPTER_LOUD_ROTOR_SOUND;
	int SF_HELICOPTER_ELECTRICAL_DRONE;
	bool SF_HELICOPTER_LIGHTS;
	bool SF_HELICOPTER_IGNORE_AVOID_FORCES;
	bool SF_HELICOPTER_AGGRESSIVE;
	bool SF_HELICOPTER_LONG_SHADOW;*/
...
//	DEFINE_FIELD( m_nSpotlightAttachment, FIELD_INTEGER ),
	...
	/*DEFINE_KEYFIELD(SF_HELICOPTER_LOUD_ROTOR_SOUND, FIELD_BOOLEAN, "Enable loud rotor sound"),
	DEFINE_KEYFIELD(SF_HELICOPTER_ELECTRICAL_DRONE, FIELD_INTEGER, "Enable electical gun"),
	DEFINE_KEYFIELD(SF_HELICOPTER_LIGHTS, FIELD_BOOLEAN, "Enable lights"),
	DEFINE_KEYFIELD(SF_HELICOPTER_IGNORE_AVOID_FORCES, FIELD_BOOLEAN, "Enable ignore avoid forces"),
	DEFINE_KEYFIELD(SF_HELICOPTER_AGGRESSIVE, FIELD_BOOLEAN, "Enable agressive behavior"),
	DEFINE_KEYFIELD(SF_HELICOPTER_LONG_SHADOW, FIELD_BOOLEAN, "Enable long shadow"),*/
...
void CNPC_AttackHelicopter::Precache( void )
{
	...
	//if ( SF_HELICOPTER_ELECTRICAL_DRONE )
	...
	//if ( SF_HELICOPTER_LOUD_ROTOR_SOUND )
...
void CNPC_AttackHelicopter::Spawn( void )
{
	...
	//float flChaseDist = SF_HELICOPTER_AGGRESSIVE ? 
...
void CNPC_AttackHelicopter::ExplodeAndThrowChunk( const Vector &vecExplosionPos )
{
	...
	if(GetCrashPoint() != NULL)
	{
		...
		ExplosionCreate( vecExplosionPos, QAngle(0,0,1), this, 100, 128, false, true );
	}
...
void CNPC_AttackHelicopter::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	if ((info.GetDamageType() & DMG_AIRBOAT | DMG_DISSOLVE | DMG_SHOCK | DMG_GAUSS | DMG_PHYSGUN) ||
		(info.GetInflictor() && info.GetInflictor()->Classify() == CLASS_MISSILE) ||
		(info.GetAttacker() && info.GetAttacker()->Classify() == CLASS_MISSILE))
	{
...
int CNPC_AttackHelicopter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if( info.GetInflictor() != this )
	{
		if (((info.GetDamageType() & DMG_AIRBOAT) == 0 &&
			(info.GetDamageType() & DMG_DISSOLVE) == 0 &&
			(info.GetDamageType() & DMG_SHOCK) == 0 &&
			(info.GetDamageType() & DMG_GAUSS) == 0 &&
			(info.GetDamageType() & DMG_PHYSGUN) == 0) &&
...
int CNPC_AttackHelicopter::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	// Chain
	int nRetVal = /*info.GetDamage();*/BaseClass::OnTakeDamage_Alive( info );
	...
	float flAdditionalHeight, float flMinDistFromSegment, float flMaxDistFromSegment, Vector *pVecAccel )
{
	...
	//if ( SF_HELICOPTER_IGNORE_AVOID_FORCES )
...
void CNPC_AttackHelicopter::UpdateFacingDirection( const Vector &vecActualDesiredPosition )
{
	...
	//bool bSeenTargetRecently = SF_HELICOPTER_AGGRESSIVE || ( m_flLastSeen + 5 > gpGlobals->curtime ); 
...
void CNPC_AttackHelicopter::Hunt( void )
{
	...
	//if ( SF_HELICOPTER_AGGRESSIVE && GetEnemy() )
...
void CGrenadeHelicopter::DoExplosion( const Vector &vecOrigin, const Vector &vecVelocity )
{
	/*ExplosionCreate( GetAbsOrigin(), GetAbsAngles(), GetOwnerEntity() ? GetOwnerEntity() : this, sk_helicopter_grenadedamage.GetFloat(), 
		sk_helicopter_grenaderadius.GetFloat(), (SF_ENVEXPLOSION_NOSPARKS|SF_ENVEXPLOSION_NODLIGHTS|SF_ENVEXPLOSION_NODECAL|SF_ENVEXPLOSION_NOFIREBALL|SF_ENVEXPLOSION_NOPARTICLES), true,
	...
	if ( GetWaterLevel() != 0 && (GetWaterType() & CONTENTS_WATER) )
	{
		...
		{
			DispatchParticleEffect("water_splash_explosion", WorldSpaceCenter(), vec3_angle);
		}
		else
		{
			/*ExplosionCreate(GetAbsOrigin(), GetAbsAngles(), GetOwnerEntity() ? GetOwnerEntity() : this, sk_helicopter_grenadedamage.GetFloat(),
	...
	else
	{
		...
		if (cvar->FindVar("oc_particle_explosions")->GetInt())
		{
			//DispatchParticleEffect("Heli_Bomb", GetAbsOrigin(), vec3_angle);
		...
		CPASFilter filter(GetAbsOrigin());
		te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 155, 155, 0, 2, 240, 0.11, 0);//OverCharged
	}
