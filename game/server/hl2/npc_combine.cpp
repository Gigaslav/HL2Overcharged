...
DEFINE_KEYFIELD(m_disableShadow, FIELD_BOOLEAN, "DisableShadow"),
...
void CNPC_Combine::Spawn( void )
{
	...
	m_flNextGrenadeCheck	= gpGlobals->curtime + 0.3;//OverCharged//1.0
	...
	case TASK_RANGE_ATTACK1:
		{
			...
			//m_nShots = (GetActiveWeapon() != NULL) ? GetActiveWeapon()->GetRandomBurst() : 1;
			m_nShots = GetActiveWeapon()->GetRandomBurst();
			m_flShotDelay = GetActiveWeapon()->GetFireRate();

			if (FClassnameIs(GetActiveWeapon(), "weapon_combinesniper") || FClassnameIs(GetActiveWeapon(), "weapon_sniper"))
				m_flShotDelay += 3.5;
...
void CNPC_Combine::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (GetActiveWeapon() != NULL && GetActiveWeapon()->m_nSkin > 0 && m_disableShadow)
	{
		GetActiveWeapon()->m_nSkin = 0;
		GetActiveWeapon()->RemoveEffects(EF_NOSHADOW);
	}
...
int CNPC_Combine::SelectSchedule( void )
{
	...
	int Rndm = RandomInt(0,1);//OverCharged

	if (!cvar->FindVar("oc_npc_combines_allow_underbarrel_shot")->GetInt())
		Rndm = 0;
		...
		if ( m_flNextGrenadeCheck < gpGlobals->curtime )
		{
			...
			if ( IsElite() )
			{
				if ( FVisible( m_hForcedGrenadeTarget ) )
				{
					...
					if (Rndm)//OverCharged
						return SCHED_COMBINE_FORCED_GRENADE_THROW;//OverCharged
					else//OverCharged
			...
			{
				if (FVisible(m_hForcedGrenadeTarget))//OverCharged
				{
					m_vecAltFireTarget = vecTarget;
					m_hForcedGrenadeTarget = NULL;
					return SCHED_COMBINE_AR2_ALTFIRE;
				...
				return SCHED_COMBINE_AR2_ALTFIRE;//OverCharged
				}
			}
		}
...
void CNPC_Combine::HandleAnimEvent( animevent_t *pEvent )
{
	...
	{
//////////////////////////////////////////////////////////////////////////Firing smg grenade//////OverCharged//////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		else if ( pEvent->event == COMBINE_AE_ALTFIRE )
		{
			if( IsElite() )
			{
				...
				DelaySquadAltFireAttack( 4.0f );//10
	...
	else
	{
		...
		case COMBINE_AE_AIM:	
			{
				handledEvent = true;
				/*if (GetActiveWeapon()->m_iClassname == AllocPooledString("weapon_sniper"))
...
WeaponProficiency_t CNPC_Combine::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
	...
	else if (FClassnameIs(pWeapon, "weapon_combinesniper") || FClassnameIs(pWeapon, "weapon_sniper"))
	{
		m_disableShadow = true;
		AddEffects(EF_NOSHADOW);

		/*SetModelName(MAKE_STRING("models/Combine_Sniper_Camo.mdl"));
		PrecacheModel("models/Combine_Sniper_Camo.mdl");
		SetModel("models/Combine_Sniper_Camo.mdl");*/

		//IMaterial *pMaterial = materials->FindMaterial("effects/shaders/postproc_battery", TEXTURE_GROUP_CLIENT_EFFECTS);
		//IMaterialVar *var;
		if (g_iSkillLevel == SKILL_EASY || g_iSkillLevel == SKILL_MEDIUM)
		{
			if (m_nSkin != 2)
			{
				m_nSkin = 2;
