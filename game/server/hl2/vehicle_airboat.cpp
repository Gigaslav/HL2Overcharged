...
#include "OverCharged/Entity_WeaponDetach.h"
...
ConVar oc_airboat_checking_sphere_radius("oc_airboat_checking_sphere_radius", "23", FCVAR_ARCHIVE);
ConVar oc_airboat_has_gun("oc_airboat_has_gun", "0");
ConVar sk_airboat_max_ammo("sk_airboat_max_ammo", "100");
ConVar sk_airboat_recharge_rate("sk_airboat_recharge_rate", "15");
ConVar sk_airboat_drain_rate("sk_airboat_drain_rate", "10");
ConVar hud_airboathint_numentries("hud_airboathint_numentries", "10", FCVAR_NONE);
ConVar airboat_fatal_stress("airboat_fatal_stress", "5000", FCVAR_NONE, "Amount of stress in kg that would kill the airboat driver.");

extern ConVar autoaim_max_dist;

// Tree Trigger
/*class CAirboatEntity : public CBaseEntity
{
	DECLARE_CLASS(CAirboatEntity, CBaseEntity);
public:
	//void		Touch(CBaseEntity *pOther);
	Vector Min;
	Vector Max;
	Vector Pos;
	static CAirboatEntity *TriggerCreate(CBaseEntity *pOwner, const Vector &position);// , const Vector &min, const Vector &max);
};
LINK_ENTITY_TO_CLASS(airboat_entity, CAirboatEntity);

CAirboatEntity *CAirboatEntity::TriggerCreate(CBaseEntity *pOwner, const Vector &position)//, const Vector &min, const Vector &max)
{
	CAirboatEntity *pTrigger = CREATE_ENTITY(CAirboatEntity, "airboat_entity");
	...
	UTIL_SetSize(pTrigger, Vector(-13, -13, -13), Vector(13, 13, 13));
	NDebugOverlay::Box(position, Vector(-13, -13, -13), Vector(13, 13, 13), 0, 255, 0, true, 10.1f);
	//pTrigger->Min = min;
	//pTrigger->Max = max;
	//pTrigger->Pos = position;
	return pTrigger;
}*/
...
public:
	...
	CBasePlayer *pPlayer;
	/*CEntityWeaponDetach *pWDetachA = NULL;
	~CPropAirboat() 
	{ 
		if (pWDetachA)
		{
...
DEFINE_FIELD(HasTakenAirboatGun, FIELD_BOOLEAN),
DEFINE_FIELD(repeathasgun, FIELD_BOOLEAN),
//DEFINE_FIELD(pWDetachA, FIELD_EHANDLE),
...
{
	if (oc_airboat_has_gun.GetInt())
	{
		inputdata_t inputdata;
		inputdata.value.SetBool(true);
		InputEnableGun(inputdata);
	}

	/*if (pWDetachA == NULL)
	{
		pWDetachA = new CEntityWeaponDetach(this, "gun", Vector(-13, -13, -13), Vector(13, 13, 13));
		pWDetachA->SetOwnerEntity(this);
		pWDetachA->FollowEntity(this);
	...
	/*if (pWDetachA == NULL)
	{
		pWDetachA = pWDetachA->entCreate(this, GetAbsOrigin(), Vector(-13, -13, -13), Vector(13, 13, 13));// , Vector(-23, -23, -23), Vector(23, 23, 23));
		pWDetachA->SetParent(this);
	}*/

	/*int attachment1 = LookupAttachment("gun");
	Vector vecOrigin2;
	GetAttachment(attachment1, vecOrigin2);
	Vector triggerPosition, vForward, vRight, vUp;

	AngleVectors(GetAbsAngles(), &vForward, &vRight, &vUp);
	triggerPosition = GetAbsOrigin() + (vForward * 64) + (vUp * 64);
	m_pTrigger = CAirboatEntity::TriggerCreate(this, triggerPosition);// , Vector(-23, -23, -23), Vector(23, 23, 23));
	m_pTrigger->SetParent(this->GetDriver());
	UTIL_SetSize(m_pTrigger, Vector(-13, -13, -13), Vector(13, 13, 13));*/
...
ConVar oc_test_sphere_x("oc_test_sphere_x", "0");
ConVar oc_test_sphere_y("oc_test_sphere_y", "0");
ConVar oc_test_sphere_z("oc_test_sphere_z", "0");
...
void CPropAirboat::Think(void)
{
	...
	//if (m_bHasGun)
	{
		...
		{
			pWDetachA->SetOwnerEntity(this);
			pWDetachA->LegacyThink(this, "weapon_airboatgun", "gun", "Press 'use' button to mount back the cannon", "Press 'use' button to tear off the cannon", oc_airboat_checking_sphere_radius.GetFloat(), HasTakenAirboatGun);
		}
		else
		{
	...
	/*if (m_bAllowDetach)
		CheckAttachedGun("gun", oc_airboat_checking_sphere_radius.GetFloat(), true);*/

	//if (m_bAllowDetach && CheckAttachedGun("gun", oc_airboat_checking_sphere_radius.GetFloat(), true))
	//	GunText(oc_airboat_checking_sphere_radius.GetFloat(), m_bHasGun, "Press 'use' button to mount back the cannon", "Press 'use' button to pick up the cannon");

	// BJ: Goddamn localisation strings
	if (m_bAllowDetach && CheckAttachedGun("gun", oc_airboat_checking_sphere_radius.GetFloat(), true))
		GunText(oc_airboat_checking_sphere_radius.GetFloat(), m_bHasGun, "#OVR_MountAirboatGun", "#OVR_DismountAirboatGun");
...
void CPropAirboat::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (!CheckAttachedGun("gun", oc_airboat_checking_sphere_radius.GetFloat(), true))
	...
	if (m_bCanDetach)
	{
		...
		int attachment = LookupAttachment("gun");
		//int tau = LookupAttachment("taucannon");
		if ((attachment) && repeathasgun)
		{
			Vector vecOrigin, vecOrigin2;// = this->WorldSpaceCenter();
			QAngle angDir, angDir2;

			GetAttachment(attachment, vecOrigin, angDir);

			repeathasgun = true;
			if (/*pWDetachA != NULL && pWDetachA->canTakeOffTheGun && */(!HasTakenAirboatGun) && !pPlayer->HasNamedPlayerItem("weapon_airboatgun"))
			{
				pWeapon = pPlayer->GetActiveWeapon();
				HasTakenAirboatGun = true;
				pPlayer->Weapon_SetLast(pWeapon);
				pPlayer->GiveNamedItem("weapon_airboatgun");
				...
				//HasGunS = true;
				//pPlayer->Weapon_Switch((CBaseCombatWeapon*)pPlayer->GiveNamedItem("weapon_airboatgun"));
				HasGunS = false;
				return;
			}
			else if (/*((tr2.endpos.DistTo(vecStart) > 20) && (tr2.endpos.DistTo(vecStart) <= 58 ))*//*pWDetachA != NULL && pWDetachA->canTakeOffTheGun && */(HasTakenAirboatGun) && (FClassnameIs(pPlayer->GetActiveWeapon(), "weapon_airboatgun")))
			{
				if (cvar->FindVar("oc_weapon_holster")->GetInt())
				{
					cvar->FindVar("oc_weapon_holster")->SetValue(0);
					HasGunS = true;
				}
				...
				UTIL_Remove(pPlayer->GetActiveWeapon());
				//pPlayer->Weapon_SlotOccupied(pPlayer->GetActiveWeapon());
				//pPlayer->Weapon_GetLast();

				SetBodygroup(1, true);
				pPlayer->SwitchToNextBestWeapon(pPlayer->Weapon_GetLast());
				//if (FClassnameIs(pPlayer->Weapon_GetLast(), "weapon_airboatgun"))
				//pPlayer->SwitchToNextBestWeapon(pPlayer->Weapon_GetLast());
				/*else
					pPlayer->Weapon_Switch(pPlayer->Weapon_GetLast());*/

				//(pPlayer->Weapon_GetSlot(0));//random->RandomInt(1, 3)));
				...
				if (HasGunS)
					cvar->FindVar("oc_weapon_holster")->SetValue(1);
...
int CPropAirboat::GetTracerAttachment(void)
{
...
void CPropAirboat::DoMuzzleFlash(void)
{
	...
	else
	{
		...
		data.m_nAttachmentIndex = 1;
		data.m_nEntIndex = this->entindex();
		DispatchEffect("AirboatGunMuzzleFlashParticle", data);
	}
}
...
void CPropAirboat::FireGun()
{
	...
	*/
	...
	/*float Radius;// = 110;
	Radius = random->RandomInt(150, 250);
	CPASFilter filter(vecGunPosition);
	te->DynamicLight(filter, 0.0, &vecGunPosition, 0, random->RandomInt(95, 125), random->RandomInt(155, 195), 3, Radius, random->RandomInt(0.018, 0.23), Radius / 0.5);//OverCharged
	DispatchParticleEffect("abgun_normal2", PATTACH_POINT_FOLLOW, this, "muzzle", false);*/
	DoMuzzleFlash();
