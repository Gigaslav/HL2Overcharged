...
bool CNPC_BaseZombie::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	if ( hl2_episodic.GetBool() )
	{
		...
		if (  (info.GetAmmoType() == GetAmmoDef()->Index("CombineHeavyCannon")) && (info.GetDamageType() & DMG_BULLET) )//OverCharged
			return true;
	}
...
HeadcrabRelease_t CNPC_BaseZombie::ShouldReleaseHeadcrab( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	if ( m_iHealth <= 0 )
	{
		...
		{
			//if (oc_npc_zombies_dont_less_headcrab.GetBool())
			if (cvar->FindVar("oc_npc_zombies_dont_less_headcrab")->GetInt() == 1)
			{
				return RELEASE_NO;
			}
			else
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	else
	{
		...
		if( ShouldBecomeTorso( info, flDamageThreshold ) )
		{
			...
			{
				/*if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() == 0)
					BecomeRagdoll(infoKilled, info.GetDamageForce());*/
				...
				if ( hl2_episodic.GetBool() )
				{
					if  ((bHitByCombineCannon) && (info.GetDamageType() & DMG_BULLET) )//OverCharged
					{
						// Catch on fire.
...
void CNPC_BaseZombie::DieChopped( const CTakeDamageInfo &info )
{
	...
	/*if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() == 1)
		BecomeRagdoll(infoKilled, info.GetDamageForce());*/

	CBaseEntity *pLegGib = CreateRagGib(GetLegsModel(), GetAbsOrigin(), GetAbsAngles(), vecLegsForce, flFadeTime, ShouldIgniteZombieGib(), BloodColor());
	if ( pLegGib )
	{
	...
	{
		CBaseEntity *pTorsoGib = CreateRagGib(GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles, forceVector, flFadeTime, ShouldIgniteZombieGib(), BloodColor());
		//CBaseEntity *pTorsoGib = CreateRagGibBodygrouped(GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles, forceVector, flFadeTime, ShouldIgniteZombieGib(), ZOMBIE_BODYGROUP_HEADCRAB, m_fIsHeadless);
	...
	else
	{
		...
		if (FClassnameIs(this, "npc_fastzombie"))
			headless = m_fIsHeadless;
		else
			headless = !m_fIsHeadless;*/

		CBaseEntity *pTorsoGib = CreateRagGibBodygrouped(GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles, forceVector, flFadeTime, ShouldIgniteZombieGib(), ZOMBIE_BODYGROUP_HEADCRAB, !m_fIsHeadless, BloodColor());
...
void CNPC_BaseZombie::BecomeTorso( const Vector &vecTorsoForce, const Vector &vecLegsForce )
{
	...
	if ( m_fIsTorso == true )
	{
		...
		CBaseEntity *pGib = CreateRagGib(GetLegsModel(), GetAbsOrigin() - Vector(0, 0, 40), GetAbsAngles(), vecLegsForce, flFadeTime, false, BloodColor());
...
bool CNPC_BaseZombie::BecomeRagdoll( const CTakeDamageInfo &info, const Vector &forceVector )
{
	...
	//if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() == 1)
	{
		...
		else
		{
			...
			if (IsChopped(info) || IsSquashed(info))
				RemoveDeferred();
...
void CNPC_BaseZombie::ReleaseHeadcrab( const Vector &vecOrigin, const Vector &vecVelocity, bool fRemoveHead, bool fRagdollBody, bool fRagdollCrab )
{
	...
	if (fRagdollCrab)
	{
		...
		CBaseEntity *pGib = CreateRagGib(GetHeadcrabModel(), vecOrigin, GetLocalAngles(), vecVelocity, 15, ShouldIgniteZombieGib(), BloodColor());
	...
	if( fRagdollBody )
	{
		...
		{
			BecomeRagdoll(infoKilled, vec3_origin);
		}*/
	}
}
