...
ConVar	oc_intern_item_var("oc_intern_item_var", "0", FCVAR_HIDDEN);
class CItem_OverrideItemCrate : public CBaseEntity
{
public:
	DECLARE_CLASS(CItem_OverrideItemCrate, CBaseEntity);
	DECLARE_DATADESC();

	//void Precache(void){ BaseClass::Precache(); };
	void Spawn(void){ isOverride(); BaseClass::Spawn(); };

	bool _override;

	bool CItem_OverrideItemCrate::isOverride()
	{
		if (_override)
			oc_intern_item_var.SetValue(1);
		else
			oc_intern_item_var.SetValue(0);
		return _override;
	}
};
LINK_ENTITY_TO_CLASS(item_override_item_crate, CItem_OverrideItemCrate);
BEGIN_DATADESC(CItem_OverrideItemCrate)
DEFINE_KEYFIELD(_override, FIELD_BOOLEAN, "IsOverride"),
END_DATADESC()
...
{ 
	...
	oc_intern_item_var.SetValue(0);//Inclusive, and avaiable only for mapadd spawns
	DisableAutoFade();
...
void CItem_ItemCrate::OnBreak( const Vector &vecVelocity, const AngularImpulse &angImpulse, CBaseEntity *pBreaker )
{
	...
	if (oc_intern_item_var.GetBool())
	{
		m_strItemClass = AllocPooledString("item_dynamic_resupply");
	}
...
//		vecActualVelocity += vecVelocity;
		...
		pSpawn->Spawn();
		//DevMsg("pSpawn->GetEntityName(): %s \n", pSpawn->GetEntityName());
		...
		{
			//DevMsg("FClassnameIs( pSpawn, item_dynamic_resupply ): %s \n");
