...
ConVar	sk_gunship_health("sk_gunship_health", "500");
/*
...
BEGIN_DATADESC( CNPC_CombineGunship )
	...
	DEFINE_FIELD( DamageCount, FIELD_INTEGER ),
	...
	DEFINE_KEYFIELD(m_bAllowGroundAttack, FIELD_BOOLEAN, "AllowGroundAttack"),
...
void CNPC_CombineGunship::Spawn( void )
{
	...
	m_iHealth = sk_gunship_health.GetInt();
...
void CNPC_CombineGunship::PrescheduleThink( void )
{
	...
	else if( m_lifeState == LIFE_DYING )
	{
		...
		if ( bCreateExplosion )
		{
			...
			ExplosionCreate( explodePoint, QAngle(0,0,1), this, 100, 128, false, true );
		}
...
void CNPC_CombineGunship::FireCannonRound( void )
{
	...
	else
	{
		...
		info.m_iPlayerDamage = pAmmoDef->NPCDamage(m_iAmmoType);
...
void CNPC_CombineGunship::SelfDestruct( void )
{
	...
	for( int i = 0 ; i < 6 ; i++ )
	{
		...
		ExplosionCreate( vecOrigin + vecDelta, QAngle( -90, 0, 0 ), this, 10, 10, false, true );
	}
...
{
	// Reflect bullets
	if (info.GetDamageType() & DMG_BULLET || info.GetDamageType() & DMG_GAUSS || info.GetDamageType() & DMG_ENERGYBEAM)
	{
...
int	CNPC_CombineGunship::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	/*if ((inputInfo.GetDamageType() & DMG_AIRBOAT))
	{
		CTakeDamageInfo info = inputInfo;
		info.SetDamage((GetMaxHealth() / 10) + 1);
	...
	if ( m_bInvulnerable == false )
	{
		...
		if ((inputInfo.GetDamageType() & DMG_AIRBOAT || inputInfo.GetDamageType() & DMG_DISSOLVE || inputInfo.GetDamageType() & DMG_SHOCK || inputInfo.GetDamageType() & DMG_GAUSS || inputInfo.GetDamageType() & DMG_PHYSGUN))
			iHealthIncrements = 40;
		else
		    iHealthIncrements = sk_gunship_health_increments.GetInt();
	...
	if (inputInfo.GetDamageType() == DMG_BULLET || inputInfo.GetDamageType() == DMG_GAUSS || info.GetDamageType() & DMG_ENERGYBEAM)
	{
		m_iHealth -= info.GetDamage()/3;
		DamageCount += info.GetDamage() / 3;
	}
	else
	{
		m_iHealth -= info.GetDamage();
		DamageCount += info.GetDamage();
	}
