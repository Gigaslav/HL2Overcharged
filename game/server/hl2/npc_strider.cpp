...
//#endif // HL2_EPISODIC
	...
	DEFINE_FIELD(hurtDamage, FIELD_TIME),
	DEFINE_FIELD(MaxHealth, FIELD_TIME),
...
void CNPC_Strider::Spawn()
{
	...
	MaxHealth = m_iHealth / 5.f;
	hurtDamage = MaxHealth;
}
...
void CNPC_Strider::HandleAnimEvent( animevent_t *pEvent )
{
	...
	case STRIDER_AE_CANNONHIT:
		CreateConcussiveBlast( m_blastHit, m_blastNormal, this, 1.f );
		break;
...
int CNPC_Strider::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	//if ((info.GetDamageType() & DMG_BLAST) || (info.GetDamageType() & DMG_AIRBOAT | DMG_DISSOLVE | DMG_SHOCK | DMG_GAUSS | DMG_PHYSGUN))// && info.GetMaxDamage() > 50 )
	{
		...
		if ( dist < 200 )
		{
			...
			//if( bPlayer )
			{
				if (info.GetDamageType() & (DMG_DISSOLVE | DMG_DISSOLVE_EGON))
					damage = info.GetDamage() / 1.5f;
				else
...
void CNPC_Strider::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( IsUsingAggressiveBehavior() )
	{
		...
		ExplosionCreate( vecExplode, QAngle(0,0,1), this, 100, 128, false, false );
	}
