...
//#include "overcharged/singleplayer_animstate.h"		// Overcharged: Player anims
...
public:
	...
	virtual void		SetPlayerAnimation(PLAYER_ANIM playerAnim);
	...
	void				SetBodyRenderingState(bool HEVSuit);
	void				SetAdmireGlovesVisible(bool visible);
	void				SetHandsVisible(bool visible);
	void				SetRightLegVisible(bool visible);
	...
	//CBaseFlex			*pPlayerModel;

	float				m_flPlayerTimeNextAnim;
	// from CBasePlayer
	...
	void StartZooming(bool ironsighted = false, float fov = 25);
	void StopZooming( bool ironsighted = false );
	bool IsZooming( void );
	...
	void  KickEvent(void);
	void  KickEventHandle(void);
	void  EventHit(bool swing = false);
	void  WeaponSwing(void);
	void  WeaponSwingHandle(bool stop = false);
	void  WeaponSwingHit(void);
	int   KickAnim;
	bool  Kicking;
	bool  Kicked;
	bool  Swinged;
	bool  Swing;
	float m_flKickEventAnimTime;
	...
	void CheckLean();			// Light Kill : Lean functions
	void StartLeaning();
	void StopLeaning();
...
//	CSinglePlayerAnimState *m_pPlayerAnimState;		// Overcharged: Player anims
//	QAngle m_angEyeAngles;

protected:	// Jeep: Portal_Player needs access to this variable to overload PlayerUse for picking up objects through portals
	bool				m_bPlayUseDenySound;		// Signaled by PlayerUse, but can be unset by HL2 ladder code...

	bool            m_bIsLeaning;	// BriJee OVR: Server side leaning
private:
	...
	float				timeholster;	// BriJee OVR: Grenade throw 
	float				timethrow;
	float				timedeploy;
	bool				WantThrow;
	bool				bWpnFireBlock;
