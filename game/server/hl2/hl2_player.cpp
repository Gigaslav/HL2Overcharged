...
#include "grenade_frag.h"	// BriJee OVR: Grenade types for fast throw
#include "overcharged/grenade_flash.h"
#include "overcharged/grenade_smoke.h"
...
ConVar oc_player_allow_kick("oc_player_allow_kick", "0", FCVAR_ARCHIVE, "Player allow kick");
ConVar oc_player_allow_kick_doors("oc_player_allow_kick_doors", "0", FCVAR_ARCHIVE, "Player allow kick doors");
ConVar oc_player_allow_swing("oc_player_allow_swing", "0", FCVAR_ARCHIVE, "Player allow swing");
ConVar oc_player_swing_range("oc_player_swing_range", "85", FCVAR_ARCHIVE);
...
BEGIN_DATADESC( CHL2_Player )
	DEFINE_FIELD(heavyWeight, FIELD_BOOLEAN),
	DEFINE_FIELD( m_nControlClass, FIELD_INTEGER ),
	DEFINE_EMBEDDED( m_HL2Local ),

	DEFINE_FIELD(bg_rh, FIELD_INTEGER),
	DEFINE_FIELD(bg_lh, FIELD_INTEGER),
	DEFINE_FIELD(bg_rl, FIELD_INTEGER),
	DEFINE_FIELD(bg_rh_val, FIELD_INTEGER),
	DEFINE_FIELD(bg_lh_val, FIELD_INTEGER),
	DEFINE_FIELD(bg_rl_val, FIELD_INTEGER),
	...
	DEFINE_FIELD( m_flPlayerTimeNextAnim, FIELD_TIME ),
	/*
	...
	*/
	...
	DEFINE_FIELD( KickAnim, FIELD_INTEGER ),
	DEFINE_FIELD( Kicking, FIELD_BOOLEAN ),
	DEFINE_FIELD( Kicked, FIELD_BOOLEAN ),
	DEFINE_FIELD( Swinged, FIELD_BOOLEAN ),
	DEFINE_FIELD( Swing, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_flKickEventAnimTime, FIELD_TIME ),
...
CHL2_Player::CHL2_Player()
{
	...
	heavyWeight = false;
	m_flPlayerTimeNextAnim = gpGlobals->curtime;
...
void CHL2_Player::Precache( void )
{
	...
	PrecacheModel(cvar->FindVar("oc_playermodel_suit")->GetString());//("models/player.mdl");	//old model
	PrecacheModel(cvar->FindVar("oc_playermodel_nosuit")->GetString());
	...
	//BriJee OVR: Precache custom things
	PrecacheScriptSound("HL2Player.NightvisionOn");
	PrecacheScriptSound("HL2Player.NightvisionOff");

	PrecacheScriptSound("HL2Player.DeployWeapon");
	//PrecacheScriptSound("HL2Player.HolsterWeapon");

	PrecacheScriptSound("HL2Player.IronsightOn");
	PrecacheScriptSound("HL2Player.IronsightOff");

	PrecacheScriptSound("WeaponSwing.HitWorld");	// BriJee OVR: Weapon swing sounds
	PrecacheScriptSound("WeaponSwing.Miss");
	PrecacheScriptSound("WeaponSwing.HitAlly");
	PrecacheScriptSound("WeaponSwing.HitEnemy");

	PrecacheModel("models/player/v_kick.mdl");
	PrecacheModel("models/player/v_kick_nosuit.mdl");
	PrecacheScriptSound("Player.KickAir");
	PrecacheScriptSound("Player.KickSolid");
	PrecacheScriptSound("Player.KickBody");

	PrecacheScriptSound(cvar->FindVar("oc_player_death_sound_name")->GetString());		// BriJee OVR: Player death music name
}
...
{
	if (IsSuitEquipped() && !m_HL2Local.m_bIsIronsighted)
	{
...
void CHL2_Player::WeaponSwing(void)		// BriJee OVR: Weapon Swing Addition
{
	if (IsOnLadder())// || 
		return;
	...
	if (pWeapon->m_flNextSilencer > gpGlobals->curtime)
		return;

	if (pWeapon->isPlayingLoopingSound())
		pWeapon->DestroyLoopSound();
	...
	if (cvar->FindVar("oc_state_InSecondFire")->GetInt() != 0 ||
		...
		this->IsInAVehicle() ||
		this->IsOnLadder() ||
		!pWeapon->GetWpnData().SwingHitTime
		|| pWeapon->GetWpnData().animData[pWeapon->m_bFireMode].SwingAnim == ACT_RESET)
	return;
	...
	Activity swingAnim = pWeapon->GetWpnData().animData[pWeapon->m_bFireMode].SwingAnim != ACT_RESET ?
		pWeapon->GetWpnData().animData[pWeapon->m_bFireMode].SwingAnim :
		ACT_VM_MELEE;

	pWeapon->SendWeaponAnim(swingAnim);
	
	pWeapon->m_flTimeWeaponIdle =
		pWeapon->m_flNextInspectAnimation =
		pWeapon->m_flNextLaserSwitchTime =
		pWeapon->m_flNextSilencer =
		pWeapon->m_flNextShotgunReload =
		pWeapon->m_flNextSwitchTime = 
	...
	float seqDuration = pWeapon->SequenceDuration();

	float checkTime = seqDuration * pWeapon->GetWpnData().SwingHitTime;

	m_flSwingEventAnimTime = gpGlobals->curtime + checkTime;// pWeapon->SequenceDuration();
...
void CHL2_Player::WeaponSwingHandle(bool stop)
{
	CBaseCombatWeapon *pWeapon = GetActiveWeapon();
	...
	if (stop || !vm || !pWeapon)
	{
		m_flSwingEventAnimTime = 0.f;// gpGlobals->curtime;
		Swinged = false;
		Swing = Swinged;
	}
	if (Swinged)
	{
		//float checkTime = pWeapon->GetWpnData().SwingHitTime;
		if ((m_flSwingEventAnimTime < gpGlobals->curtime/* + checkTime*/) && Swing)
		{
			WeaponSwingHit();
			Swing = false;
		}

		if (m_flSwingEventAnimTime < gpGlobals->curtime)
		{
			Swinged = false;
			Swing = Swinged;
...
void CHL2_Player::WeaponSwingHit(void)
{
	...
	if (!pWeapon || pPlayer->IsInAVehicle())
		return;
	...
	vecStop = vecStart + vecDir * oc_player_swing_range.GetInt();

	UTIL_TraceHull(vecStart, vecStop, Vector(-3, -3, -3), Vector(3, 3, 3), MASK_SHOT, pPlayer, COLLISION_GROUP_NONE, &tr);

	Disposition_t RelShip = pPlayer->IRelationType(tr.m_pEnt);	// Relationships check.

	if (!tr.m_pEnt || (tr.surface.flags & SURF_SKY))	// Check all surfaces...
	{
		EmitSound("WeaponSwing.Miss");
		pPlayer->ViewPunch(QAngle(-2, 5, random->RandomFloat(-1, 2)));
	}
	else if (tr.m_pEnt->IsNPC())		// We see npc ? lets hit him.
	{
		//MikeD
		//WTF is this, BriJee?
		/*if (pWeapon->IsBehindTarget(tr.m_pEnt))
		{
			CBaseEntity *pHurt = pPlayer->CheckTraceHullAttack(pPlayer->Weapon_ShootPosition(), vecEnd,
				Vector(-1, -1, -1), Vector(0, 0, 0), cvar->FindVar("oc_weapon_swing_backstab_dmg")->GetFloat(), DMG_CLUB, 0.75);

			if (pHurt)
			{
				EmitSound("WeaponSwing.HitEnemy");

				UTIL_ScreenShake(tr.endpos, 1.0, 50.0, 0.45, 150.0, SHAKE_START);
			}
			else
			{
				if (RelShip == D_LI || D_NU)
					EmitSound("WeaponSwing.HitAlly");
				else
					EmitSound("WeaponSwing.Miss");
			}
		}
		...
		{
			if (pWeapon && FClassnameIs(pWeapon, "weapon_pistol") || FClassnameIs(pWeapon, "weapon_glock")
				|| FClassnameIs(pWeapon, "weapon_beretta") || FClassnameIs(pWeapon, "weapon_pistol_silent")	//fx
				|| FClassnameIs(pWeapon, "weapon_dualpistol") || FClassnameIs(pWeapon, "weapon_laserpistol")
				|| FClassnameIs(pWeapon, "weapon_alyxgun") || FClassnameIs(pWeapon, "weapon_357")
				|| FClassnameIs(pWeapon, "weapon_flaregun") || FClassnameIs(pWeapon, "weapon_deagle"))
			{
				CBaseEntity *pHurt = pPlayer->CheckTraceHullAttack(pPlayer->Weapon_ShootPosition(), vecEnd,
					Vector(-1, -1, -1), Vector(12, 12, 12), cvar->FindVar("oc_weapon_swing_dmg_pistols")->GetFloat(), DMG_CLUB, 0.75);
			...
			else if (pWeapon && FClassnameIs(pWeapon, "weapon_smg1") || FClassnameIs(pWeapon, "weapon_mp5")
				|| FClassnameIs(pWeapon, "weapon_ak47") || FClassnameIs(pWeapon, "weapon_m4")	//fx
				|| FClassnameIs(pWeapon, "weapon_sg552") || FClassnameIs(pWeapon, "weapon_oicw")
				|| FClassnameIs(pWeapon, "weapon_ar2") || FClassnameIs(pWeapon, "weapon_machinegun"))	//machinegun addition
			{
				CBaseEntity *pHurt = pPlayer->CheckTraceHullAttack(pPlayer->Weapon_ShootPosition(), vecEnd,
					Vector(-1, -1, -1), Vector(12, 12, 12), cvar->FindVar("oc_weapon_swing_dmg_machineguns")->GetFloat(), DMG_CLUB, 0.75);
			...
			{
				CBaseEntity *pHurt = pPlayer->CheckTraceHullAttack(pPlayer->Weapon_ShootPosition(), vecEnd,
					Vector(-1, -1, -1), Vector(12, 12, 12), cvar->FindVar("oc_weapon_swing_dmg_other")->GetFloat(), DMG_CLUB, 0.75);
		...
		//MikeD
		//This is how it must works
		{
			//Get eye-orient forward vector
			Vector hitDirection;
			pPlayer->EyeVectors(&hitDirection, NULL, NULL);
			VectorNormalize(hitDirection);

			//Create our Takedmg obj
			CTakeDamageInfo info(pPlayer, pPlayer, pWeapon->GetWpnData().SwingDamage, DMG_CLUB);
			...
			//Play camera smooth snap
			pPlayer->ViewPunch(QAngle(-8, random->RandomFloat(-2, 2), -3));

			//Scale dmg relative to skill level
			info.AdjustPlayerDamageInflictedForSkillLevel();

			//Calc dmg force
			CalculateMeleeDamageForce(&info, hitDirection, tr.endpos);

			//Dispatch dmg to entity
			tr.m_pEnt->DispatchTraceAttack(info, hitDirection, &tr);

			ApplyMultiDamage();

			//Check glass triggers
			TraceAttackToTriggers(info, tr.startpos, tr.endpos, hitDirection);
	...
	else								// Alright its non npc, but hit it anyway.
	{
		CBaseEntity *pHurt = pPlayer->CheckTraceHullAttack(pPlayer->Weapon_ShootPosition(), vecEnd,
			Vector(-1, -1, -1), Vector(0, 0, 0), 25, DMG_CLUB, 0.75);

		if (pHurt)
		{
			// All works right, nothing here
		}
		else
		{
			// Need new prop hit checker
		}

		// BriJee : Server Side Ragdolls & Objects? min 1 damage to get work right
		RadiusDamage(CTakeDamageInfo(this, this, pWeapon->GetWpnData().SwingDamage, DMG_CLUB), tr.endpos, 20.0f, CLASS_PLAYER, NULL);
		...
		UTIL_ScreenShake(tr.endpos, 1.0, 50.0, 0.35, 150.0, SHAKE_START);
		pPlayer->ViewPunch(QAngle(-4, random->RandomFloat(-2, 2), 1));
		EmitSound("WeaponSwing.HitWorld");
	}
}
ConVar oc_test_weapon_weight("oc_test_weapon_weight", "0");
// void CHL2_Player::ThirdpersonShootPosition(void)		// DEFINE LATER!!!
//======================= BriJee OVR: END
...
void CHL2_Player::PostThink( void )
{
	...
	if (!GetActiveWeapon() && m_flAdmireGlovesAnimTime < gpGlobals->curtime)
	{
		CBaseAnimating *pAnimating = this->GetBaseAnimating();
		if (pAnimating)
		{
			if (pAnimating->GetBodygroup(bg_rh) || pAnimating->GetBodygroup(bg_lh))//1 - invisible
	...
	if (!g_fGameOver && !IsPlayerLockedInPlace() && IsAlive())
	{
		if (oc_player_allow_swing.GetBool())
		{
			if ((m_afButtonPressed & IN_SWING) && !Swinged && m_flSwingEventAnimTime < gpGlobals->curtime)
			{
				WeaponSwing();	// Wpn swing
			}
			if (Swinged && !IsInAVehicle())
			{
				WeaponSwingHandle();
			}
			else if (Swinged && IsInAVehicle())
			{
				WeaponSwingHandle(true);
			}
		}

		if (oc_player_allow_kick.GetBool())
		{
			if (IsSuitEquipped() && /*GetActiveWeapon() && */!IsRunning() && !(GetFlags() & FL_DUCKING))
			{
				if ((m_afButtonPressed & IN_KICK) && !IsInAVehicle())
				{
					QAngle AnglesEyes = EyeAngles();

					if (AnglesEyes.x <= 60.0f || AnglesEyes.x >= -75.0f)
					{
						CBaseViewModel *vm = GetViewModel(1);
						if (vm)
						{
							if (m_flKickEventAnimTime < gpGlobals->curtime + vm->SequenceDuration()*0.5f)
								KickEvent();
						}
						else
							KickEvent();
				...
				if (Kicking && !IsInAVehicle())
				{
					KickEventHandle();
				}
				else if (Kicking && IsInAVehicle())
				{
					CBaseViewModel *vm = GetViewModel(1);
					if (vm)
					{
						Kicking = false;
						Kicked = Kicking;
						m_flKickEventAnimTime = 0.0f;
						vm->SetWeaponModel(NULL, NULL);
						UTIL_RemoveImmediate(vm);
					}
				}
			}
		}
...
void CHL2_Player::KickEvent(void)
{
	MDLCACHE_CRITICAL_SECTION();
	CBaseViewModel *vm = GetViewModel(1);

	const char *suitModel = "models/player/v_kick.mdl";
	const char *nonSuitModel = "models/player/v_kick_nosuit.mdl";

	if (!vm)
	{
		CreateViewModel(1, true);
		vm = GetViewModel(1);
		vm->m_bIsLeg = true;
	}
	if (vm)
	{
		SetRightLegVisible(false);

		if (IsSuitEquipped())
		{
			vm->SetWeaponModel(suitModel, NULL);
		}
		else
		{
			vm->SetWeaponModel(nonSuitModel, NULL);
		}

		KickAnim = (int)ACT_VM_PRIMARYATTACK;//random->RandomInt(0, 1) == 0 ? (int)ACT_VM_PRIMARYATTACK : (int)ACT_VM_SECONDARYATTACK;

		int	idealSequence = vm->SelectWeightedSequence((Activity)KickAnim);

		//DevMsg("idealSequence: %i\n", idealSequence);
		...
		if (!vm->IsActivityFinished())
		{
			vm->StopAnimation();
			m_flKickEventAnimTime = 0.0f;
			vm->ResetSequence(KickAnim);
		}

		ShowViewModel(Kicking);

		EmitSound("Player.KickAir");

		if (idealSequence >= 0)
		{
			vm->SendViewModelMatchingSequence(idealSequence);
			m_flKickEventAnimTime = gpGlobals->curtime + vm->SequenceDuration(idealSequence);
		}
		/*Vector origin = vm->GetViewOffset();
		origin.z -= 150;
		vm->SetViewOffset(origin);*/
		//vm->SetRenderMode();
		/*Vector origin = vm->GetAbsOrigin();
		origin.z -= 150;
		vm->SetAbsOrigin(origin);*/
	}
}

ConVar oc_player_kick_damage("oc_player_kick_damage", "40", FCVAR_ARCHIVE);
ConVar oc_player_kick_range("oc_player_kick_range", "85", FCVAR_ARCHIVE);
ConVar oc_player_kick_rate("oc_player_kick_rate", "0.85", FCVAR_ARCHIVE);
void CHL2_Player::KickEventHandle(void)
{
	CBaseViewModel *vm = GetViewModel(1);
	if (vm)
	{
		if (Kicking)
		{
			if (m_flKickEventAnimTime < gpGlobals->curtime + (vm->SequenceDuration()*oc_player_kick_rate.GetFloat()) && !Kicked)
			{
				Kicked = true;

				float vel = 80.f;
				Vector recoilForce = GetAbsVelocity() - BaseClass::GetAutoaimVector(0) * vel;
				SetAbsVelocity(recoilForce);

				EventHit();
			}

			if (m_flKickEventAnimTime > gpGlobals->curtime)
			{
				vm->m_flPlaybackRate = 1.0f;
				vm->StudioFrameAdvance();
			}
			else if (m_flKickEventAnimTime < gpGlobals->curtime)
			{
				Kicking = false;
				Kicked = Kicking;
				m_flKickEventAnimTime = 0.0f;
...
void CHL2_Player::EventHit(bool swing)
{
	//ViewPunch(QAngle(-8, random->RandomFloat(-2, 2), -3));
	CBasePlayer *pPlayer = ToBasePlayer(this);
	Vector vecAiming = pPlayer->GetAutoaimVector(AUTOAIM_SCALE_DEFAULT);
	trace_t tr;
	Vector vecStart = pPlayer->EyePosition();
	Vector vecStop = vecStart + vecAiming * oc_player_kick_range.GetFloat();
	UTIL_TraceHull(vecStart, vecStop, Vector(-3,-3,-3), Vector(3,3,3), MASK_SHOT, pPlayer, COLLISION_GROUP_NONE, &tr);

	UTIL_ScreenShake(tr.endpos, 1.0, 50.0, 0.35, 150.0, SHAKE_START);
	pPlayer->ViewPunch(QAngle(-4, 5, random->RandomFloat(-1, 2)));
	//pPlayer->ViewPunch(QAngle(-2, 5, random->RandomFloat(-1, 2)));
	...
	{
		if (pHitEntity->IsNPC())
			EmitSound("Player.KickBody");
		else
			EmitSound("Player.KickSolid");

		Vector hitDirection;
		pPlayer->EyeVectors(&hitDirection, NULL, NULL);
		VectorNormalize(hitDirection);

		CTakeDamageInfo info(pPlayer, pPlayer, oc_player_kick_damage.GetFloat(), DMG_CLUB);
		info.SetAttacker(pPlayer);
		...
		if (oc_player_allow_kick_doors.GetBool())
		{
			if (FClassnameIs(pHitEntity, "prop_door") || FClassnameIs(pHitEntity, "prop_door_rotating"))
			{
				CBasePropDoor *pDoor = (CBasePropDoor*)pHitEntity;
				if (pDoor)
				{
					if (oc_player_allow_kick_doors.GetInt() == 2)
					{
						pDoor->Kick(this, this, USE_TOGGLE, 0.f);
					}
					else
					{
						...
						CalculateMeleeDamageForce(&info, normal, tr.endpos);

						info.SetDamageForce(normal);
						pDoor->TakeKick(info);
					}
				}
			}
		...
		if (pPhysics != NULL)
		{
		Vector vel = vecStart + vecStop;
		AngularImpulse	angImp(15, 15, 15);
		...
		{
			pHitEntity->DispatchTraceAttack(info, hitDirection, &tr);
			ApplyMultiDamage();
...
void CHL2_Player::SetPlayerAnimation(PLAYER_ANIM playerAnim)
{
	int animDesired;
	...
	else if (playerAnim == PLAYER_IDLE || playerAnim == PLAYER_WALK)
	{
		...
		else
		{
			...
			{
				if ((GetWaterLevel() > 1) ? speed > 15 : speed > 0)
				{
					if (hasWeapons)
						idealActivity = ACT_HL2MP_WALK_CROUCH;
					else
	...
	else
	{
		...
		animDesired = this->m_hUseEntity ? -1 : SelectWeightedSequence(Weapon_TranslateActivity(idealActivity));
...
//#define PLAYER_MODEL "models/player/gordon_hev_body_legs.mdl"
//-----------------------------------------------------------------------------
...
void CHL2_Player::Spawn(void)
{
	...
	SetBodyRenderingState(IsSuitEquipped());
...
//#endif 
	...
	//=============================== Overcharged Reset player state here
	cvar->FindVar("oc_state_InSecondFire_sniper")->SetValue(0);
...
{
	bool notInSilencering = GetActiveWeapon() ? GetActiveWeapon()->m_flNextSilencer < gpGlobals->curtime : true;
	bool notInSwitch = GetActiveWeapon() ? GetActiveWeapon()->m_flNextSwitchTime < gpGlobals->curtime : true;
	//bool notInInspect = GetActiveWeapon() ? GetActiveWeapon()->m_flNextInspectAnimation < gpGlobals->curtime : true;
	//bool notInReload = GetActiveWeapon() ? !GetActiveWeapon()->m_bInReload && !GetActiveWeapon()->m_bReloadComplete && !GetActiveWeapon()->m_iShotgunReloadState : true;

	return ( m_bSprintEnabled &&										// Only if sprint is enabled 
			!IsWalking() &&	
			notInSilencering &&
			notInSwitch &&
			//!cvar->FindVar("oc_state_InSecondFire")->GetInt() && 
			//!cvar->FindVar("oc_state_IRsight_on")->GetInt() &&
...
void CHL2_Player::StartSprinting( void )
{
	...
	/*if (GetActiveWeapon())
	{*/
		/*float massKoefficient = 0.f;
		if (GetActiveWeapon()->GetWpnData().weaponMassKoefficient && oc_test_weapon_weight.GetFloat() == 0)
			massKoefficient = GetActiveWeapon()->GetWpnData().weaponMassKoefficient;
		else if (!GetActiveWeapon()->GetWpnData().weaponMassKoefficient)
			massKoefficient = 1.f;
		else if (oc_test_weapon_weight.GetFloat() != 0.f)
			massKoefficient = oc_test_weapon_weight.GetFloat();

		SetMaxSpeed(HL2_SPRINT_SPEED * massKoefficient);*/
		/*if (GetActiveWeapon()->GetWpnData().PlayerDisableSprint)
			EnableSprint(!GetActiveWeapon()->GetWpnData().PlayerDisableSprint);
		else
			EnableSprint(!GetActiveWeapon()->GetWpnData().PlayerDisableSprint);

		if (GetActiveWeapon()->GetWpnData().PlayerSpeedRunKoefficient)
			SetMaxSpeed(HL2_SPRINT_SPEED * GetActiveWeapon()->GetWpnData().PlayerSpeedRunKoefficient);
		else
...
void CHL2_Player::StopSprinting( void )
{
	...
	if( IsSuitEquipped() )
	{
		/*if (oc_test_weapon_weight.GetFloat() == 0)
		{
			if (GetActiveWeapon() && GetActiveWeapon()->GetWpnData().PlayerSpeedWalkKoefficient)
				SetMaxSpeed(HL2_NORM_SPEED * GetActiveWeapon()->GetWpnData().PlayerSpeedWalkKoefficient);
			else
				SetMaxSpeed(HL2_NORM_SPEED);
		}
		else
		{
			SetMaxSpeed(HL2_NORM_SPEED * oc_test_weapon_weight.GetFloat());
		}*/
...
//-----------------------------------------------------------------------------
void CHL2_Player::StartZooming(bool ironsighted, float fov)
{
	if (SetFOV(this, fov, 0.4f))
	{
		m_HL2Local.m_bZooming = true;
		m_HL2Local.m_bIsIronsighted = ironsighted;
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHL2_Player::StopZooming(bool ironsighted)
{
	...
	if ( SetFOV( this, iFOV, 0.2f ) )
	{
		...
		m_HL2Local.m_bIsIronsighted = false;
	}
}
...
{
	// Clears the animation state. // Overcharged: Player anims
...
bool CHL2_Player::ApplyBattery( float powerMultiplier )
{
	...
	if ((ArmorValue() < MAX_NORMAL_BATTERY) && IsSuitEquipped())
	{
		...
		CanDropGarbage = true;
		garbage.AddToHead(MAKE_STRING("item_battery"));
		garbageIndex.AddToHead(gpGlobals->curtime + cvar->FindVar("oc_player_garbage_drop_delay")->GetFloat());
		...
		EntityMessageBegin(this);
			WRITE_BYTE(PLAYER_MSG_DO_BATTERY);
		MessageEnd();
...
int	CHL2_Player::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if (cvar->FindVar("oc_player_nofalldamage")->GetInt() == 1)		// L1ght 15 : Let's do this Portal style ;D
	{
	...
	if( info.GetDamageType() & DMG_BLAST_SURFACE )
	{
		if( GetWaterLevel() > 2 )
		{
			...
			if (info.GetInflictor() && info.GetInflictor()->GetWaterLevel() == 0)
			{
				return 0;
			}
		}
	...
	if (info.GetDamageType() == DMG_GENERIC && info.GetAttacker() && info.GetInflictor() && info.GetAttacker() == this && info.GetInflictor() == this)
	{
...
void CHL2_Player::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (cvar->FindVar("oc_player_death_sound")->GetInt() == 1)	// BriJee OVR : Play death music
	{
		EmitSound(cvar->FindVar("oc_player_death_sound_name")->GetString());
	}
...
{
	if (cvar->FindVar("oc_allow_pick_stunbaton")->GetInt() == 0 || HasNamedPlayerItem("weapon_stunbaton"))	// BriJee OVR: Allow to pick stunstick, next turn into battery
	{// обратная логика функции
#ifndef HL2MP	
	...
	else if (cvar->FindVar("oc_allow_pick_stunbaton")->GetInt() == 1 && !HasNamedPlayerItem("weapon_stunbaton"))
	{
		if (pWeapon->ClassMatches("weapon_stunstick"))
		{
	...
	if (cvar->FindVar("oc_allow_pick_dual_pistols")->GetInt() == 1)		// BriJee OVR: Allow to pick double pistols //MideD: Very bad thing...
	{
		if (pWeapon->ClassMatches("weapon_pistol") 
			&& HasNamedPlayerItem("weapon_pistol") 
			&& !HasNamedPlayerItem("weapon_dual_pistols") )
			//&& (pWeapon->GetDefaultClip1() - pWeapon->GetMaxClip1() >= pWeapon->GetPrimaryAmmoCount()))
		{
			int clip1 = pWeapon->Clip1();
			int clip2 = pWeapon->Clip2();

			int PrimaryAmmoCount = pWeapon->GetPrimaryAmmoCount();
			int SecondaryAmmoCount = pWeapon->GetSecondaryAmmoCount();

			UTIL_Remove(pWeapon);
			GiveNamedItem("weapon_dual_pistols");

			CBaseCombatWeapon *pDPistols = GetWeapon("weapon_dual_pistols");
			if (pDPistols)
			{
				CBaseCombatWeapon *pPistol = GetWeapon("weapon_pistol");
				if (pPistol)
				{
					pDPistols->m_iClip1 = Clamp(clip1  /*+ clip1-1*/, 0, pDPistols->GetDefaultClip1());
					pDPistols->m_iClip2 = Clamp(pPistol->Clip2() + clip2, 0, pDPistols->GetDefaultClip2());
					pDPistols->SetPrimaryAmmoCount(PrimaryAmmoCount);
					pDPistols->SetSecondaryAmmoCount(SecondaryAmmoCount);
				}
			}
		}
		/*else if (pWeapon->ClassMatches("weapon_pistol")
			&& (HasNamedPlayerItem("weapon_pistol")
			|| HasNamedPlayerItem("weapon_dual_pistols"))
			&& (pWeapon->GetDefaultClip1() - pWeapon->GetMaxClip1() >= pWeapon->GetPrimaryAmmoCount()))
		{
...
//if (cvar->FindVar("oc_single_primary_weapon_mode")->GetInt() == 1)
{
...
//	if (cvar->FindVar("oc_single_primary_weapon_mode")->GetInt() == 1)
	...
	{
#else
		//BaseClass::AddGrenadeToSlot(pWeapon);
		if (cvar->FindVar("oc_player_allow_fast_gren_throw")->GetInt())
			UpdateGrenadeInfo();

		/*for (int i = 0; i < m_hMyWeapons.Count(); i++)
		{
			if (m_hMyWeapons[i].Get()->GetWpnData().iSlot == pWeapon->GetWpnData().iSlot)
			{
				if (m_hMyWeapons[i].Get()->GetWpnData().iPosition == pWeapon->GetWpnData().iPosition)
				{
					if (m_hMyWeapons[i].Get()->GetWpnData().m_bReservedSlot)
						return false;
				}
			}
		}*/
		...
		//if (cvar->FindVar("oc_single_primary_weapon_mode")->GetInt() == 1)
		{
			...
			if (m_afButtonPressed & IN_USE)
			{
				...
				if ((pWeapon != NULL) && (Weapon_CanSwitchTo(pWeapon)))
				{
					...
					pWeapon->Deploy();		// BriJee OVR: Added. This will prevent 'empty hands' on few guns with long animations.
...
void CHL2_Player::UpdateWeaponPosture( void )
{
	...
	if ( pWeapon && m_LowerWeaponTimer.Expired() && pWeapon->CanLower() )
	{
		...
		// BriJee OVR: Simple weapon lower on sprint
		/*if (cvar->FindVar("oc_weapons_allow_sprint_lower")->GetInt() == 1)
		{
			if (m_fIsSprinting)
			{
				Weapon_Lower();
...
bool CHL2_Player::Weapon_Lower( void )
{
	...
	if (pWeapon->IsNearWall() || pWeapon->IsScopeSighted() || pWeapon->IsIronSighted())
		return false;

	//cvar->FindVar("oc_weapon_is_lowered")->SetValue(1);
...
bool CHL2_Player::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	...
	if (cvar->FindVar("oc_player_allow_fast_gren_throw")->GetInt())
	{
		if (pPlayer && pWeapon && pPlayer->grenadeState <= 0 && pWeapon->thisType == TYPE_GRENADE)
			return false;
	}
#endif
...
void CHL2_Player::ItemPostFrame()
{
	...
	//if (GetActiveWeapon() /*&& !GetActiveWeapon()->IsHolstered()*/)// ? GetActiveWeapon()->thisType != TYPE_GRENADE : true)
	/*{
		CBaseAnimating *pAnimating = this->GetBaseAnimating();
		if (pAnimating)
		{
			...
			//bool weapVisible = GetActiveWeapon() != NULL ? !(GetActiveWeapon()->IsWeaponVisible()) : false;

			/*bool weapVisible = GetActiveWeapon() == NULL ? true : false;

			if (IsSuitEquipped())
			{
				if (m_hUseEntity != NULL)
				{
					if (!pAnimating->GetBodygroup(bg_rh) || !pAnimating->GetBodygroup(bg_lh))//0 - visible
						SetHandsVisible(false);
				}
				else
				{
					if (pAnimating->GetBodygroup(bg_rh) || pAnimating->GetBodygroup(bg_lh))//1 - invisible
						SetHandsVisible(weapVisible);
				}
			}
			else
			{
	...
	{
		CBaseAnimating *pAnimating = this->GetBaseAnimating();
		if (pAnimating && gpGlobals->curtime > m_flAdmireGlovesAnimTime)
		{
			if (IsSuitEquipped())
			{
				if (pAnimating->GetBodygroup(bg_rh) || pAnimating->GetBodygroup(bg_lh))//0 - visible
	...
	CheckLean();	// BriJee : Checking lean state
}

void CHL2_Player::SetBodyRenderingState(bool HEVSuit)
{
	CBaseAnimating *pAnimating = this->GetBaseAnimating();
	...
	if (pAnimating)
	{
		...
		{
			PrecacheModel(cvar->FindVar("oc_playermodel_nosuit")->GetString());
			SetModel(cvar->FindVar("oc_playermodel_nosuit")->GetString());
		}
	}
...
void CHL2_Player::SetRightLegVisible(bool visible)
{
	if (!cvar->FindVar("oc_player_draw_body")->GetInt())
		return;
	...
	if (pAnimating)
	{
		if (visible)
		{
			bg_rl = pAnimating->FindBodygroupByName("right_leg");
			bg_rl_val = 0;//Visible

			pAnimating->SetBodygroup(bg_rl, bg_rl_val);
		}
		else
		{
			bg_rl = pAnimating->FindBodygroupByName("right_leg");
			bg_rl_val = 1;//InVisible
...
void CHL2_Player::SetHandsVisible(bool visible)
{
	if (!cvar->FindVar("oc_player_draw_body")->GetInt())
		return;
	...
	if (pAnimating)// 0 - visible; 1 - InVisible
	{
		if (visible)
		{
			bg_rh = pAnimating->FindBodygroupByName("right_hand");
			bg_lh = pAnimating->FindBodygroupByName("left_hand");

			bg_rh_val = 0;//Visible
			bg_lh_val = 0;//Visible

			pAnimating->SetBodygroup(bg_rh, bg_rh_val);//Visible
			pAnimating->SetBodygroup(bg_lh, bg_lh_val);//Visible
		}
		else
		{
			bg_rh = pAnimating->FindBodygroupByName("right_hand");
			...
			bg_rh_val = 1;//InVisible
			bg_lh_val = 1;//InVisible

			pAnimating->SetBodygroup(bg_rh, bg_rh_val);//InVisible
			pAnimating->SetBodygroup(bg_lh, bg_lh_val);//InVisible
		}
	}
}

void CHL2_Player::SetAdmireGlovesVisible(bool visible)
{
...
//==============================BriJee OVR: Server side leaning
void CHL2_Player::CheckLean()
{


	if ((m_nButtons & IN_FORWARD) || (m_nButtons & IN_BACK)			// BriJee : First check walking off
	...
	else
	{
		...
		//if(IsSuitEquipped())
		//{
		if (/*m_afButtonPressed*/ m_nButtons & IN_LEANLEFT)
			StartLeaning();
...
void CHL2_Player::StartLeaning()
{
	...
	//if (IsWalking())	// BriJee : No leaning on walk
	//return;

	//Create new vectors
	Vector forward, lean, currentoffset, newoffset;
	currentoffset = GetViewOffset();
	
	AngleVectors(EyeAngles(), &forward, &lean, NULL);
	newoffset = currentoffset;


	iDynLeanValue1 = iDynLeanValue1 + 1;	// BriJee : Dynamic algorithm
	if (iDynLeanValue1 > 8)
		iDynLeanValue1 = 8;

	iDynLeanValue2 = iDynLeanValue2 - 1;
	if (iDynLeanValue2 < -8)
		iDynLeanValue2 = -8;
	...
	if (m_nButtons & IN_LEANLEFT)
	{
		...
		newoffset.x = clamp(lean.x, iDynLeanValue2, iDynLeanValue1);
		newoffset.y = clamp(lean.y, iDynLeanValue2, iDynLeanValue1);
...
void CHL2_Player::StopLeaning()
{
	...
	m_bIsLeaning = false;
	iDynLeanValue1 = 0;	// addition
	iDynLeanValue2 = 0;
}
//==============================BriJee OVR: Server side leaning
// Set the activity based on an event or current state	//================================= Overcharged Player Anims definition
/*void CHL2_Player::SetAnimation(PLAYER_ANIM playerAnim)
{
	int animDesired;
	...
	//================================= Overcharged Player Anims definition
