...
extern ConVar oc_blood_displays_on_all_normals;
...
{
	if (info.GetAttacker() && IsPlayerAlly() && cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() && info.GetAttacker()->IsPlayer())
	{
		return true;
	}
	...
	int canDamage = /*cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() ? 1 : */(CapabilitiesGet() & bits_CAP_FRIENDLY_DMG_IMMUNE);
	// FIXME: hook a friendly damage filter to the npc instead?
	if (canDamage && info.GetAttacker() && info.GetAttacker() != this)
	{
		...
		if (bHitByVehicle || (npcEnemy && /*(cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() ?  false : */npcEnemy->IRelationType(this) == D_LI))
		{
...
int CAI_BaseNPC::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if (info.GetAttacker() && IsPlayerAlly() && cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() && info.GetAttacker()->IsPlayer())
	{
		if (this->Classify() == CLASS_PLAYER_ALLY || this->Classify() == CLASS_PLAYER_ALLY_VITAL)
		{
			frdShootsCount++;
			if (frdShootsCount >= frdShootsBorder)
			{
				frdShootsCount = 0;

				if (!(info.GetDamageType() & DMG_CRUSH) && !(info.GetDamageType() & DMG_PHYSGUN))//Avoid physics crush angry
				{

					CBaseEntity *pEntsInArea[512];
					int nEntsCount = UTIL_EntitiesInSphere(pEntsInArea, sizeof(pEntsInArea), this->GetAbsOrigin(), cvar->FindVar("oc_ally_canbecome_enemy_in_radius")->GetInt(), 0);
					for (int i = 0; i < nEntsCount; i++)
					{
						if (pEntsInArea[i] != NULL && pEntsInArea[i]->GetEnemy() == NULL)
						{
							if (pEntsInArea[i]->IsNPC() &&
								(cvar->FindVar("oc_ally_canbecome_enemy_by_class")->GetInt() ?
								(pEntsInArea[i]->GetClassname() == this->GetClassname()) :
								(pEntsInArea[i]->Classify() == CLASS_PLAYER_ALLY || pEntsInArea[i]->Classify() == CLASS_PLAYER_ALLY_VITAL)))//pEntsInArea[i]->GetClassname() == this->GetClassname())
							{
								CAI_BaseNPC *pPlrAlly = static_cast<CAI_BaseNPC *>(pEntsInArea[i]);
								if (pPlrAlly != NULL)
									pPlrAlly->AddClassRelationship(CLASS_PLAYER, D_HT, 99);
							}
						}
					}
				}
	...
	if ( m_flLastDamageTime != gpGlobals->curtime )
	{
		...
		if (info.GetAttacker() && info.GetAttacker()->IsPlayer())//OverCharged FIX
		{
			m_OnDamagedByPlayer.FireOutput( info.GetAttacker(), this );
...
void CAI_BaseNPC::FireBulletsShotgun(const FireBulletsInfo_t &info)
{
#ifdef HL2_DLL
	// If we're shooting at a bullseye, become perfectly accurate if the bullseye demands it
	if (GetEnemy() && GetEnemy()->Classify() == CLASS_BULLSEYE)
	{
		...
		if (pBullseye && pBullseye->UsePerfectAccuracy())
		{
			FireBulletsInfo_t accurateInfo = info;
			accurateInfo.m_vecSpread = vec3_origin;
			BaseClass::FireBulletsShotgun(accurateInfo);
			return;
		}
	}
#endif

	BaseClass::FireBulletsShotgun(info);
}

void CAI_BaseNPC::FireBulletsVehicle(const FireBulletsInfo_t &info)
{
#ifdef HL2_DLL
	// If we're shooting at a bullseye, become perfectly accurate if the bullseye demands it
	if (GetEnemy() && GetEnemy()->Classify() == CLASS_BULLSEYE)
	{
		...
		if (pBullseye && pBullseye->UsePerfectAccuracy())
		{
			FireBulletsInfo_t accurateInfo = info;
			accurateInfo.m_vecSpread = vec3_origin;
			BaseClass::FireBulletsVehicle(accurateInfo);
			return;
		}
	}
#endif

	BaseClass::FireBulletsVehicle(info);
}
//-----------------------------------------------------------------------------
// Shot statistics
//-----------------------------------------------------------------------------
...
void CAI_BaseNPC::MakeDamageBloodDecal ( int cCount, float flNoise, trace_t *ptr, Vector vecDir )
{
	...
	for ( i = 0 ; i < cCount ; i++ )
	{
		...
		if (oc_blood_displays_on_all_normals.GetInt())
		{
			AI_TraceLine(ptr->endpos, ptr->endpos + vecTraceDir * 172, MASK_ALL, this, COLLISION_GROUP_INTERACTIVE, &Bloodtr);
...
void CAI_BaseNPC::NPCInit ( void )
{
	...
	if (IsPlayerAlly() && cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt())
	{
		RemoveEFlags(EFL_NO_DISSOLVE | EFL_NO_MEGAPHYSCANNON_RAGDOLL | EFL_NO_PHYSCANNON_INTERACTION);
		RemoveFlag(EFL_NO_DISSOLVE);
		RemoveFlag(EFL_NO_MEGAPHYSCANNON_RAGDOLL);
		RemoveFlag(EFL_NO_PHYSCANNON_INTERACTION);
		RemoveSpawnFlags(EFL_NO_DISSOLVE | EFL_NO_MEGAPHYSCANNON_RAGDOLL | EFL_NO_PHYSCANNON_INTERACTION);
		CapabilitiesRemove(bits_CAP_NO_HIT_PLAYER | bits_CAP_NO_HIT_SQUADMATES | bits_CAP_FRIENDLY_DMG_IMMUNE);
	}
...
void CAI_BaseNPC::EmitNpcSound(const char* soundName, int iEntIndex)
{
	...
	default:
		if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER))
		{
			...
			else if ( pEvent->event == AE_NPC_RAGDOLL )
			{
				...
				if (CanBecomeServersideRagdoll() && ((GetFlags() & FL_ONFIRE) || cvar->FindVar("oc_ragdoll_serverside")->GetInt()))
				{
...
void CAI_BaseNPC::MagOutNPC(void)
{
	Vector attachOrigin;
	QAngle attachAngles;

	if (this->GetActiveWeapon()->GetWpnData().m_bHasMag && 
		this->GetActiveWeapon()->GetAttachment("muzzle", attachOrigin, attachAngles))
	{
		if (!UTIL_GetIsPrecachedModel(this, this->GetActiveWeapon()->GetWpnData().iMagName))
			return;

		Vector Origin;
		QAngle Ang;

		this->GetActiveWeapon()->GetAttachment("muzzle", Origin, Ang);

		QAngle AngEye;
		Vector forward, right, up;
		AngEye = this->EyeAngles();
		AngleVectors(AngEye, &forward, &right, &up);
		/*if (cvar->FindVar("oc_mag_test_pos_enabled")->GetInt() == 1)
		{

			if (DoOnceMPN)
			{
				cvar->FindVar("oc_mag_test_pos_x")->SetValue(this->GetActiveWeapon()->GetWpnData().Mag_Offset.x);
				cvar->FindVar("oc_mag_test_pos_y")->SetValue(this->GetActiveWeapon()->GetWpnData().Mag_Offset.y);
				cvar->FindVar("oc_mag_test_pos_z")->SetValue(this->GetActiveWeapon()->GetWpnData().Mag_Offset.z);

				cvar->FindVar("oc_mag_test_ang_x")->SetValue(this->GetActiveWeapon()->GetWpnData().Mag_Angle.x);
				cvar->FindVar("oc_mag_test_ang_y")->SetValue(this->GetActiveWeapon()->GetWpnData().Mag_Angle.y);
				cvar->FindVar("oc_mag_test_ang_z")->SetValue(this->GetActiveWeapon()->GetWpnData().Mag_Angle.z);
				DoOnceMPN = false;
			}
			Origin += cvar->FindVar("oc_mag_test_pos_x")->GetFloat();
			Origin += cvar->FindVar("oc_mag_test_pos_y")->GetFloat();
			Origin += cvar->FindVar("oc_mag_test_pos_z")->GetFloat();
		...
		else if (cvar->FindVar("oc_mag_test_pos_enabled")->GetInt() == 0)*/
		{
			...
			Origin += forward * this->GetActiveWeapon()->GetWpnData().Mag_Offset.x;
			Origin += right * this->GetActiveWeapon()->GetWpnData().Mag_Offset.y;
			Origin += up * this->GetActiveWeapon()->GetWpnData().Mag_Offset.z;

			Ang.x += this->GetActiveWeapon()->GetWpnData().Mag_Angle.x;
			Ang.y += this->GetActiveWeapon()->GetWpnData().Mag_Angle.y;
			Ang.z += this->GetActiveWeapon()->GetWpnData().Mag_Angle.z;

		}

		//int MagNumber = this->GetActiveWeapon()->GetWpnData().MagType;
		CBaseEntity *pVial = NULL; // Null
		pVial = CBaseEntity::Create("item_empty_mag", Origin, Ang, this); // Creates

		//pVial->PrecacheModel(this->GetActiveWeapon()->GetWpnData().iMagName);
		pVial->SetModel(this->GetActiveWeapon()->GetWpnData().iMagName);
...
void CAI_BaseNPC::EmitNPCSound(const char* soundName, CBaseCombatCharacter *pCharacter, int entIndex)
{
	CPASAttenuationFilter filter(pCharacter);
...
BEGIN_DATADESC( CAI_BaseNPC )
	...
	DEFINE_FIELD(frdShootsCount, FIELD_INTEGER),
	DEFINE_FIELD(frdShootsBorder, FIELD_INTEGER),
...
{
	frdShootsBorder = random->RandomInt(2,3);
	frdShootsCount = 0;
