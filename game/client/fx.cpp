	...
	bool bOneFrame )
{
	...
	for ( i = 1; i < 9; i++ )
	{
		...
		C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();//OverCharged(CSS Working MuzzleFlash)
...
void MuzzleFlashCallback( const CEffectData &data )
{
	...
	bool firstPerson = data.m_bCustomColors;//true;//(data.m_fFlags & MUZZLEFLASH_FIRSTPERSON) != 0;
	/*if (data.m_fFlags == 7)
		firstPerson = true;*/
	tempents->MuzzleFlash(vecOrigin, vecAngles, data.m_fFlags & (~MUZZLEFLASH_FIRSTPERSON), data.m_hEntity, firstPerson, data.m_nAttachmentIndex);
}
...
{
	tempents->MuzzleFlash_Combine_NPC(data.m_hEntity, data.m_nAttachmentIndex);
}

DECLARE_CLIENT_EFFECT("Ar2MuzzleFlash", Ar2MuzzleFlashCallback);

void Ar2MuzzleFlashParticleCallback(const CEffectData &data)
{
	tempents->MuzzleFlash_Combine_NPC_Particle_Effect(data.m_hEntity, data.m_nAttachmentIndex);
}

DECLARE_CLIENT_EFFECT("Ar2MuzzleFlashParticle", Ar2MuzzleFlashParticleCallback);

void AirboatGunMuzzleFlashParticleCallback(const CEffectData &data)
{
	tempents->MuzzleFlash_AirboatGun_NPC_Particle_Effect(data.m_hEntity, data.m_nAttachmentIndex);
}

DECLARE_CLIENT_EFFECT("AirboatGunMuzzleFlashParticle", AirboatGunMuzzleFlashParticleCallback);


void APCMuzzleFlashParticleCallback(const CEffectData &data)
{
	tempents->MuzzleFlash_APC_NPC_Particle_Effect(data.m_hEntity, data.m_nAttachmentIndex);
}

DECLARE_CLIENT_EFFECT("APCMuzzleFlashParticle", APCMuzzleFlashParticleCallback);
//-----------------------------------------------------------------------------
// Purpose: 
// Input  : &origin - 
//			&velocity - 
...
void DlightCallback(const CEffectData &data)
{
	IClientRenderable *pRenderable = data.GetRenderable();
	if (!pRenderable)
		return;
	...
	if (pRenderable->GetAttachment(data.m_nAttachmentIndex, position, angles))
	{
		...
		dlight_t *dl = effects->CL_AllocDlight(1);	// BriJee OVR : Light our entity
		dl->origin = position;	//effect_origin;
		dl->color.r = 255;
		dl->color.g = 140;
		dl->color.b = 0;
		dl->color.exponent = 5;
		dl->radius = 40.0f * (engine->MapHasHDRLighting() ? 1 : 0.3);
		dl->die = gpGlobals->curtime + 0.1f; //0.2f;
		dl->decay = 512.0f;//0.05f; //0.0f;
	}

}
DECLARE_CLIENT_EFFECT("DlightEffect", DlightCallback);

void BubblesCallback(const CEffectData &data)
{
	IClientRenderable *pRenderable = data.GetRenderable();
	if (!pRenderable)
		return;
...
DECLARE_CLIENT_EFFECT("Bubbles", BubblesCallback);
...
void FX_CommonTracer(const char *name, Vector& start, Vector& end, float velocity, float length, float scale, bool makeWhiz)
{
	VPROF_BUDGET("FX_CommonTracer", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	Vector	vNear, dStart, dEnd, shotDir;
	float	totalDist;
	...
	//float length = random->RandomFloat(250.0f, 500.0f);
	float life = (totalDist + length*3) / velocity;	//NOTENOTE: We want the tail to finish its run as well

	//Add it
	FX_AddDiscreetLine(start, shotDir, velocity, length, totalDist, scale, life, name);
}
//-----------------------------------------------------------------------------
...
void FX_BuildTeslaZap( const CEffectData &data )
{
	...
	/*if (pEntity != NULL)
	{
		dlight_t *el = effects->CL_AllocDlight(LIGHT_INDEX_TE_DYNAMIC + pEntity->entindex());

		// Randomly place it
		el->origin = beamInfo.m_vecStart;
		...
		el->radius = random->RandomInt(132, 128);
		el->decay = el->radius / 0.1f;
		el->die = gpGlobals->curtime + 0.1f;
	}*/
}
...
void TracerEffectCallBack(const CEffectData &data)
{
	if (data.entindex() > 0)
	{
		IClientRenderable *pRenderable = data.GetRenderable();
		if (pRenderable)
		{
			...
			trace_t tr;
			tr.endpos = data.m_vOrigin,
			tempents->TracerCallBack(data.m_hEntity, data.m_nAttachmentIndex, tr, data.m_nDamageType);

		}
	}
}
DECLARE_CLIENT_EFFECT("TracerEffect", TracerEffectCallBack);

void MuzzleLightEffectCallBack(const CEffectData &data)
{
	if (data.entindex() > 0)
	{
		IClientRenderable *pRenderable = data.GetRenderable();
		if (pRenderable)
		{
			...
			tempents->MuzzleLightCallBack();
		}
	}
}

DECLARE_CLIENT_EFFECT("MuzzleLight", MuzzleLightEffectCallBack);
...
/*void MuzzleFlashHoldCallBack(const CEffectData &data)
{
	...
	if (pEntity)
	{
		C_BaseCombatWeapon *pWeapon = static_cast<C_BaseCombatWeapon*>(pEntity);
		if (!pWeapon)
			return;
