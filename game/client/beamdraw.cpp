				...
				int flags, float* color, float fadeLength, float flHDRColorScale )
{
	...
	//flags = FBEAM_SINENOISE;

	//DevMsg("flags %i \n", flags);
	/*DevMsg("source.x %2f \n", source.x);
	debugoverlay->AddLineOverlay(delta, delta, 0, 255, 0, false, 10);
	debugoverlay->AddLineOverlay(delta, source, 255, 0, 0, false, 10);*/
	...
	Vector delta2 = delta;//Over
	//delta2 += RandomVector(-30.f, 30.f);

	ComputeBeamPerpendicular( delta2, &perp1 );
	...
	perp1 += Vector(dot,dot,dot);//(dot, dot);*/
	...
	for ( i = 0; i < segments; i++ )
	{
		...
		//curSeg.m_vPos += RandomVector(-230.f, 230.f);//Over

		// UNDONE: Make this a spline instead of just a line?
		VectorMA( source, fraction, delta2, curSeg.m_vPos );
		...
		//{
			/*Vector test2;
			float factor2 = prgNoise[noiseIndex >> 16] * scale;
			float	s2, c2;
			SinCos(fraction*M_PI*length + freq, &s2, &c2);
			VectorMA(curSeg.m_vPos, factor2 * s2, CurrentViewUp() + RandomVector(-2.f, 2.f), curSeg.m_vPos);
			// Rotate the noise along the perpendicluar axis a bit to keep the bolt from looking diagonal
			VectorMA(curSeg.m_vPos, factor2 * c2, CurrentViewRight() + RandomVector(-2.f, 2.f), curSeg.m_vPos);*/
			
			//VectorMA(curSeg.m_vPos, factor2 * c2, CurrentViewForward(), curSeg.m_vPos);
			//DevMsg("length: %2f \n", length);
			//curSeg.m_vPos += RandomVector(-13.f, 13.f);/////
		//}
...
void DrawBeamQuadraticDouble(const Vector &start, const Vector &control, const Vector &control2, const Vector &end, float width, const Vector &color, float scrollOffset, float flHDRColorScale)
{
	int subdivisions = 16;
	...
	/*BeamSeg_t seg2;
	seg2.m_flAlpha = 1.0;
	seg2.m_flWidth = width;

	float t2 = 0;
	float u2 = fmod(scrollOffset, 1);
	float dt2 = 1.0 / (float)subdivisions;
	for (int i2 = 0; i2 <= subdivisions; i2++, t2 += dt2)
	{
		float omt2 = (1 - t2);
		float p02 = omt2*omt2;
		float p12 = 2 * t2*omt2;
		...
		seg2.m_vPos = p02 * start + p12 * control2 + p22 * end;
		seg2.m_flTexCoord = u - t;
		if (i == 0 || i == subdivisions)
		{
			// HACK: fade out the ends a bit
			seg.m_vColor = vec3_origin;
