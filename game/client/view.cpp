...
ConVar v_viewmodel_fov( "viewmodel_fov_default", "54", FCVAR_ARCHIVE|FCVAR_HIDDEN );
#else
ConVar v_viewmodel_fov( "viewmodel_fov_default", "54", FCVAR_CHEAT|FCVAR_HIDDEN );
#endif
...
void CViewRender::Init( void )
{
	...
	//SSAO Render Targets//OverCharged
//	ITexture *depthOld = materials->FindTexture("_rt_FullFrameDepth", TEXTURE_GROUP_RENDER_TARGET);
	static int flags2 = TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_RENDERTARGET;
	if (depthOld)
		flags2 = depthOld->GetFlags();
	...
	materials->CreateNamedRenderTargetTextureEx("_rt_SSAO", iW, iH, RT_SIZE_NO_CHANGE, materials->GetBackBufferFormat(),
		MATERIAL_RT_DEPTH_SHARED, flags2, 0);
	materials->EndRenderTargetAllocation();
	//End
}
...
float CViewRender::GetZNear()
{
	...
	if (pPlayer)
	{
		if (!pPlayer->IsSuitEquipped())
			return 2;
		else if (pPlayer->IsSuitEquipped() && !pPlayer->GetActiveWeapon())
			return 2;
	}
...
void CViewRender::SetUpViews()
{
	...
	{
		float vm_fov = pPlayer->GetActiveWeapon()->GetWpnData().viewmodel_fov;

		if (vm_fov != NULL && vm_fov != 0)
		{
			m_View.fovViewmodel = abs(vm_fov - flFOVOffset);//OverCharged
		}
		else
			m_View.fovViewmodel = abs(g_pClientMode->GetViewModelFOV() - flFOVOffset);//OverCharged
	}
	else
	{
		//Adjust the viewmodel's FOV to move with any FOV offsets on the viewer's end
		//view.fovViewmodel = g_pClientMode->GetViewModelFOV() - flFOVOffset;//-Вызывало ошибку с отзеркаливанием оружия при зуме//OverCharged
		m_View.fovViewmodel = abs(g_pClientMode->GetViewModelFOV() - flFOVOffset);//OverCharged
	}
