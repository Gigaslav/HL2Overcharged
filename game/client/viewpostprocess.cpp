...
static ConVar mat_bloom_scalefactor_scalar("mat_bloom_scalefactor_scalar", "1.0", FCVAR_ARCHIVE);		// BJ over twi tweak - archive
...
static float g_vMotionBlurValues[4] = { 0.2f, 0.2f, 0.2f, 0.2f };
...
//SSAO
ConVar r_post_draw_ssao_cr("r_post_draw_ssao_cr", "0", FCVAR_ARCHIVE);
ConVar r_post_draw_ssao_cr_blur("r_post_draw_ssao_cr_blur", "0", FCVAR_ARCHIVE);
ConVar r_post_draw_ssao_cr_combined("r_post_draw_ssao_cr_combined", "0", FCVAR_ARCHIVE);
//void CViewRender::DoSSAO(const CViewSetup &view)
void DoSSAO(const CViewSetup &view)
{
	if (!r_post_draw_ssao_cr.GetInt() || !cvar->FindVar("r_post_processing")->GetInt())
		return;
	...
	IMaterial *pSSAOCalcMat = materials->FindMaterial("effects/shaders/ssao/ssao", TEXTURE_GROUP_OTHER, true);
	...
	DrawScreenEffectMaterial(pSSAOCalcMat, x, y, width, height);*/
	...
	if (r_post_draw_ssao_cr_blur.GetBool())
	{
		IMaterial *pSSAOBlurMat = materials->FindMaterial("effects/shaders/ssao/ssaoblur", TEXTURE_GROUP_OTHER, true);
	...
	if (r_post_draw_ssao_cr_combined.GetBool())
	{
		IMaterial *pSSAOCombineMat = materials->FindMaterial("effects/shaders/ssao/ssao_combine", TEXTURE_GROUP_OTHER, true);
