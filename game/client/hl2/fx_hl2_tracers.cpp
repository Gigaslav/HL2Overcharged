...
// Purpose: Gauss Gun's Tracer
//-----------------------------------------------------------------------------
void CommonTracerCallback(const CEffectData &data)
{
	//C_BaseCombatCharacter *pCharacter = ClientEntityList().GetBaseEntity(data.entindex())->MyCombatCharacterPointer();
	C_BaseCombatCharacter *pCharacter = data.GetEntity()->MyCombatCharacterPointer();
	if (!pCharacter)
		return;

	if (pCharacter->IsPlayer())
	{
		C_BaseCombatWeapon *pWeapon = pCharacter->GetActiveWeapon();
		if (pWeapon)
		{
			const char *name = pWeapon->GetWpnData().iOldMuzzleTracerType;
			float flVelocity = random->RandomFloat(pWeapon->GetWpnData().iOldMuzzleTracerVelocityMin, pWeapon->GetWpnData().iOldMuzzleTracerVelocityMax);
			bool bWhiz = pWeapon->GetWpnData().iOldMuzzleTracerWhiz;
			float length = random->RandomFloat(pWeapon->GetWpnData().iOldMuzzleTracerLengthMin, pWeapon->GetWpnData().iOldMuzzleTracerLengthMax);
			float scale = random->RandomFloat(pWeapon->GetWpnData().iOldMuzzleTracerScaleMin, pWeapon->GetWpnData().iOldMuzzleTracerScaleMax);

			CLIENTEFFECT_MATERIAL(name)

			FX_CommonTracer(name, (Vector&)data.m_vStart, (Vector&)data.m_vOrigin, flVelocity, length, scale, bWhiz);
		}
	}
	else
	{
		...
		if (pWeapon)
		{
			const char *name = pWeapon->GetWpnData().iOldMuzzleTracerTypeNPC;
			float flVelocity = random->RandomFloat(pWeapon->GetWpnData().iOldMuzzleTracerVelocityMinNPC, pWeapon->GetWpnData().iOldMuzzleTracerVelocityMaxNPC);
			bool bWhiz = pWeapon->GetWpnData().iOldMuzzleTracerWhizNPC;
			float length = random->RandomFloat(pWeapon->GetWpnData().iOldMuzzleTracerLengthMinNPC, pWeapon->GetWpnData().iOldMuzzleTracerLengthMaxNPC);
			float scale = random->RandomFloat(pWeapon->GetWpnData().iOldMuzzleTracerScaleMinNPC, pWeapon->GetWpnData().iOldMuzzleTracerScaleMaxNPC);
...
DECLARE_CLIENT_EFFECT("CommonTracer", CommonTracerCallback);
//-----------------------------------------------------------------------------
...
void AR2ImpactCallback( const CEffectData &data )
{
	...
	el->radius = random->RandomInt(90, 130) * (engine->MapHasHDRLighting() ? 1 : 0.3);
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void AR2BulletLightCallback(const CEffectData &data)
{
	...
	el->radius = random->RandomInt(80, 120) * (engine->MapHasHDRLighting() ? 1 : 0.3);
	el->decay = el->radius / 0.05f;
	el->die = gpGlobals->curtime + 0.1f;
}

DECLARE_CLIENT_EFFECT("AR2BulletLight", AR2BulletLightCallback);
//-----------------------------------------------------------------------------
...
void MuzzleFlash_Airboat( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	...
	// Tack on the smoke
	pParticle = (SimpleParticle *) pSimple->AddParticle( sizeof( SimpleParticle ), pSimple->GetPMaterial( "effects/combinemuzzle_smoke" ), vec3_origin );
...
void MuzzleFlash_Hunter( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	...
	el->radius = random->RandomInt(120, 200) * (engine->MapHasHDRLighting() ? 1 : 0.3);
	el->decay = el->radius / 0.05f;
