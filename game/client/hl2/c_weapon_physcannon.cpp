...
void C_WeaponPhysCannon::ClientThink( void )
{
	...
	QAngle angAngles;
	//int iLaserAttachment = pOwner->GetViewModel()->LookupAttachment("Muzzle");
	pBeamEnt->GetAttachment(1, vecOrigin2, angAngles);
	::FormatViewModelAttachment(vecOrigin2, false);

	dlight_t *el = effects->CL_AllocDlight(LIGHT_INDEX_MUZZLEFLASH);//( index );
	el->origin = vecOrigin2;
	...
	el->radius = random->RandomInt(42, 58);
...
//-----------------------------------------------------------------------------
void PhyscannonLightCallback(const CEffectData &data)
{
	{
		VPROF_BUDGET("PhyscannonLightCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
		...
		el->color.r = 255;
		el->color.g = random->RandomInt(140, 140);
		el->color.b = random->RandomInt(0, 5);
		el->color.exponent = 1;

		el->radius = random->RandomInt(80, 90) * (engine->MapHasHDRLighting() ? 1 : 0.3);
		el->decay = el->radius / 0.1f;
		el->die = gpGlobals->curtime + 0.1f;
	}
}

DECLARE_CLIENT_EFFECT("PhyscannonLight", PhyscannonLightCallback);

void PhyscannonLightIncreasedCallback(const CEffectData &data)
{
	{
		VPROF_BUDGET("PhyscannonLightIncreasedCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
		...
		el->radius = random->RandomInt(85, 95) * (engine->MapHasHDRLighting() ? 1 : 0.3);
		el->decay = el->radius / 0.1f;
		el->die = gpGlobals->curtime + 0.1f;
	}
}

DECLARE_CLIENT_EFFECT("PhyscannonLightIncreased", PhyscannonLightIncreasedCallback);

void MegaPhyscannonLightCallback(const CEffectData &data)
{
	{
		VPROF_BUDGET("MegaPhyscannonLightCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
		...
		//el->m_Direction = data.m_vNormal;
		el->color.r = 0;
		el->color.g = random->RandomInt(191, 191);
		el->color.b = random->RandomInt(255, 255);
		el->color.exponent = 2;
...
DECLARE_CLIENT_EFFECT("MegaPhyscannonLight", MegaPhyscannonLightCallback);




void MegaPhyscannonBeamsCallback(const CEffectData &data)
{
	...
	if (pRenderable->GetAttachment(data.m_nAttachmentIndex, position, angles))
	{
		...
		/*{
			...
			dl->color.r = 0;
			dl->color.g = random->RandomInt(191, 191);
			dl->color.b = random->RandomInt(255, 255);
			dl->color.exponent = 2;
			dl->radius = random->RandomInt(115, 125) * (engine->MapHasHDRLighting() ? 1 : 0.3);
