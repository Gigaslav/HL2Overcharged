...
ITexture *GetScopeTexture(void)
{
	if (!s_pScopeTexture)
	{
		/*ITexture *text = materials->FindTexture("_rt_Scope", TEXTURE_GROUP_RENDER_TARGET);
		IMaterialVar *var;
		var = text->FindVar("$MUTABLE_01", NULL);
...
/*static CTextureReference s_pCamoTexture;
ITexture *GetCamoTexture(void)
{
	if (!s_pCamoTexture)
	{
		s_pCamoTexture.Init(materials->FindTexture("_rt_Camo", TEXTURE_GROUP_RENDER_TARGET));
		Assert(!IsErrorTexture(s_pCamoTexture));
		AddReleaseFunc();
	}
	return s_pCamoTexture;
}*/
//////////////////////////////////////////////////////////////////////////////////
//=============================================================================
