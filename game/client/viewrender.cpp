...
#include "ShaderEditor/Grass/CGrassCluster.h"//OverCharged
#include "ShaderEditor/ShaderEditorSystem.h"//OverCharged
...
#ifdef MAPBASE
#include "OverCharged/RTT/c_func_fake_worldportal.h"
#endif

// Projective textures
#include "C_Env_Projected_Texture.h"

#include "screen_overlay_multiple.h"
...
static ConVar fog_start("fog_start", "-1", FCVAR_REPLICATED);
static ConVar fog_end("fog_end", "-1", FCVAR_REPLICATED);
static ConVar fog_color("fog_color", "-1 -1 -1", FCVAR_REPLICATED);
static ConVar fog_enable("fog_enable", "1", FCVAR_REPLICATED);
static ConVar fog_startskybox("fog_startskybox", "-1", FCVAR_REPLICATED);
static ConVar fog_endskybox("fog_endskybox", "-1", FCVAR_REPLICATED);
static ConVar fog_maxdensityskybox("fog_maxdensityskybox", "-1", FCVAR_REPLICATED);
static ConVar fog_colorskybox("fog_colorskybox", "-1 -1 -1", FCVAR_REPLICATED);
static ConVar fog_enableskybox("fog_enableskybox", "1", FCVAR_REPLICATED);
static ConVar fog_maxdensity("fog_maxdensity", "-1", FCVAR_REPLICATED);
...
static ConVar r_post_processing_special("r_post_processing_special", "1");//OverCharged
#ifdef TF_CLIENT_DLL
								...
								int nClearFlags, view_id_t viewID, bool bDrawViewModel, int baseDrawFlags, ViewCustomVisibility_t *pCustomVisibility )
{
	...
	g_ShaderEditorSystem->CustomViewRender(&g_CurrentViewID, fogVolumeInfo, info);//OverCharged
...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	else
	{
		...
		{
			g_ShaderEditorSystem->UpdateSkymask( view.x, view.y, view.width, view.height, false );//OverCharged
		...
		if( !g_pIntroData )
		{
			...
			if ( ( bDrew3dSkybox = pSkyView->Setup( view, &nClearFlags, &nSkyboxVisible ) ) != false )
			{
				g_ShaderEditorSystem->UpdateSkymask(view.x, view.y, view.width, view.height, false);//Ov
				AddViewToScene( pSkyView );
			}
		...
		if (r_post_processing_special.GetInt())//OverCharged
		{
			//Вызывать DOF необходимо именно в этом участке функции, чтобы он не размывал viewmodel
			DrawDOF(view.x, view.y, view.width, view.height);//OverCharged
			//DrawSSAO2_Support(view.x, view.y, view.width, view.height);//OverCharged
			DrawIRDOF(view.x, view.y, view.width, view.height);//OverCharged
		}
		...
		g_ShaderEditorSystem->UpdateSkymask(view.x, view.y, view.width, view.height, bDrew3dSkybox);//Survivor mp//OverCharged
		...
		g_ShaderEditorSystem->CustomPostRender();//OverCharged
	...
	//Do SSAO//PostProcessing
//	if ((r_post_draw_ssao_cr.GetInt()))// && (r_post_processing_special.GetInt()))//OverCharged
//	{
		DoSSAO(view);
//	}
	...
	CViewSetup ViewCoord = view;
	cvar->FindVar("oc_viewport_vector_x")->SetValue(ViewCoord.origin.x);
	cvar->FindVar("oc_viewport_vector_y")->SetValue(ViewCoord.origin.y);
	cvar->FindVar("oc_viewport_vector_z")->SetValue(ViewCoord.origin.z);
	//Draw the scope too
	...
	const CViewSetup &cameraView, C_BasePlayer *localPlayer, int x, int y, int width, int height )
{
 ...
 ConVar oc_weapon_free_aim_movemax_viewmodel_RT("oc_weapon_free_aim_movemax_viewmodel_RT", "0.85", FCVAR_ARCHIVE);
...
static ConVar oc_adjust_weapon_rt_offset_y("oc_adjust_weapon_rt_offset_y", "12.0", FCVAR_REPLICATED, "Y Axis");
static ConVar oc_adjust_weapon_rt_offset_x("oc_adjust_weapon_rt_offset_x", "-8.0", FCVAR_REPLICATED, "X Axis");
static ConVar oc_adjust_weapon_rt_offset_z("oc_adjust_weapon_rt_offset_z", "-90.0", FCVAR_REPLICATED, "Z Angle");
static ConVar oc_adjust_weapon_rt_offset_fov("oc_adjust_weapon_rt_offset_fov", "-90.0", FCVAR_REPLICATED, "Z Angle");
static ConVar oc_adjust_weapon_rt_offset("oc_adjust_weapon_rt_offset", "0.0", FCVAR_REPLICATED);

static ConVar oc_adjust_weapon_rtiron_offset_y("oc_adjust_weapon_rtiron_offset_y", "12.0", FCVAR_REPLICATED, "Y Axis");
static ConVar oc_adjust_weapon_rtiron_offset_x("oc_adjust_weapon_rtiron_offset_x", "-8.0", FCVAR_REPLICATED, "X Axis");
static ConVar oc_adjust_weapon_rtiron_offset_z("oc_adjust_weapon_rtiron_offset_z", "-90.0", FCVAR_REPLICATED, "Z Angle");
static ConVar oc_adjust_weapon_rtiron_offset("oc_adjust_weapon_rtiron_offset", "0.0", FCVAR_REPLICATED, "Player is firing state.");

static ConVar oc_test_sniper_rt_x("oc_test_sniper_rt_x", "0.0", FCVAR_REPLICATED, "Player is firing state.");

//void CViewRender::DrawScope(const CViewSetup &viewSet)
void CViewRender::DrawScope(/*const CViewSetup &viewSet*/void)
{
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if (!pPlayer)
		return;
	...
	bool sightedEvent = pWeapon->IsScopeSighted() || pWeapon->IsIronSighted();

	bool enabled = pWeapon->IsNearWall() || pPlayer->IsRunning() || pWeapon->m_bInReload || pWeapon->m_flNextInspectAnimation > gpGlobals->curtime;

	const char *attachment = pWeapon->GetWpnData().rtAttachment;
	int iAttachment = pVm->LookupAttachment(attachment);//"Scope"

	pPlayer->GetViewModel()->GetAttachment(iAttachment, vecLightOrigin, ang);

	RT = vecLightOrigin;

	MAngle = pPlayer->EyeAngles();

	if (enabled)
	{
		MAngle.x += pWeapon->GetWpnData().RT_Angle.x + AngleDiff(ang.x, pPlayer->EyeAngles().x);
		MAngle.y += pWeapon->GetWpnData().RT_Angle.y + AngleDiff(ang.y, pPlayer->EyeAngles().y);
		MAngle.z += pWeapon->GetWpnData().RT_Angle.z + AngleDiff(ang.z, pPlayer->EyeAngles().z);
	}
	else
	{
		if (!sightedEvent)
		{
			if (oc_adjust_weapon_rt_offset.GetInt() == 1)
			{
				float RT_adjust_right = oc_adjust_weapon_rt_offset_y.GetFloat();
				float RT_adjust_up = oc_adjust_weapon_rt_offset_x.GetFloat();
				float RT_adjust_roll = oc_adjust_weapon_rt_offset_z.GetFloat();

				MAngle.x += RT_adjust_up;
				MAngle.y += RT_adjust_right;
				MAngle.z += RT_adjust_roll;
			}
			else
			{
				MAngle.x += pWeapon->GetWpnData().RT_Angle.x;
				MAngle.y += pWeapon->GetWpnData().RT_Angle.y;
				MAngle.z += pWeapon->GetWpnData().RT_Angle.z;
			}
		}
	}
	...
	else
	{
		if (cvar->FindVar("oc_state_IRsight_on")->GetInt() == 1)
		{
			if (oc_adjust_weapon_rtiron_offset.GetInt() == 1)
			{
				float RTiron_adjust_up = oc_adjust_weapon_rtiron_offset_x.GetFloat();
				float RTiron_adjust_right = oc_adjust_weapon_rtiron_offset_y.GetFloat();
				float RTiron_adjust_roll = oc_adjust_weapon_rtiron_offset_z.GetFloat();

				MAngle.x += RTiron_adjust_up;
				MAngle.y += RTiron_adjust_right;
				MAngle.z += RTiron_adjust_roll;
			}
			else
			{

					/*Vector eye, eyeFwd, eyeR, eyeU;
					pPlayer->EyePositionAndVectors(&eye, &eyeFwd, &eyeR, &eyeU);

					Vector dir = pPlayer->GetAutoaimVector(AUTOAIM_SCALE_DEFAULT);

					Vector vecOrigin;
					C_BaseViewModel *pBeamEnt = pPlayer->GetViewModel();
					int iLaserAttachment = pPlayer->GetViewModel()->LookupAttachment(pPlayer->GetActiveWeapon()->GetWpnData().rtAttachment);
					pBeamEnt->GetAttachment(iLaserAttachment, vecOrigin, MAngle);
					Vector vecFwd, vecR, vecU;
					QAngle newAng;
					pBeamEnt->GetVectors(&vecFwd, &vecR, &vecU);

					vecOrigin = ((vecOrigin + eye) + dir* MAX_TRACE_LENGTH);

					VectorAngles(vecOrigin, newAng);*/
				...
				/*MAngle.x = MAngle.x + pPlayer->GetActiveWeapon()->GetWpnData().RT_AngleIron.x;
				MAngle.y = MAngle.y + pPlayer->GetActiveWeapon()->GetWpnData().RT_AngleIron.y;
				MAngle.z = MAngle.z + pPlayer->GetActiveWeapon()->GetWpnData().RT_AngleIron.z;*/
			}
		}
		else
		{
			...
			QAngle view = pPlayer->EyeAngles();
			MAngle = view;
			/*MAngle.x += pPlayer->GetActiveWeapon()->GetWpnData().RT_Angle.x;
			MAngle.y += pPlayer->GetActiveWeapon()->GetWpnData().RT_Angle.y;
			MAngle.z += pPlayer->GetActiveWeapon()->GetWpnData().RT_Angle.z;*/
		}
	}
#endif
	...
	if (::input->CAM_IsFreeAiming())
	{
		...
		Vector vecForward = RT;
		Vector2D vec_AimPos = ::input->CAM_GetFreeAimCursor();

		int screen_x, screen_y;
		engine->GetScreenSize(screen_x, screen_y);
		screen_x *= vec_AimPos.x * oc_weapon_free_aim_movemax_viewmodel_RT.GetFloat() + 0.5f;
		screen_y *= vec_AimPos.y * oc_weapon_free_aim_movemax_viewmodel_RT.GetFloat() + 0.5f;
		ScreenToWorld(screen_x, screen_y, pPlayer->GetFOV(), RT, MAngle, vecForward);

		UTIL_TraceLine(RT, RT + vecForward * MAX_TRACE_LENGTH, MASK_SHOT, pPlayer, COLLISION_GROUP_NONE, &tr);

		VectorAngles(vecForward, MAngle);
	}

	CViewSetup scopeView = GetView(STEREO_EYE_MONO);//viewSet;
	...
	ITexture *pRenderTarget = GetScopeTexture();
		...
		scopeView.angles = MAngle;
		
		scopeView.width = pRenderTarget->GetActualWidth();
		scopeView.height = pRenderTarget->GetActualHeight();

		if (!oc_adjust_weapon_rt_offset.GetInt())
			scopeView.fov = pWeapon->GetMaxFOV(); // изменение FOV в скрипте
		else if (oc_adjust_weapon_rt_offset.GetInt())
			scopeView.fov = oc_adjust_weapon_rt_offset_fov.GetInt();

		if ((scopeView.fov == 0) || (scopeView.fov == NULL))//Исправление для шейдера солнечных лучей. Если аргумент не принимает значений, либо 0, то выйти из функции.
			return;
		...
		SafeRelease(pSkyView);
		ViewDrawScene(false, SKYBOX_3DSKYBOX_VISIBLE, scopeView, VIEW_CLEAR_DEPTH, VIEW_MAIN);
		render->PopView(m_Frustum);

}

void CViewRender::DrawScopeFalse(const CViewSetup &viewSet)
{
	...
	//scopeView.fov = 0.0f; // изменение FOV в скрипте
	if ((scopeView.fov == 0) || (scopeView.fov == NULL))//Исправление для шейдера солнечных лучей. Если аргумент не принимает значений, либо 0, то выйти из функции.
		return;
	...
	//ViewDrawScene(CASCADEDCONFIG_NONE, false, SKYBOX_3DSKYBOX_VISIBLE, scopeView, VIEW_CLEAR_DEPTH, VIEW_MONITOR);//GSTRING
	render->PopView(m_Frustum);
}
