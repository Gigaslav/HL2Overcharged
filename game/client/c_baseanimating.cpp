...
//	DEFINE_PRED_ARRAY( m_flPoseParameter, FIELD_FLOAT, MAXSTUDIOPOSEPARAM, FTYPEDESC_INSENDTABLE ),
	...
	DEFINE_FIELD(ForcedRagdollOnClient, FIELD_BOOLEAN),
...
void C_ClientRagdoll::OnRestore( void )
{
	...
	else if ( GetFlags() & FL_ONFIRE )
	{
		...
		//SetEffectEntity(pFireChild);
		//Set the new fire child as the new effect entity.
		SetEffectEntity( pNewFireChild );

		/*pFireChild->AddEffects(EF_BRIGHTLIGHT);
		if (pFireChild->IsEffectActive(EF_BRIGHTLIGHT) || pFireChild->IsEffectActive(EF_DIMLIGHT))
		{
			dlight_t *dl = effects->CL_AllocDlight(index);
			dl->origin = pFireChild->GetAbsOrigin();
			dl->origin[2] += 16;
			dl->color.r = 254;
			dl->color.g = 174;
			dl->color.b = 10;
			dl->radius = cvar->FindVar("oc_env_fire_light_radius")->GetFloat() + random->RandomFloat(-20, 20);
			dl->die = gpGlobals->curtime + 5.001;
		}
		pNewFireChild->AddEffects(EF_BRIGHTLIGHT);
		if (pNewFireChild->IsEffectActive(EF_BRIGHTLIGHT) || pNewFireChild->IsEffectActive(EF_DIMLIGHT))
		{
			dlight_t *dl = effects->CL_AllocDlight(index);
			dl->origin = pNewFireChild->GetAbsOrigin();
			dl->origin[2] += 16;
			dl->color.r = 254;
			dl->color.g = 174;
			dl->color.b = 10;
...
void C_ClientRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
	...
	if (cvar->FindVar("oc_ragdoll_show_blood")->GetInt() == 1)
	{
		float flNoise;
		...
		const surfacedata_t *ptexdata = physprops->GetSurfaceData(pTrace->surface.surfaceProps);
		Vector vecTraceDir = dir * -1;// trace in the opposite direction the shot came from (the direction the shot is going)
		...
		{
			BloodColor = 1;
			DevMsg("CHAR_TEX_ALIENFLESH \n");
			DevMsg("Type: %s \n", GetModelName());
			//cvar->FindVar("oc_blood_color")->SetValue(1);
			//DevMsg("bloodColor: %.i \n", cvar->FindVar("oc_blood_color")->GetInt());
		}
		if ((char)ptexdata->game.material == CHAR_TEX_BLOODYFLESH)
		{
			DevMsg("CHAR_TEX_BLOODYFLESH \n");
			DevMsg("Type: %s \n", GetModelName());
			if (/*(Q_strcmp(STRING(GetModelName()), "models/zombie/classic.mdl") == 0) || (Q_strcmp(STRING(GetModelName()), "models/zombie/fast.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/classic_legs.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/classic_torso.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/poison.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/fast_torso.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/zombie_soldier.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/zombie_soldier_torso.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/zombie/zombie_soldier_legs.mdl") == 0) ||
				(Q_strcmp(STRING(GetModelName()), "models/headcrabblack.mdl") == 0)) ||*/

				Q_strstr(GetModelName(), "models/Zombie/Classic.mdl") ||
				Q_strstr(GetModelName(), "models/Zombie/") ||
				Q_strstr(GetModelName(), "models/Zombie/Fast.mdl") ||
				Q_strstr(GetModelName(), "models/Zombie/Classic_legs.mdl") ||
				Q_strstr(GetModelName(), "models/Zombie/Classic_torso.mdl") ||
				Q_strstr(GetModelName(), "models/Zombie/Poison.mdl") ||
				Q_strstr(GetModelName(), "models/zombie/Fast_torso.mdl") ||
				Q_strstr(GetModelName(), "models/Zombie/Zombie_soldier.mdl") ||
				Q_strstr(GetModelName(), "models/zombie/Zombie_soldier_torso.mdl") ||
				Q_strstr(GetModelName(), "models/zombie/Zombie_soldier_legs.mdl") ||
				Q_strstr(GetModelName(), "models/headcrabblack.mdl") ||
		...
		if ((char)ptexdata->game.material == CHAR_TEX_FLESH)
		{
				...
				Q_strstr(GetModelName(), "models/zombie/Zombie_soldier_legs.mdl"))
			{
				...
				BloodColor = 0;
			//cvar->FindVar("oc_blood_color")->SetValue(0);
			//DevMsg("bloodColor: %.i \n", cvar->FindVar("oc_blood_color")->GetInt());
		}
		if ((char)ptexdata->game.material == CHAR_TEX_ANTLION)
		{
			DevMsg("CHAR_TEX_ANTLION \n");
			DevMsg("Type: %s \n", GetModelName());
			BloodColor = 2;
			//cvar->FindVar("oc_blood_color")->SetValue(2);
			//DevMsg("bloodColor: %.i \n", cvar->FindVar("oc_blood_color")->GetInt());
		}
		...
		//if ((char)ptexdata->game.material == CHAR_TEX_FLESH)
		{
			//SpawnBlood(ptr->endpos, dir, BLOOD_COLOR_RED, 100);
			Vector temp;
			VectorSubtract(pTrace->endpos, pTrace->plane.normal, temp);
			QAngle vecAngles;
			VectorAngles(temp, vecAngles);

			if (BloodColor == 0)
			DispatchParticleEffect("blood_impact_red_01", pTrace->endpos, vecAngles);
			else if (BloodColor == 1)
				DispatchParticleEffect("blood_impact_yellow_01", pTrace->endpos, vecAngles);
			else if (BloodColor == 2)
				DispatchParticleEffect("blood_impact_green_01", pTrace->endpos, vecAngles);

			if (cvar->FindVar("oc_blood_displays_on_all_normals")->GetInt() == 0)
			{
				trace_t Bloodtr;
				UTIL_TraceLine(pTrace->endpos, pTrace->startpos + vecTraceDir * -flTraceDist, MASK_SOLID_BRUSHONLY & ~CONTENTS_GRATE, this, COLLISION_GROUP_NONE, &Bloodtr);
			...
			{
				trace_t Bloodtr;
				UTIL_TraceLine(pTrace->endpos, pTrace->startpos + vecTraceDir * -flTraceDist, MASK_ALL, this, COLLISION_GROUP_INTERACTIVE, &Bloodtr);
				if (Bloodtr.fraction != 1.0)
				{
					UTIL_BloodDecalTrace(&Bloodtr, BloodColor);
				}
		...
		/*else if ((char)ptexdata->game.material == CHAR_TEX_ALIENFLESH)
		{
			Vector temp;
			VectorSubtract(pTrace->endpos, pTrace->plane.normal, temp);
			QAngle vecAngles;
			...
			if (cvar->FindVar("oc_blood_displays_on_all_normals")->GetInt() == 0)
			{
				...
				if (Bloodtr.fraction != 1.0)
				{
					...
					UTIL_BloodDecalTrace(&Bloodtr, 2);
				}
			}
			else if (cvar->FindVar("oc_blood_displays_on_all_normals")->GetInt() == 1)
			{
		...
		else if ((char)ptexdata->game.material == CHAR_TEX_ANTLION)
		{
			Vector temp;
			VectorSubtract(pTrace->endpos, pTrace->plane.normal, temp);
			QAngle vecAngles;
		...
		/*debugoverlay->AddLineOverlay(pTrace->startpos, pTrace->endpos, 0, 255, 0, false, 10);
		Vector temp;
		VectorSubtract(pTrace->endpos, pTrace->plane.normal, temp);
		QAngle vecAngles;
		VectorAngles(temp, vecAngles);
...
void C_BaseAnimating::DoAnimationEvents( CStudioHdr *pStudioHdr )
{
	...
	for (int i = 0; i < (int)seqdesc.numevents; i++)
	{
		...
		if ( (pevent[i].cycle > m_flPrevEventCycle && pevent[i].cycle <= flEventCycle) )
		{
			...
			/*DevMsg("[i]: %i \n", i);
			DevMsg("pevent[i].cycle: %2f \n", pevent[i].cycle);
			DevMsg("pevent[i].event: %i \n", pevent[i].event);*/
			FireEvent( GetAbsOrigin(), GetAbsAngles(), pevent[ i ].event, pevent[ i ].pszOptions() );
		}
	}
...
void UpdateAllSounds(const CEffectData &data)
{
	//NotWorking
	return;
	...
	if (!pPlayer)
		return;
		//pPlayer->UpdateAllSounds();

	//return;

	CUtlVector<SndInfo_t, CUtlMemory<SndInfo_t, int>> &sndlist = CUtlVector<SndInfo_t, CUtlMemory<SndInfo_t, int>>();
	enginesound->GetActiveSounds(sndlist);

	for (int i = 0; i < sndlist.Count(); i++)
	{
		auto ep = sndlist.Element(i);
		
		//ep.m_nPitch *= data.m_flScale;
		
		ep.m_nPitch = pPlayer->GetSlowMoIsEnabled() ? 30.f : 100.f;
		/*int id = ep.m_nGuid;
		auto soundName = soundemitterbase->GetSoundName(id);

		HSOUNDSCRIPTHANDLE handle = (HSOUNDSCRIPTHANDLE)soundemitterbase->GetSoundIndex(STRING(soundName));
		...
		if (!params)
			continue;
		float flPitchScale = cvar->FindVar("host_timescale")->GetFloat();
		params->SetPitch(flPitchScale*100.f);

		soundemitterbase->UpdateSoundParameters(soundName, *params);*/

#ifdef GAME_DLL
		CBaseEntity *pEntity = UTIL_EntityByIndex(ep.m_nSoundSource);
#else
		C_BaseEntity *pEntity = ClientEntityList().GetEnt(ep.m_nSoundSource);
#endif
		if (pEntity)
		{
			EmitSound_t params;
			params.m_pSoundName = "HL2Player.SlowIn";
			params.m_nChannel = ep.m_nChannel;
			params.m_flVolume = ep.m_flVolume;
			params.m_nPitch = 100.f;//ep.m_nPitch;
		...
		auto sndName = soundemitterbase->GetSoundName(i);

		CSoundParametersInternal params;
		params.CopyFrom(*soundemitterbase->InternalGetParametersForSound(i));

		params.SetPitch(flPitchScale*100.f);

		soundemitterbase->UpdateSoundParameters(sndName, params);*/
	}
}
DECLARE_CLIENT_EFFECT("UpdateAllSounds", UpdateAllSounds);
//-----------------------------------------------------------------------------
...
bool C_BaseAnimating::DispatchMuzzleEffect( const char *options, bool isFirstPerson )
{
	...
	if (isFirstPerson)
	{
		...
		//DevMsg("isFirstPerson pWeapon \n");

		int index = random->RandomInt(0, pWeapon->GetWpnData().externalIndex);

		if (pPlayer->GetWaterLevel() != 3)
			name = pWeapon->GetWpnData().iMuzzleFlashType[index];
		else
			name = pWeapon->GetWpnData().iMuzzleFlashUnderWaterType;

		colR = pWeapon->GetWpnData().iMuzzleFlashLightR;
		colG = pWeapon->GetWpnData().iMuzzleFlashLightG;
		colB = pWeapon->GetWpnData().iMuzzleFlashLightB;

		MFType = pWeapon->GetWpnData().oldFlashType;


		attachmentIndex = pWeapon->GetWpnData().iAttachment;// LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment);

	}	                
	else                
	{
		C_BaseCombatCharacter *pOwner = ToBaseCombatCharacter(GetOwnerEntity());//GetOwnerEntity()

		if (pOwner)
		{
			if (pOwner->IsPlayer() && cvar->FindVar("oc_player_draw_body")->GetInt() && !cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
				return false;

			int indexNpc = random->RandomInt(0, pOwner->GetActiveWeapon()->GetWpnData().externalIndexNpc);

			if (pOwner->GetActiveWeapon()->WeaponGetWaterLevel() != 3)
			{
				//DevMsg("not under water \n");
				name = pOwner->GetActiveWeapon()->GetWpnData().iMuzzleFlashTypeNpc[indexNpc];
			}
			else
			{
				//DevMsg("under water \n");
				name = pOwner->GetActiveWeapon()->GetWpnData().iMuzzleFlashUnderWaterTypeNpc;
			}
			colR = pOwner->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightR;
			colG = pOwner->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightG;
			colB = pOwner->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightB;

			MFType = pOwner->GetActiveWeapon()->GetWpnData().oldFlashTypeNPC;
			attachmentIndex = LookupAttachment(pOwner->GetActiveWeapon()->GetWpnData().MuzzleAttachmentNPC);
		}
		else if (!pOwner && token)
		{
			...
			{
				MFType = MUZZLEFLASH_COMBINE;
			}
			else if (Q_stricmp(token, "SMG1") == 0)
			{
				MFType = MUZZLEFLASH_SMG1;
			}
			else if (Q_stricmp(token, "SMG2") == 0)//OverCharged
			{
				MFType = MUZZLEFLASH_SMG2;
			}
			else if (Q_stricmp(token, "AK47") == 0)//OverCharged
			{
				MFType = MUZZLEFLASH_AK47;
			}
			else if (Q_stricmp(token, "DUAL_FLASH") == 0)//OverCharged
			{
				MFType = MUZZLEFLASH_DUAL_FLASH;
			}
			else if (Q_stricmp(token, "PISTOL") == 0)
			{
				MFType = MUZZLEFLASH_PISTOL;
			}
			else if (Q_stricmp(token, "SHOTGUN") == 0)
			{
				MFType = MUZZLEFLASH_SHOTGUN;
			}
			else if (Q_stricmp(token, "357") == 0)
			{
				MFType = MUZZLEFLASH_357;
			}
			else if (Q_stricmp(token, "RPG") == 0)
			{
				MFType = MUZZLEFLASH_RPG;
			}
			else if (Q_stricmp(token, "SNIPER") == 0)
			{
				MFType = MUZZLEFLASH_SNIPER;
			}
			else if (Q_stricmp(token, "GAUSS") == 0)
			{
				MFType = MUZZLEFLASH_GAUSS;
			}
			else if (Q_stricmp(token, "RIFLE") == 0)
			{
				//NOTENOTE: This means you specified an invalid muzzleflash type, check your spelling?
				//		Assert( 0 );
				MFType = MUZZLEFLASH_RIFLE;
			}
			else if (Q_stricmp(token, "ABGUN") == 0)
			{
				MFType = MUZZLEFLASH_ABGUN;
			}
			else if (Q_stricmp(token, "SHOCK") == 0)
			{
				MFType = MUZZLEFLASH_SHOCK;
			}
			else
			{
				Assert(0);
	...
	// Send it out
	tempents->MuzzleFlash( name, colR, colG, colB, MFType, GetRefEHandle(), attachmentIndex, isFirstPerson );

	return true;
}


bool C_BaseAnimating::DispatchMuzzleSecondaryEffect(const char *options, bool isFirstPerson)
{
	const char	*p = options;
	char		token[128];
	int			MFType = 0;
	...
	if (isFirstPerson)
	{
		...
		{
			int index = random->RandomInt(0, pWeapon->GetWpnData().externalIndexS);

			if (pPlayer->GetWaterLevel() != 3)
				name = pWeapon->GetWpnData().iMuzzleFlashTypeS[index];
			else
				name = pWeapon->GetWpnData().iMuzzleFlashUnderWaterType;
			...
			/*if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
				attachmentIndex = LookupAttachment(pWeapon->GetWpnData().iSecondaryAttachment);
			else*/
				attachmentIndex = LookupAttachment(pWeapon->GetWpnData().iSecondaryAttachment);
		}
	}
	...
	// Send it out
	tempents->MuzzleFlashSecondary(name, colR, colG, colB, MFType, GetRefEHandle(), attachmentIndex, isFirstPerson);

	return true;
}

bool C_BaseAnimating::DispatchMuzzleChargeEffect(const char *options)
{
	const char	*p = options;
	char		token[128];
	int			MFType = MUZZLEFLASH_COMBINE;
	int			attachmentIndex = -1;
	// Get the first parameter
	p = nexttoken(token, p, ' ');
	// Get the second parameter
	...
	const char *name = pWeapon->GetWpnData().iMuzzleFlashDelayed;

	bool FP = true;

	if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
	{
		attachmentIndex = pWeapon->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment);
		FP = false;
	}
	else
		attachmentIndex = LookupAttachment(pWeapon->GetWpnData().iSecondaryAttachment);
	...
	// Send it out
	tempents->MuzzleFlashSecondary(name, colR, colG, colB, MFType, GetRefEHandle(), attachmentIndex, FP);
...
/*bool C_BaseAnimating::DispatchMuzzleEffect( const char *options, bool isFirstPerson )
{
	...
	if ( token ) 
	{
		...
		{
			weaponType = MUZZLEFLASH_AK47;
		}
		else if (Q_stricmp(token, "DUAL_FLASH") == 0)//OverCharged
		{
			weaponType = MUZZLEFLASH_DUAL_FLASH;
		}
		else if ( Q_stricmp( token, "PISTOL" ) == 0 )
		{
		...
		{
			weaponType = MUZZLEFLASH_GAUSS;
		}
		else if (Q_stricmp(token, "RIFLE") == 0)
		{
			//NOTENOTE: This means you specified an invalid muzzleflash type, check your spelling?
	//		Assert( 0 );
			weaponType = MUZZLEFLASH_RIFLE;
		}
	}
	else
	{
...
void MaterialFootstepSound( C_BaseAnimating *pEnt, bool bLeftFoot, float flVolume )
{
	...
	if (pPlayer && cvar->FindVar("oc_player_draw_body")->GetInt())
		return;*/
...
void MuzzleFlashHoldCallBack(const CEffectData &data)
{
	C_BaseAnimating *pBAnim = dynamic_cast<C_BaseAnimating *>(ClientEntityList().GetBaseEntity(data.entindex()));

	if (pBAnim)
	{
		C_BaseCombatWeapon *pWeapon = static_cast<C_BaseCombatWeapon*>(ClientEntityList().GetEnt(data.entindex()));
		if (pWeapon)
		{
			pBAnim->DispatchMuzzleHoldingEffect(pWeapon);
		}
	}
}
DECLARE_CLIENT_EFFECT("MuzzleFlashHolding", MuzzleFlashHoldCallBack);

void C_BaseAnimating::DispatchMuzzleHoldingEffect(C_BaseCombatWeapon *pWeapon)
{
	if (!pWeapon)
		return;

	int attachment = LookupAttachment(pWeapon->GetWpnData().MuzzleAttachmentNPC);
	if (attachment == -1)
		attachment = 1;
	...
	MFType = pWeapon->GetWpnData().oldFlashTypeNPC;


	int indexNpc = random->RandomInt(0, pWeapon->GetWpnData().externalIndexNpc);

	if (pWeapon->WeaponGetWaterLevel() != 3)
	{
		name = pWeapon->GetWpnData().iMuzzleFlashTypeNpc[indexNpc];
	}
	else
	{
		name = pWeapon->GetWpnData().iMuzzleFlashUnderWaterTypeNpc;
	}

	// Send it out
	tempents->MuzzleFlash(name, colR, colG, colB, MFType, GetRefEHandle(), attachment, false);
	tempents->MuzzleflashCreateWeaponLight(GetRefEHandle());
}


void CommonShellEjectCallBack(const CEffectData &data)
{
	C_BaseViewModel *pEntity = dynamic_cast<C_BaseViewModel *>(ClientEntityList().GetEnt(data.entindex()));

	if (!pEntity)
		return;

	if (MainViewOrigin().DistToSqr(pEntity->GetAbsOrigin()) < (256 * 256))
	{
		...
		if (pRenderable)
		{
			...
			if (pEntity->GetAttachment(data.m_nAttachmentIndex, attachOrigin, attachAngles))
				tempents->EjectBrass(attachOrigin, attachAngles, pRenderable->GetRenderAngles(), 0);
		}
	}
}
DECLARE_CLIENT_EFFECT("CommonShellEject", CommonShellEjectCallBack);

void FireEventCallBack(const CEffectData &data)
{
	/*C_BaseEntity *pEntity = ClientEntityList().GetEnt(data.entindex());
	C_BaseAnimating *pBAnim = pEntity ? pEntity->GetBaseAnimating() : NULL;
	if (!pBAnim)
		return;*/
	...
	if (pBAnim)
		pBAnim->FireEvent(data.m_vOrigin, data.m_vAngles, data.m_nAttachmentIndex, "PISTOL MUZZLE");
}
DECLARE_CLIENT_EFFECT("FireEvent", FireEventCallBack);
...
void C_BaseAnimating::FireEvent( const Vector& origin, const QAngle& angles, int event, const char *options )
{
	...
	case CL_EVENT_FOOTSTEP_LEFT:
		{
			if (this->IsPlayer() && cvar->FindVar("oc_player_draw_body")->GetInt())
				break;
#ifndef HL2MP
			char pSoundName[256];
	...
	// Eject brass
	case CL_EVENT_EJECTBRASS1://For one barrel weapons
		if ( m_Attachments.Count() > 0 )
		{
			if ( MainViewOrigin().DistToSqr( GetAbsOrigin() ) < (256 * 256) )
			{
				...
				if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
				{
					if (!V_strcmp(C_BaseEntity::GetClassname(), "viewmodel"))
					{
						C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();

						if (pPlayer && pPlayer->GetActiveWeapon() && pPlayer->GetActiveWeapon()->GetAttachment(2, attachOrigin, attachAngles))
						{
							tempents->EjectBrass(attachOrigin, attachAngles, GetAbsAngles(), atoi(options), pPlayer->GetActiveWeapon());
						}
					}
					else
					{
						C_BaseCombatCharacter *pOwner = ToBaseCombatCharacter(GetOwnerEntity());
						if (pOwner)
							tempents->EjectBrass(attachOrigin, attachAngles, GetAbsAngles(), atoi(options), pOwner);
					}
				}
				else
				{
					...
					{
						//if (C_BaseEntity::GetClassname() == AllocPooledString("viewmodel"))
						if (!V_strcmp(C_BaseEntity::GetClassname(), "viewmodel"))
						{
							C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
							if (pPlayer)
								tempents->EjectBrass(attachOrigin, attachAngles, GetAbsAngles(), atoi(options), pPlayer);
						}
						else
						{
							C_BaseCombatCharacter *pOwner = ToBaseCombatCharacter(GetOwnerEntity());
		...
		break;
	case CL_EVENT_EJECTBRASS_MAG://For one barrel weapons
		if (m_Attachments.Count() > 0)
		{
			if (MainViewOrigin().DistToSqr(GetAbsOrigin()) < (256 * 256))
			{
				...
				if (GetAttachment("muzzle", attachOrigin, attachAngles))
				{
					Vector	vecOrigin, vecAngles;
					QAngle	angAngles;

					GetAttachment("muzzle", vecOrigin, angAngles);
					::FormatViewModelAttachment(vecOrigin, false);
				}
			}
		}
		break;
	case CL_EVENT_EJECTBRASS_MAG_NPC://For one barrel weapons
		if (m_Attachments.Count() > 0)
		{
			if (MainViewOrigin().DistToSqr(GetAbsOrigin()) < (256 * 256))
			{
				...
				if (GetAttachment("muzzle", attachOrigin, attachAngles))
				{
					Vector	vecOriginN, vecAnglesN;
					QAngle	angAnglesN;

					GetAttachment("muzzle", vecOriginN, angAnglesN);
				}
			}
		}
		break;
		// Eject brass2//OverCharged
	case CL_EVENT_EJECTBRASS_FIRST_BARREL://For two barrel weapons
		if (m_Attachments.Count() > 0)
		{
			if (MainViewOrigin().DistToSqr(GetAbsOrigin()) < (256 * 256))
			{
		...
		break;
		// Eject brass2//OverCharged
	case CL_EVENT_EJECTBRASS_SECOND_BARREL://For two barrel weapons
		if (m_Attachments.Count() > 0)
		{
			if (MainViewOrigin().DistToSqr(GetAbsOrigin()) < (256 * 256))
			{
				Vector attachOrigin;
				QAngle attachAngles;
				if (GetAttachment(4, attachOrigin, attachAngles))
				{
					tempents->EjectBrass(attachOrigin, attachAngles, GetAbsAngles(), atoi(options));
				}
			}
	...
	case AE_MUZZLEFLASH_SECONDARY:
	{
		// Send out the effect for a player
		DispatchMuzzleSecondaryEffect(options, true);
		break;
	}

	case AE_MUZZLEFLASH_CHARGE:
	{
		// Send out the effect for a player
		DispatchMuzzleChargeEffect(options);
		break;
	}
...
void C_BaseAnimating::RagdollMoved( void ) 
{
	...
	/*int Y = random->RandomInt(1,25);
	if ((cvar->FindVar("oc_ragdoll_show_blood")->GetInt() == 1) && (Y == 15))
	{
		trace_t tr;
		Vector damagePos;
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				//DevMsg("game.material: %s", pSurfaceData->game.material);
				//DevMsg("game.material: %s", this->GetModelName());
				if ((Q_strcmp(STRING(GetModelName()), "models/zombie/classic.mdl") == 0) || (Q_strcmp(STRING(GetModelName()), "models/zombie/fast.mdl") == 0) ||
					(Q_strcmp(STRING(GetModelName()), "models/zombie/classic_legs.mdl") == 0) ||
					(Q_strcmp(STRING(GetModelName()), "models/zombie/classic_torso.mdl") == 0) ||
					(Q_strcmp(STRING(GetModelName()), "models/zombie/poison.mdl") == 0) ||
		...
		if (BloodColor == 0)
			DispatchParticleEffect("blood_impact_red_01", m_pRagdoll->GetRagdollOrigin() + Vector(random->RandomFloat(-9, 9), random->RandomFloat(-9, 9), random->RandomFloat(-9, 9)), this->GetAbsAngles());
		else if (BloodColor == 1)
			DispatchParticleEffect("blood_impact_yellow_01", m_pRagdoll->GetRagdollOrigin() + Vector(random->RandomFloat(-9, 9), random->RandomFloat(-9, 9), random->RandomFloat(-9, 9)), this->GetAbsAngles());
		else if (BloodColor == 2)
			DispatchParticleEffect("blood_impact_green_01", m_pRagdoll->GetRagdollOrigin() + Vector(random->RandomFloat(-9, 9), random->RandomFloat(-9, 9), random->RandomFloat(-9, 9)), this->GetAbsAngles());

		if (cvar->FindVar("oc_blood_displays_on_all_normals")->GetInt() == 0)
		{
			UTIL_TraceLine(m_pRagdoll->GetRagdollOrigin(), m_pRagdoll->GetRagdollOrigin() + Vector(random->RandomFloat(-120, 120), random->RandomFloat(-120, 120), random->RandomFloat(-120, 120)), MASK_SOLID_BRUSHONLY & ~CONTENTS_GRATE, this, COLLISION_GROUP_NONE, &tr);
		}
		else if (cvar->FindVar("oc_blood_displays_on_all_normals")->GetInt() == 1)
		{
			UTIL_TraceLine(m_pRagdoll->GetRagdollOrigin(), m_pRagdoll->GetRagdollOrigin() + Vector(random->RandomFloat(-120, 120), random->RandomFloat(-120, 120), random->RandomFloat(-120, 120)), MASK_ALL, this, COLLISION_GROUP_INTERACTIVE, &tr);//COLLISION_GROUP_INTERACTIVE

		}
		//debugoverlay->AddLineOverlay(damagePos, tr.endpos + damagePos, 0, 255, 0, false, 10);
		int RandBlood = RandomInt(1, 10);
		if (BloodColor == 0)
		{
			if (RandBlood == 5)
			{
				UTIL_BloodDecalTrace(&tr, 0);
			}
		}
		else if (BloodColor == 1)//((char)ptexdata->game.material == CHAR_TEX_ALIENFLESH)
		{
			if (RandBlood == 5)
			{
		...
		else if (BloodColor == 2)//((char)ptexdata->game.material == CHAR_TEX_ANTLION)
		{
			if (RandBlood == 5)
			{
...
bool C_BaseAnimating::CheckForcedRagdollName()
{
	std::string thisName = GetClassname();
	std::transform(thisName.begin(), thisName.end(), thisName.begin(),
		[](unsigned char c){ return std::tolower(c); });

	const char* ClassName = thisName.c_str();

	if (//FStrEq(STRING(ClassName), "npc_hydra")
		/*V_strstr(STRING(ClassName), "c_npc_manhack") ||
		V_strstr(STRING(ClassName), "c_npc_turret_floor") ||
		V_strstr(STRING(ClassName), "npc_scanner") ||
		V_strstr(STRING(ClassName), "npc_rollermine") ||*/
		V_strstr(STRING(ClassName), "npc_hydra"))
	{
		return true;
	}
	return false;
...
C_BaseAnimating *C_BaseAnimating::CreateRagdollCopy(bool forced)
{
	ForcedRagdollOnClient = false;
	...
	/*if (!forced && (pRagdoll->GetFlags() & FL_ONFIRE && cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && cvar->FindVar("oc_ragdoll_serverside_interrupt")->GetBool() == false))
	{
		pRagdoll->Remove();
		return NULL;
	}*/
...
C_BaseAnimating *C_BaseAnimating::BecomeRagdollOnClientForced()
{
	ForcedRagdollOnClient = true;
	MoveToLastReceivedPosition(true);
	GetAbsOrigin();
	C_BaseAnimating *pRagdoll = CreateRagdollCopy(true);
	...
	pRagdoll->InitAsClientRagdoll(boneDelta0, boneDelta1, currentBones, boneDt, true);
...
{
	/*ForcedRagdollOnClient = CheckForcedRagdollName();

	if (!ForcedRagdollOnClient)
	{
		//if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && cvar->FindVar("oc_ragdoll_serverside_interrupt")->GetBool() == false)//canBecomeClientSideRagdoll == false)
			return nullptr;
	}*/
...
bool C_BaseAnimating::InitAsClientRagdoll( const matrix3x4_t *pDeltaBones0, const matrix3x4_t *pDeltaBones1, const matrix3x4_t *pCurrentBonePosition, float boneDt, bool bFixedConstraints, bool forced )
{

	/*if (!forced && (cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && cvar->FindVar("oc_ragdoll_serverside_interrupt")->GetBool() == false && !this->GetOwnerEntity()->IsPlayer()))
	{
		return false;
	}*/
	...
	/*if (!V_strcmp(hdr->pszName(), V_UnqualifiedFileName(cvar->FindVar("oc_playermodel_suit")->GetString())))
		Msg("Not equal \n");
	else if (V_strcmp(hdr->pszName(), V_UnqualifiedFileName(cvar->FindVar("oc_playermodel_suit")->GetString())))
		Msg("Equal \n");*/

	/*char entModelName[sizeof(V_UnqualifiedFileName(hdr->pszName()))];

	Q_snprintf(entModelName, sizeof(entModelName), "%s", V_UnqualifiedFileName(hdr->pszName()));
	V_strlower(entModelName);



	char plrModelName[sizeof(V_UnqualifiedFileName(cvar->FindVar("oc_playermodel_suit")->GetString()))];

	Q_snprintf(plrModelName, sizeof(plrModelName), "%s", V_UnqualifiedFileName(cvar->FindVar("oc_playermodel_suit")->GetString()));
	V_strlower(plrModelName);

	char plrNoSuitModelName[sizeof(V_UnqualifiedFileName(cvar->FindVar("oc_playermodel_nosuit")->GetString()))];

	Q_snprintf(plrNoSuitModelName, sizeof(plrNoSuitModelName), "%s", V_UnqualifiedFileName(cvar->FindVar("oc_playermodel_nosuit")->GetString()));
	V_strlower(plrNoSuitModelName);*/


	/*Msg("hdr->pszName() %s \n", entModelName);

	Msg("oc_playermodel_suit %s \n", plrModelName);

	if (!V_strcmp(entModelName, plrModelName))
	Msg("equal \n");
	else if (V_strcmp(entModelName, plrModelName))
	Msg("not Equal \n");*/
	//C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	/*if (C_BasePlayer::GetLocalPlayer()->IsSuitEquipped())
	{
		if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && V_strcmp(entModelName, plrModelName))// || entModelName != plrNoSuitModelName))
			return false;
	}
	else
	{
		if (cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && V_strcmp(entModelName, plrNoSuitModelName))// || entModelName != plrNoSuitModelName))
			return false;
	}*/
...
#endif
	//cvar->FindVar("oc_ragdoll_serverside_interrupt")->SetValue(0);
	return true;
}

bool C_BaseAnimating::InitAsClientRagdollDeath(const matrix3x4_t *pDeltaBones0, const matrix3x4_t *pDeltaBones1, const matrix3x4_t *pCurrentBonePosition, float boneDt, bool bFixedConstraints)
{
	CStudioHdr *hdr = GetModelPtr();
...
void C_BaseAnimating::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_nRenderFX == kRenderFxRagdoll && !m_builtRagdoll )
	{
		...
		if (!ForcedRagdollOnClient && (GetFlags() & FL_ONFIRE && cvar->FindVar("oc_ragdoll_serverside")->GetInt() != 0 && cvar->FindVar("oc_ragdoll_serverside_interrupt")->GetBool() == false))
		{
			return;
		}
		else
...
float C_BaseAnimating::SetNonThresholdPoseParameter(CStudioHdr *pStudioHdr, const char *szName, float flValue)
{
	return SetNonThresholdPoseParameter(pStudioHdr, LookupPoseParameter(pStudioHdr, szName), flValue);
}

float C_BaseAnimating::SetNonThresholdPoseParameter(CStudioHdr *pStudioHdr, int iParameter, float flValue)
{
	if (!pStudioHdr)
	{
...
void C_BaseAnimating::DoMuzzleFlashOVR(int attachmentIndex, int flashType, int r, int g, int b, const char *name)
{
	//DevMsg("DoMuzzleFlashOVR\n");
	tempents->MuzzleFlash(name, r, g, b, flashType, GetRefEHandle(), attachmentIndex, false);
}
