...
//#include "c_baseanimating.h"//////////OverCharged
#include "tracer_filter.h"//////////OverCharged
...
ConVar oc_muzzleflash_light_npc("oc_muzzleflash_light_npc", "0", FCVAR_ARCHIVE);
ConVar oc_muzzleflash_light_plr("oc_muzzleflash_light_plr", "0", FCVAR_ARCHIVE);

ConVar oc_particle_ejectbrass_smoke("oc_particle_ejectbrass_smoke", "0", FCVAR_ARCHIVE);
ConVar oc_particle_muzzleflashes("oc_particle_muzzleflashes", "0", FCVAR_ARCHIVE);
ConVar oc_particle_tracers("oc_particle_tracers", "0", FCVAR_ARCHIVE);
ConVar oc_particle_misc("oc_particle_misc", "0", FCVAR_ARCHIVE);
ConVar oc_particle_explosions("oc_particle_explosions", "0", FCVAR_ARCHIVE);
ConVar oc_particle_other_weapons("oc_particle_other_weapons", "0", FCVAR_ARCHIVE);

ConVar oc_InScope_weapon_muzzleflash_position_x("oc_InScope_weapon_muzzleflash_position_x", "0");
ConVar oc_InScope_weapon_muzzleflash_position_y("oc_InScope_weapon_muzzleflash_position_y", "0");
ConVar oc_InScope_weapon_muzzleflash_position_z("oc_InScope_weapon_muzzleflash_position_z", "0");

ConVar oc_InScope_weapon_tracers_position_x("oc_InScope_weapon_tracers_position_x", "0");
ConVar oc_InScope_weapon_tracers_position_y("oc_InScope_weapon_tracers_position_y", "0");
ConVar oc_InScope_weapon_tracers_position_z("oc_InScope_weapon_tracers_position_z", "0");

ConVar oc_InScope_weapon_muzzleflash_angle_constant("oc_InScope_weapon_muzzleflash_angle_constant", "0");
ConVar oc_InScope_weapon_muzzleflash_angle_z("oc_InScope_weapon_muzzleflash_angle_z", "0");
...
///////////////////////////////OverCharged/////////////////////
	...
	CLIENTEFFECT_MATERIAL("effects/muzzleflash3_noz")
	CLIENTEFFECT_MATERIAL("effects/muzzleflash4_noz")
	CLIENTEFFECT_MATERIAL("effects/ar2_muzzle1")
	CLIENTEFFECT_MATERIAL("effects/combinemuzzle_smoke")	
	CLIENTEFFECT_MATERIAL("effects/strider_muzzle")
	CLIENTEFFECT_MATERIAL("effects/gunshiptracer")
	CLIENTEFFECT_MATERIAL("effects/tracer_cap")
	CLIENTEFFECT_MATERIAL("effects/blueblackflash2")
	CLIENTEFFECT_MATERIAL("effects/blueblackflash1")
	CLIENTEFFECT_MATERIAL("effects/muzzleflash1")
	CLIENTEFFECT_MATERIAL("effects/muzzleflash2")
	CLIENTEFFECT_MATERIAL("effects/muzzleflash3")
	CLIENTEFFECT_MATERIAL("effects/muzzleflash4")
	CLIENTEFFECT_MATERIAL("effects/spark0")
	CLIENTEFFECT_MATERIAL("effects/combinemuzzle1")
	CLIENTEFFECT_MATERIAL("effects/combinemuzzle2")
//////////////////////////////////////////////////////////////
...
bool C_LocalTempEntity::IsActive( void )
{
	...
	if (pPlayer)
	{
	//if ( (life >= 0.65 ) && ( flags & FTENT_SMOKE ) && ( sv_ejectbrass_smoke.GetBool() == true ) && ( cl_ejectbrass.GetBool() == true ) )
	if ( (life >= 0.65f ) && ( flags & ( FTENT_COLLIDEWORLD | FTENT_FADEOUT | FTENT_GRAVITY |  FTENT_ROTATE | FTENT_SMOKE) ) && ( sv_ejectbrass_smoke.GetBool() == true ) && ( cl_ejectbrass.GetBool() == true ) )
	{
	CSmartPtr<CSimpleEmitter> pSimple = CSimpleEmitter::Create( "EjectBrass" );
	SimpleParticle *sParticle;
	pSimple->SetDrawBeforeViewModel( true );
	sParticle = (SimpleParticle *) pSimple->AddParticle( sizeof( SimpleParticle ), g_Mat_DustPuff[1], GetLocalOrigin() );//pSimple->GetPMaterial( "effects/spark2" ),//g_Mat_DustPuff[RandomInt(0,1)]
	//		if ( sParticle == NULL )
	//		return;

	sParticle->m_flLifetime		= 0.01f;
	sParticle->m_flDieTime		= 0.33f;
	//	sParticle->m_flDieTime		= D;
	sParticle->m_vecVelocity.Init();
	sParticle->m_vecVelocity = 2; //* random->RandomFloat( 8.0f, 64.0f );
	//	sParticle->m_vecVelocity[2] += random->RandomFloat( 4.0f, 16.0f );

	int color = 255;//random->RandomInt( 255, 300 );
	sParticle->m_uchColor[0]	= color;
	sParticle->m_uchColor[1]	= color;
	sParticle->m_uchColor[2]	= color;

	sParticle->m_uchStartAlpha	= random->RandomInt( 1500, 1800 );
	sParticle->m_uchEndAlpha	= 0;

	sParticle->m_uchStartSize	= random->RandomInt( 1.0f, 1.1f );
	sParticle->m_uchEndSize		= sParticle->m_uchStartSize * 2.5f;
	sParticle->m_flRoll			= random->RandomInt( 0, 360 );
	sParticle->m_flRollDelta	= random->RandomFloat( -0.5f, 0.5f );
	//	DispatchParticleEffect( "muzzle_smoke_shotgun_flash", GetLocalOrigin(), QAngle(0,0,0) );
	//	i++;
...
void CTempEnts::EjectBrass( const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type )
{
	...
	char shellName[128];
	V_snprintf(shellName, sizeof(shellName), "%s", AllocPooledString(GetActiveWeapon()->GetWpnData().iWeaponShellModel));

	const model_t *pModel = (model_t *)engine->LoadModel(shellName);
	...
	if (pPlayer)
	{
		if (pPlayer->GetActiveWeapon())
			pTemp->bounceSound = pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellBounceSound;
	}
	...
	if (type == 3)
		dir /= random->RandomFloat(3.0f, 4.0f);

	pTemp->SetVelocity(Vector(dir[0] + random->RandomFloat(-64, 64),
		dir[1] + random->RandomFloat(-64, 64),
		dir[2] + random->RandomFloat(0, 64)));

	if (oc_particle_ejectbrass_smoke.GetBool())
	{
		char shellParticle[128];
		V_snprintf(shellParticle, sizeof(shellParticle), "%s", AllocPooledString(GetActiveWeapon()->GetWpnData().iWeaponShellParticle));

		if (AllocPooledString(GetActiveWeapon()->GetWpnData().iWeaponShellParticle) != AllocPooledString("None"))
			pTemp->AddParticleEffect(shellParticle);
	}

	pTemp->die = gpGlobals->curtime + 3.0f + random->RandomFloat(0.0f, 1.0f);	// Add an extra 0-1 secs of life	
}
void CTempEnts::EjectBrass(const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type, C_BaseCombatCharacter *pOwner)
{
	if (cl_ejectbrass.GetBool() == false)
		return;
	if (!pOwner)
	...
	const model_t *pModel = (model_t *)engine->LoadModel(pOwner->GetActiveWeapon()->GetWpnData().iWeaponShellModel);
	...
	if (pOwner->GetActiveWeapon()->GetWpnData().iWeaponShellExhaustParticle != AllocPooledString("None") && pOwner->GetActiveWeapon()->WeaponGetWaterLevel() != 3)
		Shell_exhaust_Npc(pOwner);

	pTemp->bounceSound = pOwner->GetActiveWeapon()->GetWpnData().iWeaponShellBounceSound;
	...
	if (oc_particle_ejectbrass_smoke.GetBool() && pOwner->GetActiveWeapon()->WeaponGetWaterLevel() != 3)// && !plrIsUnderWater)//OverCharged
	{
		//char shellParticle[128];
		//V_snprintf(shellParticle, sizeof(shellParticle), "%s", AllocPooledString(pOwner->GetActiveWeapon()->GetWpnData().iWeaponShellParticle));

		if (AllocPooledString(pOwner->GetActiveWeapon()->GetWpnData().iWeaponShellParticle) != AllocPooledString("None"))
			pTemp->AddParticleEffect(pOwner->GetActiveWeapon()->GetWpnData().iWeaponShellParticle);
	}

	pTemp->die = gpGlobals->curtime + 3.0f + random->RandomFloat(0.0f, 1.0f);	// Add an extra 0-1 secs of life	
}
void CTempEnts::EjectBrass(const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type, C_BasePlayer *pPlayer)
{
	if (cl_ejectbrass.GetBool() == false)
		return;
	...
	//char shellName[128];
	//V_snprintf(shellName, sizeof(shellName), "%s", AllocPooledString(pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellModel));

	const model_t *pModel = (model_t *)engine->LoadModel(pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellModel);
	...
	if (oc_particle_ejectbrass_smoke.GetBool() && pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellExhaustParticle != AllocPooledString("None") && pPlayer->GetWaterLevel() != 3)
		Shell_exhaust(pPlayer);

	//char shellSnd[128];
	//V_snprintf(shellSnd, sizeof(shellSnd), "%s", AllocPooledString(pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellBounceSound));
	//DevMsg("%s \n", shellSnd);
	pTemp->bounceSound = pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellBounceSound;
	//DevMsg("%s \n", pTemp->bounceSound);
	pTemp->hitSound = BOUNCE_SHELL;
	...
	if (oc_particle_ejectbrass_smoke.GetBool() && pPlayer->GetWaterLevel() != 3)// && !plrIsUnderWater)//OverCharged
	{
		//char shellParticle[128];
		//V_snprintf(shellParticle, sizeof(shellParticle), "%s", AllocPooledString(pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellParticle));

		if (AllocPooledString(pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellParticle) != AllocPooledString("None"))
			pTemp->AddParticleEffect(pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellParticle);
	}

	pTemp->die = gpGlobals->curtime + 3.0f + random->RandomFloat(0.0f, 1.0f);	// Add an extra 0-1 secs of life	
}

void CTempEnts::EjectBrass(const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type, C_BaseCombatWeapon *pWeapon)
{
	if (cl_ejectbrass.GetBool() == false)
		return;
	...
	const model_t *pModel = (model_t *)engine->LoadModel(pWeapon->GetWpnData().iWeaponShellModel);
	...
	if (oc_particle_ejectbrass_smoke.GetBool() && pWeapon->GetWpnData().iWeaponShellExhaustParticle != AllocPooledString("None") && pWeapon->GetWaterLevel() != 3)
		Shell_exhaust(pWeapon);

	pTemp->bounceSound = pWeapon->GetWpnData().iWeaponShellBounceSound;
	...
	if (oc_particle_ejectbrass_smoke.GetBool() && pWeapon->GetWaterLevel() != 3)
	{
		if (AllocPooledString(pWeapon->GetWpnData().iWeaponShellParticle) != AllocPooledString("None"))
			pTemp->AddParticleEffect(pWeapon->GetWpnData().iWeaponShellParticle);
	}

	pTemp->die = gpGlobals->curtime + 3.0f + random->RandomFloat(0.0f, 1.0f);
}
...
//			type - 
//-----------------------------------------------------------------------------
void CTempEnts::EjectBrassMag(const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type)
{
	if (cl_ejectbrass.GetBool() == false)
		return;

	const model_t *pModel = m_Mag[type];
	...
	//pTemp->flags |= (FTENT_COLLIDEALL | FTENT_FADEOUT | FTENT_GRAVITY | FTENT_ROTATE | FTENT_COLLISIONGROUP | FTENT_COLLIDEPROPS);
	pTemp->flags |= (FTENT_FADEOUT | FTENT_GRAVITY);
	...
	//Face forward
	pTemp->SetAbsAngles(gunAngles);
	/*pTemp->SetSolid(SOLID_BBOX);
	pTemp->SetCollisionBounds(vec3_origin, vec3_origin);
	pTemp->SetCollisionGroup(COLLISION_GROUP_WEAPON);*/
	pTemp->SetRenderMode(kRenderNormal);
	pTemp->tempent_renderamt = 255;		// Set this for fadeout
	...
	//	DispatchParticleEffect( "weapon_muzzle_flash_sparks4", dir, gunAngles );
	//    DispatchParticleEffect( "slime_splash_01", pos1, angles );// slime_splash_01
	//	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	//    DispatchParticleEffect( "shell_eject_smoke_flash", PATTACH_POINT_FOLLOW, pPlayer->GetViewModel(), "1", true);

	if (oc_particle_ejectbrass_smoke.GetBool() == true)//OverCharged
	{
		pTemp->AddParticleEffect("shell_exhaust_smoke");
	}
	//	pTemp->AddParticleEffect("1trail");

	//pTemp->AddParticleEffect("shell_fx_main");
	//pTemp->AddParticleEffect("ExtinguisherJet");

	pTemp->die = gpGlobals->curtime + 5.0f + random->RandomFloat(0.0f, 1.0f);	// Add an extra 0-1 secs of life	
}
/////////////////////////////////////////////////////////////////////////////
void CTempEnts::CreateSphere(const Vector &pos1, const QAngle &gunAngles, int type, C_BasePlayer *pPlayer)
{
	if (cl_ejectbrass.GetBool() == false)
		return;
	...
	if (oc_particle_ejectbrass_smoke.GetBool() && pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellExhaustParticle != AllocPooledString("None") && pPlayer->GetWaterLevel() != 3)
		Shell_exhaust(pPlayer);
	pTemp->SetParent(pPlayer->GetViewModel());
	pTemp->SetOwnerEntity(pPlayer->GetViewModel());
	pTemp->FollowEntity(pPlayer->GetViewModel());
	pTemp->bounceSound = pPlayer->GetActiveWeapon()->GetWpnData().iWeaponShellBounceSound;
	pTemp->hitSound = BOUNCE_SHELL;
	pTemp->m_nBody = 0;
	pTemp->flags |= (FTENT_FADEOUT);
...
static ConVar oc_weapon_dual_pistols_tracers_x("oc_weapon_dual_pistols_tracers_x", "0.0", FCVAR_ARCHIVE);
static ConVar oc_weapon_dual_pistols_tracers_y("oc_weapon_dual_pistols_tracers_y", "0.0", FCVAR_ARCHIVE);
static ConVar oc_weapon_dual_pistols_tracers_z("oc_weapon_dual_pistols_tracers_z", "0.0", FCVAR_ARCHIVE);

static ConVar oc_adjust_tracers_dynamic_offset_x("oc_adjust_tracers_dynamic_offset_x", "0.0", FCVAR_REPLICATED);
static ConVar oc_adjust_tracers_dynamic_offset_y("oc_adjust_tracers_dynamic_offset_y", "0.0", FCVAR_REPLICATED);
static ConVar oc_adjust_tracers_dynamic_offset_z("oc_adjust_tracers_dynamic_offset_z", "0.0", FCVAR_REPLICATED);
static ConVar oc_adjust_tracers_dynamic_offset("oc_adjust_tracers_dynamic_offset", "0", FCVAR_REPLICATED);

static ConVar oc_adjust_weapon_tracers_offset("oc_adjust_weapon_tracers_offset", "0", FCVAR_REPLICATED);
static ConVar oc_adjust_weapon_tracers_offset_x("oc_adjust_weapon_tracers_offset_x", "0", FCVAR_REPLICATED);
static ConVar oc_adjust_weapon_tracers_offset_y("oc_adjust_weapon_tracers_offset_y", "0", FCVAR_REPLICATED);
static ConVar oc_adjust_weapon_tracers_offset_z("oc_adjust_weapon_tracers_offset_z", "0", FCVAR_REPLICATED);

static ConVar oc_test_adjust_weapon_tracers2_offset("oc_test_adjust_weapon_tracers2_offset", "0", FCVAR_REPLICATED);
static ConVar oc_test_adjust_weapon_tracers2_offset_x("oc_test_adjust_weapon_tracers2_offset_x", "0", FCVAR_REPLICATED);
static ConVar oc_test_adjust_weapon_tracers2_offset_y("oc_test_adjust_weapon_tracers2_offset_y", "0", FCVAR_REPLICATED);
static ConVar oc_test_adjust_weapon_tracers2_offset_z("oc_test_adjust_weapon_tracers2_offset_z", "0", FCVAR_REPLICATED);

extern ConVar oc_muzzle_tracer_x;
extern ConVar oc_muzzle_tracer_y;
extern ConVar oc_muzzle_tracer_z;

void CTempEnts::UpdateMuzzlePosition(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("TracerLocal", VPROF_BUDGETGROUP_PARTICLE_RENDERING);


	//CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("TracerLocal", hEntity, attachmentIndex, FLE_VIEWMODEL);
	CacheMuzzleFlashes();
	
	Vector			forward(1,0,0), up(0,0,1), right(0,1,0), Dest; //NOTENOTE: All coords are in local space
	Vector			forwardD(1, 0, 0), upD(0, 0, 1), rightD(0, 1, 0); //NOTENOTE: All coords are in local space

	//pSimple->SetDrawBeforeViewModel(true);
	Vector origin;
	QAngle angles, DestAngles;

	QAngle MAngle;
	Vector vecLightOrigin, ForTracers;
	Vector Dyn;
	QAngle DAngle;
	...
	if (pWeapon != NULL)
	{
		if (pWeapon->GetWpnData().ProjectilePosition == NULL)
			return;

		if ((cvar->FindVar("oc_adjust_weapon_tracers_offset")->GetInt() == 0))
		{
			PosX1 = pWeapon->GetWpnData().ProjectilePosition.x;
			PosY1 = pWeapon->GetWpnData().ProjectilePosition.y;
			PosZ1 = pWeapon->GetWpnData().ProjectilePosition.z;


			if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
			{
				DynX1 = pWeapon->GetWpnData().ParticleTracerPosition.x;
				DynY1 = pWeapon->GetWpnData().ParticleTracerPosition.y;
				DynZ1 = pWeapon->GetWpnData().ParticleTracerPosition.z;
			}
		}
		else if ((cvar->FindVar("oc_adjust_weapon_tracers_offset")->GetInt() == 1))
		{
			PosX1 = cvar->FindVar("oc_adjust_weapon_tracers_offset_x")->GetFloat();
			PosY1 = cvar->FindVar("oc_adjust_weapon_tracers_offset_y")->GetFloat();
			PosZ1 = cvar->FindVar("oc_adjust_weapon_tracers_offset_z")->GetFloat();
		}


		if ((cvar->FindVar("oc_adjust_tracers_dynamic_offset")->GetInt() == 0))
		{
			if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
			{
				DynX1 = pWeapon->GetWpnData().ParticleTracerPosition.x;
		...
		else if ((cvar->FindVar("oc_adjust_tracers_dynamic_offset")->GetInt() == 1))
		{
			if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
			{
				DynX1 = cvar->FindVar("oc_adjust_tracers_dynamic_offset_x")->GetFloat();
				DynY1 = cvar->FindVar("oc_adjust_tracers_dynamic_offset_y")->GetFloat();
				DynZ1 = cvar->FindVar("oc_adjust_tracers_dynamic_offset_z")->GetFloat();
			}
		}
		...
		//if ((FClassnameIs(pWeapon, "weapon_dual_pistols")))
		if (pWeapon->thisType == TYPE_DUAL_PISTOLS || pWeapon->thisType == TYPE_DUAL_MACHINEGUNS)
		{
			if (cvar->FindVar("oc_use_second_ballel")->GetInt() == 0)
			{
				int iAttachment = pVm->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment2);//pVm->LookupAttachment("muzzle1");
				pVm->GetAttachment(iAttachment, vecLightOrigin, MAngle);
			}
			else
			{
				int iAttachment2 = pVm->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment);//pVm->LookupAttachment("muzzle");
				pVm->GetAttachment(iAttachment2, vecLightOrigin, MAngle);
			}
		}
		else
		{
			int iAttachment = pVm->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment);//pVm->LookupAttachment("muzzle");
			...
			if (iAttachment < 0)
			{
				...
				{
					int iAttachment3 = 2;
					pVm->GetAttachment(iAttachment3, vecLightOrigin, MAngle);

					if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
					{
						pVm->GetAttachment(iAttachment3, Dyn, DAngle);
					}

				}
				else
				{
					pVm->GetAttachment(iAttachment0, vecLightOrigin, MAngle);

					if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
					{
						pVm->GetAttachment(iAttachment0, Dyn, DAngle);
					}
				}
			}
			else
			{
				...
				if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
				{
					pVm->GetAttachment(iAttachment, Dyn, DAngle);
				}
			}
		}


		AngleVectors(MAngle, &forward, &right, &up);

		if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
		{
			AngleVectors(DAngle, &forwardD, &rightD, &upD);
		}
		...
		if (pWeapon->thisType == TYPE_DUAL_PISTOLS || pWeapon->thisType == TYPE_DUAL_MACHINEGUNS)
		{
			...
			ForTracers = vecLightOrigin;
			ForTracers += forward * oc_weapon_dual_pistols_tracers_x.GetFloat();
			ForTracers += right * oc_weapon_dual_pistols_tracers_y.GetFloat();
			ForTracers += up * oc_weapon_dual_pistols_tracers_z.GetFloat();

			cvar->FindVar("oc_muzzle_tracer_x")->SetValue(ForTracers.x);
			cvar->FindVar("oc_muzzle_tracer_y")->SetValue(ForTracers.y);
			cvar->FindVar("oc_muzzle_tracer_z")->SetValue(ForTracers.z);

			cvar->FindVar("oc_muzzle_vector_x")->SetValue(vecLightOrigin.x);
			cvar->FindVar("oc_muzzle_vector_y")->SetValue(vecLightOrigin.y);
			cvar->FindVar("oc_muzzle_vector_z")->SetValue(vecLightOrigin.z);

			//engine->ServerCmd(VarArgs("MuzzleVectors %f %f %f %f %f %f\n", ForTracers.x, ForTracers.y, ForTracers.z, vecLightOrigin.x, vecLightOrigin.y, vecLightOrigin.z), false);

			if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
			{
				cvar->FindVar("oc_muzzle_dynamic_vector_x")->SetValue(vecLightOrigin.x);
				cvar->FindVar("oc_muzzle_dynamic_vector_y")->SetValue(vecLightOrigin.y);
				cvar->FindVar("oc_muzzle_dynamic_vector_z")->SetValue(vecLightOrigin.z);
			}
		}
		else
		{
			...
			ForTracers = vecLightOrigin;
			ForTracers += forward * PosX1;
			ForTracers += right * PosY1;
			ForTracers += up *PosZ1;
	...
	if (!(pWeapon->thisType == TYPE_DUAL_PISTOLS) && !(pWeapon->thisType == TYPE_DUAL_MACHINEGUNS))
	{
		...
		else if (pWeapon->IsScopeSighted())// || (cvar->FindVar("oc_state_IRsight_on")->GetInt() == 1))
		{
			Vector vForward, vRight, vUp, vThrowPos, Muzzle;//MY(oc_InScope_weapon_oicw_muzzleflash_position_x.GetFloat(), oc_InScope_weapon_oicw_muzzleflash_position_y.GetFloat(), oc_InScope_weapon_oicw_muzzleflash_position_z.GetFloat());
			QAngle pAngles;
			pPlayer->EyeVectors(&vForward, &vRight, &vUp);
			pAngles.x = pPlayer->EyeAngles().x;// *oc_InScope_weapon_muzzleflash_angle_constant.GetFloat();
			pAngles.y = pPlayer->EyeAngles().y;// *oc_InScope_weapon_muzzleflash_angle_constant.GetFloat();
			pAngles.z = oc_InScope_weapon_muzzleflash_angle_z.GetFloat();

			vThrowPos = pPlayer->EyePosition();
			//vThrowPos = Muzzle2;

			vThrowPos += vForward * oc_InScope_weapon_tracers_position_x.GetFloat();
			vThrowPos += vRight * oc_InScope_weapon_tracers_position_y.GetFloat();
			vThrowPos += vUp * oc_InScope_weapon_tracers_position_z.GetFloat();

			cvar->FindVar("oc_muzzle_tracer_x")->SetValue(vThrowPos.x);
			cvar->FindVar("oc_muzzle_tracer_y")->SetValue(vThrowPos.y);
			cvar->FindVar("oc_muzzle_tracer_z")->SetValue(vThrowPos.z);
			
			cvar->FindVar("oc_muzzle_vector_x")->SetValue(vThrowPos.x);
			cvar->FindVar("oc_muzzle_vector_y")->SetValue(vThrowPos.y);
			cvar->FindVar("oc_muzzle_vector_z")->SetValue(vThrowPos.z);

			//engine->ServerCmd(VarArgs("MuzzleVectors %f %f %f %f %f %f\n", vThrowPos.x, vThrowPos.y, vThrowPos.z, vThrowPos.x, vThrowPos.y, vThrowPos.z), false);
			//engine->ClientCmd_Unrestricted
				//engine->ServerCmdKeyValues
		}

		if ((cvar->FindVar("oc_weapons_enable_dynamic_bullets")->GetInt() == 1))
		{
			cvar->FindVar("oc_muzzle_dynamic_vector_x")->SetValue(Dyn.x);
			cvar->FindVar("oc_muzzle_dynamic_vector_y")->SetValue(Dyn.y);
			cvar->FindVar("oc_muzzle_dynamic_vector_z")->SetValue(Dyn.z);
		}
	}
...
void CTempEnts::MuzzleFlashSecondary(const char *name, int colR, int colG, int colB, int type, ClientEntityHandle_t hEntity, int attachmentIndex, bool firstPerson)
{
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if (pPlayer && firstPerson)
	{
		MuzzleLightCallBack();
	}
	...
	if (firstPerson)
	{
		if (oc_muzzleflash_light_plr.GetInt() == 1)
			MuzzleflashCreatePlayerLight(hEntity);

		if (oc_particle_muzzleflashes.GetInt())
		{
			MuzzleFlash_General_Player_Secondary_Particle_Effect(name, colR, colG, colB, hEntity, attachmentIndex);
		}
		else
		{
			CEffectData data;
			if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
				data.m_hEntity = pWeapon;
			else
				data.m_hEntity = hEntity;

			data.m_nAttachmentIndex = attachmentIndex;

			data.m_flScale = 1.0f;
			data.m_fFlags = type;
			data.m_bCustomColors = !pWeapon->GetWpnData().oldFlashTypeTP;
	...
	{
		if (oc_muzzleflash_light_npc.GetInt() == 1)
			MuzzleflashCreateNPCLight(hEntity);

		if (oc_particle_muzzleflashes.GetInt())
		{
			MuzzleFlash_General_NPC_Secondary_Particle_Effect(name, colR, colG, colB, hEntity, attachmentIndex);
		}
		else
		{
			CEffectData data;
...
//			firstPerson - 
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash(const char *name, int colR, int colG, int colB, int type, ClientEntityHandle_t hEntity, int attachmentIndex, bool firstPerson)
{
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if (pPlayer && firstPerson)
	{
	...
	if (firstPerson)
	{
		...
		//if (oc_particle_muzzleflashes.GetInt() == 1)
		if (/*type == -1 && */oc_particle_muzzleflashes.GetInt())
		{
			MuzzleFlash_General_Player_Particle_Effect(name, colR, colG, colB, hEntity, attachmentIndex);
		}
		else
		{
			...
			int iAttachment = attachmentIndex;

			if (pWeapon)
			{
				if (pWeapon->iMuzzle == 2)
				{
					if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
						iAttachment = pWeapon->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment2);
					else
						iAttachment = pPlayer->GetViewModel()->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachment2);//pPlayer->GetViewModel()->LookupAttachment("muzzle1");
				}
				
				if (pWeapon->m_bSilenced)//(cvar->FindVar("oc_is_weapon_silenced")->GetInt() == 1)
				{
					if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
						iAttachment = pWeapon->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachmentSilencer);
					else
						iAttachment = pPlayer->GetViewModel()->LookupAttachment(pWeapon->GetWpnData().MuzzleAttachmentSilencer);
				}
			}
			...
			data.m_flScale = 1.0f;
			data.m_fFlags = type;
			data.m_bCustomColors = pWeapon->GetWpnData().oldFlashTypeTP;// !pWeapon->GetWpnData().oldFlashTypeTP;
			DispatchEffect("MuzzleFlash", data);
			/*Vector pos1 = Vector(0, 0, 0);
			QAngle angles = QAngle(0,0,0);
			tempents->MuzzleFlash(pos1, angles, type, hEntity, firstPerson, attachmentIndex);*/
		}
	...
	else
	{
		...
		if (/*type == -1 && */oc_particle_muzzleflashes.GetInt())
		{
			MuzzleFlash_General_NPC_Particle_Effect(name, colR, colG, colB, hEntity, attachmentIndex);
		}
		else
		{
...
void CTempEnts::MuzzleflashCreatePlayerLight(ClientEntityHandle_t hEntity)
{
	...
	//pEnt->CreateMuzzleFlashLight();

	C_BaseViewModel *pVM = (C_BaseViewModel*)(pEnt);

	if (pVM)
	{
		C_BasePlayer *pPlayer = ToBasePlayer(pVM->GetOwner());

		if (pPlayer && pPlayer->GetActiveWeapon() && pPlayer->GetActiveWeapon()->GetWpnData().AllowMuzzleFlashDLight)
		{
			pPlayer->dlightKey = index;

			Vector origin;
			pPlayer->GetActiveWeapon()->GetAttachment(pPlayer->GetActiveWeapon()->m_bSilenced ? "muzzle_sil" : pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightAttachment, origin);

			pPlayer->mfEl = effects->CL_AllocDlight(index);
			//dlight_t *el = effects->CL_AllocDlight(index);

			float radius = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightRadius * (engine->MapHasHDRLighting() ? 1 : 0.3);
			int red = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightR;
			int green = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightG;
			int blue = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightB;
			int exp = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightExponent;
			...
			bool illum = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightNoModelIllumination;
			float decay = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightDecayTime;
			float lifeTime = pPlayer->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightLifeTime;

			if (pPlayer->mfEl)
			{
				pPlayer->mfEl->key = index;

				pPlayer->mfEl->origin = origin;
				pPlayer->mfEl->color.r = red;
				pPlayer->mfEl->color.g = green;
				pPlayer->mfEl->color.b = blue;
				pPlayer->mfEl->color.exponent = exp;
					
				if (illum)
					pPlayer->mfEl->flags |= DLIGHT_NO_MODEL_ILLUMINATION;

				pPlayer->mfEl->radius = random->RandomInt(radius - 30.f, radius + 30.f);
				pPlayer->mfEl->decay = pPlayer->mfEl->radius / decay;// /0.3f
				pPlayer->mfEl->die = gpGlobals->curtime + lifeTime;
			}
		}	
	}
}

void CTempEnts::MuzzleflashCreateNPCLight(ClientEntityHandle_t hEntity)
{
	C_BaseEntity *pEnt = ClientEntityList().GetBaseEntityFromHandle(hEntity);
	if (!pEnt)
		return;

	C_BaseCombatCharacter *pCharacter = ToBaseCombatCharacter(pEnt->GetOwnerEntity());

	int index = pEnt->index;

	if (pCharacter && pCharacter->IsNPC())
	{
		if (pCharacter->GetActiveWeapon() && pCharacter->GetActiveWeapon()->GetWpnData().AllowMuzzleFlashDLightNpc)
		{
			Vector origin;
			pCharacter->GetActiveWeapon()->GetAttachment(pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightAttachmentNpc, origin);
			dlight_t *el = effects->CL_AllocDlight(index);

			float radius = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightRadiusNpc * (engine->MapHasHDRLighting() ? 1 : 0.3); //(engine->MapHasHDRLighting() ? 2 : 1);
			int red = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightRNpc;
			int green = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightGNpc;
			int blue = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightBNpc;
			int exp = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightExponentNpc;
			bool illum = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightNoModelIlluminationNpc;
			float decay = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightDecayTimeNpc;
			float lifeTime = pCharacter->GetActiveWeapon()->GetWpnData().iMuzzleFlashLightLifeTimeNpc;

			if (el)
			{
				el->origin = origin;// pCharacter->GetActiveWeapon()->GetAbsOrigin();
				el->key = index;

				el->color.r = red;
				el->color.g = green;
				el->color.b = blue;
				el->color.exponent = exp;

				if (illum)
					el->flags |= DLIGHT_NO_MODEL_ILLUMINATION;

				el->radius = random->RandomInt(radius - 30.f, radius + 30.f);
				el->decay = el->radius / decay;// /0.3f
...
void CTempEnts::MuzzleflashCreateWeaponLight(ClientEntityHandle_t hEntity)
{
	C_BaseCombatWeapon *pWeapon = static_cast<C_BaseCombatWeapon *>(ClientEntityList().GetBaseEntityFromHandle(hEntity));
	if (!pWeapon)
		return;

	int index = pWeapon->index;

	if (pWeapon)
	{
		if (pWeapon->GetWpnData().AllowMuzzleFlashDLightNpc)
		{
			Vector origin;
			pWeapon->GetAttachment(pWeapon->GetWpnData().iMuzzleFlashLightAttachmentNpc, origin);
			dlight_t *el = effects->CL_AllocDlight(index);

			float radius = pWeapon->GetWpnData().iMuzzleFlashLightRadiusNpc * (engine->MapHasHDRLighting() ? 1 : 0.3); //(engine->MapHasHDRLighting() ? 2 : 1);
			int red = pWeapon->GetWpnData().iMuzzleFlashLightRNpc;
			int green = pWeapon->GetWpnData().iMuzzleFlashLightGNpc;
			int blue = pWeapon->GetWpnData().iMuzzleFlashLightBNpc;
			int exp = pWeapon->GetWpnData().iMuzzleFlashLightExponentNpc;
			bool illum = pWeapon->GetWpnData().iMuzzleFlashLightNoModelIlluminationNpc;
			float decay = pWeapon->GetWpnData().iMuzzleFlashLightDecayTimeNpc;
			float lifeTime = pWeapon->GetWpnData().iMuzzleFlashLightLifeTimeNpc;

			if (el)
			{
				el->origin = origin;// pWeapon->GetAbsOrigin();
				el->key = index;
...
//			type - 
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash( const Vector& pos1, const QAngle& angles, int type, ClientEntityHandle_t hEntity, bool firstPerson, int attachmentIndex )
{
#ifdef CSTRIKE_DLL
...
void CTempEnts::LevelInit()
{
	...
	m_pShells[3] = (model_t *)engine->LoadModel( "models/weapons/MAGS/w_rif_ar2_small_mag.mdl") ;

	//m_Mag[3] = (model_t *)engine->LoadModel("models/weapons/unloaded/smg_mac10_mag.mdl");
	m_Mag[5] = (model_t *)engine->LoadModel("models/weapons/MAGS/w_smg_ump45_mag.mdl");
	m_Mag[4] = (model_t *)engine->LoadModel("models/weapons/MAGS/w_pist_p250_mag.mdl");
	m_Mag[3] = (model_t *)engine->LoadModel("models/weapons/MAGS/w_mach_m249_mag.mdl");
#endif
...
void CTempEnts::Init (void)
{
	...
	m_Mag[3] = NULL;//OverCharged
	m_Mag[4] = NULL;//OverCharged
	m_Mag[5] = NULL;//OverCharged
...
{
	m_Material_MuzzleFlash_Player[0] = ParticleMgr()->GetPMaterial("effects/muzzleflash1");
	m_Material_MuzzleFlash_Player[1] = ParticleMgr()->GetPMaterial("effects/muzzleflash2");
	m_Material_MuzzleFlash_Player[2] = ParticleMgr()->GetPMaterial("effects/muzzleflash3");
	m_Material_MuzzleFlash_Player[3] = ParticleMgr()->GetPMaterial("effects/muzzleflash4");

	m_Material_MuzzleFlash_NPC[0] = ParticleMgr()->GetPMaterial("effects/muzzleflash1");
	m_Material_MuzzleFlash_NPC[1] = ParticleMgr()->GetPMaterial("effects/muzzleflash2");
	m_Material_MuzzleFlash_NPC[2] = ParticleMgr()->GetPMaterial("effects/muzzleflash3");
	m_Material_MuzzleFlash_NPC[3] = ParticleMgr()->GetPMaterial("effects/muzzleflash4");

	m_Material_Combine_MuzzleFlash_Player[0] = ParticleMgr()->GetPMaterial("effects/combinemuzzle1_noz");
	m_Material_Combine_MuzzleFlash_Player[1] = ParticleMgr()->GetPMaterial("effects/combinemuzzle2_noz");

	m_Material_Combine_MuzzleFlash_NPC[0] = ParticleMgr()->GetPMaterial("effects/combinemuzzle1");
	m_Material_Combine_MuzzleFlash_NPC[1] = ParticleMgr()->GetPMaterial("effects/combinemuzzle2");
}

void CTempEnts::MuzzleFlash_SMG2_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	//////////////////////////////////////////////////////////////////////////////////////// мнбши лернд /////////////////////////////////////////////////////////////////////////////
	VPROF_BUDGET("MuzzleFlash_SMG2_Player_Particle_Effect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_SMG2_Player_Particle_Effect", hEntity, attachmentIndex, FLE_VIEWMODEL);
	CacheMuzzleFlashes();

	Vector			forward(7, 0, 0), right(0, 1.3, -1.3), offset; //NOTENOTE: All coords are in local space

	Vector			forward0(30, 0, 0);
	...
	//	if (cl_tracer_type.GetInt() == 2)
	//	{

	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if (!pPlayer)
		return;
	//  DispatchParticleEffect( "weapon_muzzle_flash_rifle_and_smg", PATTACH_POINT_FOLLOW, pPlayer->GetViewModel(), "muzzle", false);
	DispatchParticleEffect("pd2_muzzle_normal_oicw", PATTACH_POINT_FOLLOW, pPlayer->GetViewModel(), "muzzle", false);

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
}
void CTempEnts::MuzzleFlash_SMG2_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash_AK47_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	//Draw the cloud of fire
	FX_MuzzleEffectAttached(1.0f, hEntity, attachmentIndex, NULL, true);
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash_AK47_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("MuzzleFlash_AK47_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_AK47_Player", hEntity, attachmentIndex, FLE_VIEWMODEL);
...
void CTempEnts::MuzzleFlash_Shotgun_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	/*CSmartPtr<CTrailParticles> pTrails = CTrailParticles::Create("MuzzleFlash_Shotgun_Player");
	...
	{
		pTrailParticle = (TrailParticle *)pTrails->AddParticle(sizeof(TrailParticle), m_Material_MuzzleFlash_Player[random->RandomInt(0, 3)], origin);
...
static ConVar oc_test_muzzle_sphere_x("oc_test_muzzle_sphere_x", "1", FCVAR_ARCHIVE);
static ConVar oc_test_muzzle_sphere_y("oc_test_muzzle_sphere_y", "1", FCVAR_ARCHIVE);
static ConVar oc_test_muzzle_sphere_z("oc_test_muzzle_sphere_z", "1", FCVAR_ARCHIVE);
void CTempEnts::MuzzleFlash_General_Player_Particle_Effect(const char *name, int colR, int colG, int colB, ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("MuzzleFlash_General_Player_Particle_Effect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_General_Player_Particle_Effect", hEntity, attachmentIndex, FLE_VIEWMODEL);
	CacheMuzzleFlashes();

	if (name == NULL || (name && (!V_strlen(name) || FStrEq(name, "None") || FStrEq(name, "none"))))
		return;

	Vector			forward(7, 0, 0), right(0, 1.3, -1.3), offset; //NOTENOTE: All coords are in local space

	Vector			forward0(30, 0, 0), light;
	...
	if (cvar->FindVar("oc_state_InSecondFire")->GetInt() == 1)
	{
		...
		vThrowPos += vForward * oc_InScope_weapon_muzzleflash_position_x.GetFloat();
		vThrowPos += vRight * oc_InScope_weapon_muzzleflash_position_y.GetFloat();
		vThrowPos += vUp * oc_InScope_weapon_muzzleflash_position_z.GetFloat();

		DispatchParticleEffect(name, vThrowPos, pAngles, pVM);

		/*if (pPlayer->GetWaterLevel() != 3)
		{
			DispatchParticleEffect(name, vThrowPos, pAngles, pPlayer->GetViewModel());
		}
		else
		{
			const char *nameUW = pWeapon->GetWpnData().iMuzzleFlashUnderWaterType;

			if (nameUW == NULL || (nameUW && (!V_strlen(nameUW) || V_strcmp(nameUW, "None") || V_strcmp(nameUW, "none"))))
				return;

			Msg("Error: %s does't have underwater particle definition in script fire \n", pWeapon->GetClassname());

			DispatchParticleEffect(nameUW, vThrowPos, pAngles, pPlayer->GetViewModel());
		}*/
	}
	if (cvar->FindVar("oc_state_InSecondFire")->GetInt() == 0)
	{
		int iAttachment = attachmentIndex;
		...
		{
			char nameMF[64];	
			Q_snprintf(nameMF, sizeof(nameMF), "%s", pWeapon->GetWpnData().iAttachment_second);
			const char *nameP = nameMF;

			if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
				iAttachment = pWeapon->LookupAttachment(nameP);//muzzle1
			else
				iAttachment = pVM->LookupAttachment(nameP);
		}

		if (pWeapon->m_bSilenced)
		{
			char nameMF[64];
			Q_snprintf(nameMF, sizeof(nameMF), "%s", pWeapon->GetWpnData().iAttachment_sil);
			const char *nameP = nameMF;

			if (cvar->FindVar("oc_player_draw_body")->GetInt() && cvar->FindVar("oc_player_true_firstperson_vm")->GetInt())
				iAttachment = pWeapon->LookupAttachment(nameP);//muzzle_sil
			else
				iAttachment = pVM->LookupAttachment(nameP);
		}

		DispatchParticleEffect(name, PATTACH_POINT_FOLLOW, pVM, iAttachment, false);
	}
}

void CTempEnts::MuzzleFlash_General_NPC_Particle_Effect(const char *name, int colR, int colG, int colB, ClientEntityHandle_t hEntity, int attachmentIndex)
{
	if (name == NULL || (name && (!V_strlen(name) || FStrEq(name, "None") || FStrEq(name, "none"))))
		return;
	...
	//CBaseCombatWeapon *pEnt = ClientEntityList().GetBaseEntityFromHandle(hEntity);
	C_BaseCombatWeapon *pWeapon = static_cast<C_BaseCombatWeapon *>(ClientEntityList().GetBaseEntityFromHandle(hEntity));

	if (!pWeapon)
		return;
	DispatchParticleEffect(name, PATTACH_POINT_FOLLOW, pWeapon, attachmentIndex, false);
	//DispatchParticleEffect(name, PATTACH_POINT_FOLLOW, pEnt, "muzzle", false);
}

void CTempEnts::MuzzleFlash_General_Player_Secondary_Particle_Effect(const char *name, int colR, int colG, int colB, ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("MuzzleFlash_General_Player_Secondary_Particle_Effect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_General_Player_Particle_Effect", hEntity, attachmentIndex, FLE_VIEWMODEL);
	CacheMuzzleFlashes();
	...
	Vector			forward(7, 0, 0), right(0, 1.3, -1.3), offset;
	...
	if (cvar->FindVar("oc_state_InSecondFire")->GetInt() == 0)
	{
		DispatchParticleEffect(name, PATTACH_POINT_FOLLOW, pVM, attachmentIndex, false);
	}
}

void CTempEnts::MuzzleFlash_General_NPC_Secondary_Particle_Effect(const char *name, int colR, int colG, int colB, ClientEntityHandle_t hEntity, int attachmentIndex)
{
	if (name == NULL || (name && (!V_strlen(name) || FStrEq(name, "None") || FStrEq(name, "none"))))
		return;
	...
	C_BaseCombatWeapon *pWeapon = static_cast<C_BaseCombatWeapon *>(ClientEntityList().GetBaseEntityFromHandle(hEntity));
	//C_BaseEntity *pWeapon = ClientEntityList().GetBaseEntityFromHandle(hEntity);
	if (!pWeapon)
		return;
...
void CTempEnts::MuzzleFlash_Combine_NPC_Particle_Effect(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("MuzzleFlash_Combine_NPC_Particle_Effect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	*/
	...
	DispatchParticleEffect("ar2_normal_npc", origin, angles);
	DispatchParticleEffect("muzzle_sparks_ar2_npc", origin, angles);
	DispatchParticleEffect("muzzle_sparks_ar2_360_npc", origin, angles);*/

	// Get our attachment's transformation matrix
	FX_GetAttachmentTransform(hEntity, attachmentIndex, &origin, &angles);
	
	C_BaseCombatCharacter *pOwner = ToBaseCombatCharacter(ClientEntityList().GetBaseEntity(hEntity.GetEntryIndex()));//GetOwnerEntity()
	if (pOwner)
		DispatchParticleEffect("ar2_normal_npc", PATTACH_POINT_FOLLOW, pOwner, attachmentIndex, true);
}
void CTempEnts::MuzzleFlash_AirboatGun_NPC_Particle_Effect(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("MuzzleFlash_AirboatGun_NPC_Particle_Effect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	C_BaseAnimating *pVehicleAnimating = dynamic_cast<C_BaseAnimating *>(ClientEntityList().GetBaseEntity(hEntity.GetEntryIndex()));
	//C_BaseAnimating *pOwner = ToBaseCombatCharacter(ClientEntityList().GetBaseEntity(hEntity.GetEntryIndex()));//GetOwnerEntity()
	if (pVehicleAnimating)
		DispatchParticleEffect("abgun_normal2", PATTACH_POINT_FOLLOW, pVehicleAnimating, 1, false);
}

void CTempEnts::MuzzleFlash_APC_NPC_Particle_Effect(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	VPROF_BUDGET("MuzzleFlash_AirboatGun_NPC_Particle_Effect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	//C_BaseAnimating *pOwner = ToBaseCombatCharacter(ClientEntityList().GetBaseEntity(hEntity.GetEntryIndex()));//GetOwnerEntity()
	if (pVehicleAnimating)
		DispatchParticleEffect("abgun_normal2", PATTACH_POINT_FOLLOW, pVehicleAnimating, attachmentIndex, false);
}

//////////////////////////////////////////////////////////////////////OverCharged///////////////////////////////////////////////////////////////////////////////////
void CTempEnts::Shell_exhaust(C_BasePlayer *pPlayer)
{
	if (!pPlayer)
		return;
	...
	const char *nameP = pWeapon->GetWpnData().iWeaponShellExhaustParticle;

	if (nameP == NULL || (nameP && (!V_strlen(nameP) || FStrEq(nameP, "None") || FStrEq(nameP, "none"))))
	{
		Msg("Error: %s does't have shell exhaust particle definition in script fire \n", pWeapon->GetClassname());
	...
	if (pWeapon->iMuzzle == 2)
	{
		iAttachment = pPlayer->GetViewModel()->LookupAttachment(pWeapon->GetWpnData().iWeaponShellExhaustAttachmentDual2);//4
	}
	else
	{
		iAttachment = pPlayer->GetViewModel()->LookupAttachment(pWeapon->GetWpnData().iWeaponShellExhaustAttachment);//3
	}

	DispatchParticleEffect(nameP, PATTACH_POINT_FOLLOW, pPlayer->GetViewModel(), iAttachment, false);

}
void CTempEnts::Shell_exhaust(C_BaseCombatWeapon *pWeapon)
{
	if (!pWeapon)
		return;
	...
	if (pWeapon->iMuzzle == 2)
	{
		iAttachment = pWeapon->LookupAttachment(pWeapon->GetWpnData().iWeaponShellExhaustAttachmentDual2);
	}
	else
	{
		iAttachment = pWeapon->LookupAttachment(pWeapon->GetWpnData().iWeaponShellExhaustAttachment);
	}

	DispatchParticleEffect(nameP, PATTACH_POINT_FOLLOW, pWeapon, iAttachment, false);

}
void CTempEnts::Shell_exhaust_Npc(C_BaseCombatCharacter *pOwner)
{
	if (!pOwner)
		return;
	...
	int iAttachment = pWeapon->LookupAttachment(pWeapon->GetWpnData().iWeaponShellExhaustAttachment);

	DispatchParticleEffect(nameP, PATTACH_POINT_FOLLOW, pWeapon, iAttachment, false);

}

void CTempEnts::TracerCallBack(ClientEntityHandle_t hEntity, int attachmentIndex, const trace_t &tr, int trType)
{
	VPROF_BUDGET("TracerCallBack", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("TracerCallBack", hEntity, attachmentIndex);
	CacheMuzzleFlashes();

	Vector	origin;

	pSimple->SetDrawBeforeViewModel(false);
	...
	int indexT = random->RandomInt(0, pWeapon->GetWpnData().externalIndexT);
	char nameT2[256];
	const char *nameT;
	Q_snprintf(nameT2, sizeof(nameT2), "%s", pWeapon->GetWpnData().iMuzzleTracerType[indexT]);
	nameT = nameT2;

	if (nameT2 == NULL || (nameT2 && (!V_strlen(nameT2) || FStrEq(nameT2, "None") || FStrEq(nameT2, "none"))))
	{
		Msg("Error: %s does't have tracer particle definition in script fire \n", pWeapon->GetClassname());
	...
	//int iMask = MASK_ALL;//MASK_OPAQUE_AND_NPCS
	...
	if (cvar->FindVar("oc_adjust_weapon_tracers_offset")->GetInt() == 1)
	{
		origin += Forward * cvar->FindVar("oc_adjust_weapon_tracers_offset_x")->GetFloat();
		origin += Right * cvar->FindVar("oc_adjust_weapon_tracers_offset_y")->GetFloat();
		origin += Up * cvar->FindVar("oc_adjust_weapon_tracers_offset_z")->GetFloat();
	}
	else
	{
		if (pWeapon->IsScopeSighted() && !pWeapon->IsIronSighted())
		{
			if (oc_particle_tracers.GetInt())
			{
				origin += Forward * pWeapon->GetWpnData().ScopeParticleTracerPosition.x;
				origin += Right * pWeapon->GetWpnData().ScopeParticleTracerPosition.y;
				origin += Up * pWeapon->GetWpnData().ScopeParticleTracerPosition.z;
			}
			else
			{
				origin += Forward * pWeapon->GetWpnData().ScopeOriginTracerPosition.x;
				origin += Right * pWeapon->GetWpnData().ScopeOriginTracerPosition.y;
				origin += Up * pWeapon->GetWpnData().ScopeOriginTracerPosition.z;
			}
		}
		else if (pWeapon->IsIronSighted() && !pWeapon->IsScopeSighted())
		{
			if (oc_particle_tracers.GetInt())
			{
				origin += Forward * pWeapon->GetWpnData().IrSightParticleTracerPosition.x;
				origin += Right * pWeapon->GetWpnData().IrSightParticleTracerPosition.y;
				origin += Up * pWeapon->GetWpnData().IrSightParticleTracerPosition.z;
			}
			else
			{
				origin += Forward * pWeapon->GetWpnData().IrSightOriginTracerPosition.x;
				origin += Right * pWeapon->GetWpnData().IrSightOriginTracerPosition.y;
				origin += Up * pWeapon->GetWpnData().IrSightOriginTracerPosition.z;
			}
		}
		else
		{
			if (oc_particle_tracers.GetInt())
			{
				origin += Forward * pWeapon->GetWpnData().ParticleTracerPosition.x;
				origin += Right * pWeapon->GetWpnData().ParticleTracerPosition.y;
				origin += Up * pWeapon->GetWpnData().ParticleTracerPosition.z;
			}
			else
			{
				origin += Forward * pWeapon->GetWpnData().OriginTracerPosition.x;
				origin += Right * pWeapon->GetWpnData().OriginTracerPosition.y;
				origin += Up * pWeapon->GetWpnData().OriginTracerPosition.z;
			}
		}
	}

	UTIL_TraceLine(pPlayer->EyePosition(), tr.endpos, MASK_SHOT, pPlayer, COLLISION_GROUP_NONE, &tr2);

	//UTIL_ParticleTracer(nameT, origin, tr.endpos, pPlayer->GetViewModel()->entindex(), attachmentIndex, trType);//OverCharged
	//DispatchParticleEffect(nameT, origin /*vFirePos*/, tr.endpos, vec3_angle, pPlayer->GetViewModel());
	if (pWeapon->GetWpnData().TracersCombined)
	{
		UTIL_ParticleTracer(nameT, origin, tr.endpos, pPlayer->entindex(), attachmentIndex, trType);
		UTIL_Tracer(origin, tr.endpos, pPlayer->entindex(), TRACER_DONT_USE_ATTACHMENT, 6500, false, "CommonTracer");
	}
	else
	{
		if (oc_particle_tracers.GetInt())
...
void CTempEnts::MuzzleLightCallBack()
{
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if (pPlayer && pPlayer->GetActiveWeapon())
	{
		if (pPlayer->GetActiveWeapon()->MuzzleFlashTypeIndex() >= 0 && pPlayer->GetWaterLevel() != 3)
		{
			pPlayer->GetActiveWeapon()->IsMuzzleFlashActive = true;
			pPlayer->GetActiveWeapon()->IsMuzzleFlashComplete = false;
		}
	}
}
