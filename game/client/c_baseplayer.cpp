...
////////////////////////////////////////////////OverCharged/////////////////////////////////////////
extern void FormatViewModelAttachment(Vector &vOrigin, bool bInverse); // BriJee OVR: Lets begin. Flashlight viewmodel attachment.
////////////////////////////////////////////////////////////////////////////////////////////////////

ConVar oc_viewbob_walk_dist("oc_viewbob_walk_dist", "0.0", FCVAR_ARCHIVE);
ConVar oc_viewbob_walk_scale("oc_viewbob_walk_scale", "0.0", FCVAR_ARCHIVE);
ConVar oc_viewbob_vol("oc_viewbob_vol", "0.0", FCVAR_ARCHIVE);
ConVar oc_viewbob_vol_walk("oc_viewbob_vol_walk", "0.0", FCVAR_ARCHIVE);
ConVar oc_viewbob_speed("oc_viewbob_speed", "0.0", FCVAR_ARCHIVE);
ConVar oc_viewbob_speed_walk("oc_viewbob_speed_walk", "0.0", FCVAR_ARCHIVE);
ConVar oc_viewbob_model_scale_ang("oc_viewbob_model_scale_ang", "1", FCVAR_ARCHIVE);///////////OverCharged
ConVar oc_viewbob_model_scale_pos("oc_viewbob_model_scale_pos", "1", FCVAR_ARCHIVE);///////////OverCharged
...
ConVar	oc_player_draw_body("oc_player_draw_body", "0", FCVAR_ARCHIVE, "Causes the third person model to be drawn instead of the view model");
...
/*void CC_oc_enable_nightvision(void)
{
...
static ConCommand oc_enable_nightvision("oc_enable_nightvision", CC_oc_enable_nightvision, "", FCVAR_HIDDEN);*/

/*#include "hud_macros.h"
HOOK_COMMAND(oc_enable_nightvision, Nightvision);
void C_BasePlayer::UserCmd_Nightvision(void)
{
...
//static ConVar oc_viewbob_model_scale("oc_viewbob_model_scale", "1");///////////OverCharged
...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_RECV_TABLE_NOBASE( C_BasePlayer, DT_LocalPlayerExclusive )
		...
		RecvPropBool		( RECVINFO( AllowCamAttachment) ),
		RecvPropBool		( RECVINFO( SpecialBodyDraw) ),
		RecvPropBool		( RECVINFO( m_bIsRunning ) ),
		//RecvPropBool		( RECVINFO( m_bIsSighted ) ),
		RecvPropBool		( RECVINFO( m_bSlowMoIsEnabled ) ),
		RecvPropBool		( RECVINFO( m_bNightvisionEnabled ) ),
		RecvPropBool		( RECVINFO( m_bIsInSelectionMode ) ),
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		RecvPropInt		(RECVINFO(m_iBloodOverlayDetailFrame)),
		RecvPropFloat(RECVINFO(m_flNextBloodDryDisappear))
...
C_BasePlayer::C_BasePlayer() : m_iv_vecViewOffset( "C_BasePlayer::m_iv_vecViewOffset" )
{
	...
	m_iBloodOverlayDetailFrame = 0;
	m_flNextBloodDryDisappear = 0.0f;
}
...
bool C_BasePlayer::IsSighted(void)
{
...
bool C_BasePlayer::IsReloadingWeapon(void)
{
	C_BaseCombatWeapon *pWeapon = GetActiveWeapon();
	if (pWeapon)
		return pWeapon->IsInReload();
...
void C_BasePlayer::ReceiveMessage( int classID, bf_read &msg )
{
	...
	switch( messageType )
	{
		...
		//Special player's events:
		case PLAYER_MSG_DO_BATTERY:
		{
			doBatteryEffect = true;
		}
		break;
		case PLAYER_MSG_DO_HEALTH:
		{
			doHealthEffect = true;
		}
		break;
		case PLAYER_MSG_DO_RECHARGE:
		{
			doRechargeEffect = true;
		}
		break;

		case PLAYER_MSG_STOP_RECHARGE:
		{
			doRechargeEffect = false;
		}
		break;

		case PLAYER_MSG_DO_HEALTH_REGEN:
		{
			doHealthRegenEffect = true;
		}
		break;

		case PLAYER_MSG_STOP_HEALTH_REGEN:
		{
			doHealthRegenEffect = false;
		}
		break;

		//Damages:
		case PLAYER_MSG_DO_DMG_SHOCK:
		{
			doDmgShockEffect = true;
		}
		break;
		case PLAYER_MSG_DO_DMG_BULLET:
		{
			doDmgBulletEffect = true;
		}
		break;
	}
}
	...
	if ( IsLocalPlayer() )
	{
		...
		for ( int i = 0; i < MAX_AMMO_TYPES; i++ )
		{
			if ( GetAmmoCount(i) > m_iOldAmmo[i] )
			{
				...
				if (!pWeaponData || (!(pWeaponData->iFlags & ITEM_FLAG_NOAMMOPICKUPS) && !pWeaponData->allowAmmoRegen))
				{
...
void C_BasePlayer::UpdateNightvision()
{
	...
	if (m_bNightvisionEnabled)
	{
		if (!pNVLight)
		{
			...
			EmitSound(filter, 0, "Player.NightvisionOn", vOrigin);
		}
		else
		{
			...
			pNVLight->UpdateLight(EyePosition(), vecForward, vecRight, vecUp, FLASHLIGHT_DISTANCE);
		}
	}
	else
	{
		if (pNVLight)
		{
			...
			CLocalPlayerFilter filter;
			EmitSound(filter, 0, "Player.NightvisionOff", vOrigin);
		}
	}
}
...
void C_BasePlayer::UpdateFlashlight()
{
	...
	if (IsEffectActive(EF_DIMLIGHT) /*&& !m_bNightvisionEnabled*/)
	{
		if (!m_pFlashlight)
		{
		...
		if (cvar->FindVar("oc_player_flashlight_onweapon")->GetInt())
		{
			C_BasePlayer *player = C_BasePlayer::GetLocalPlayer();
			CBaseCombatWeapon *pWeapon = player->GetActiveWeapon();

			// BriJee OVR: DISABLE dynamic flashlight IF - 1 we on ladder. 2 object hold. 3 NO active weapon.
			if ((player->GetMoveType() == MOVETYPE_LADDER) | /*(cvar->FindVar("aa_PlrHoldObject")->GetInt() == 1) |*/ (pWeapon == NULL))
			{
				Vector vecForward, vecRight, vecUp;
				EyeVectors(&vecForward, &vecRight, &vecUp);
			...
			// BriJee OVR: Check FIRST and THIRD person state.
			else
			{
				QAngle angLightDir;
				Vector vecLightOrigin, vecForward, vecRight, vecUp;

				if (input->CAM_IsThirdPerson())	// 3rd just use player model attachment
				{
					GetAttachment(LookupAttachment("anim_attachment_RH"), vecLightOrigin, angLightDir);
				}
				else // 1st use viewmodel attachment
				{
					CBaseViewModel *vm = GetViewModel();	// BriJee OVR: Trying to find attachment by name & get the integer value so we can use it
					int iAttachment = vm->LookupAttachment("muzzle");

					GetViewModel()->GetAttachment( /*LookupAttachment("muzzle")*/ iAttachment, vecLightOrigin, angLightDir);
				}
...
ShadowHandle_t C_BasePlayer::GetFlashlightHandle()
{
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
...
void C_BasePlayer::AddEntity( void )
{
	...
	SetLocalAnglesDim(X_INDEX, 0); // Overcharged: Player anims fix
}
...
ConVar oc_player_legs_clip_height("oc_player_legs_clip_height", "0", FCVAR_CHEAT);
ConVar oc_player_legs_origin_shift("oc_player_legs_origin_shift", "0", FCVAR_CHEAT);
int C_BasePlayer::DrawModel( int flags )
{
	...
	if (IsAlive())
	{
		if (oc_player_legs_clip_height.GetInt() != 0)
		{
			CMatRenderContextPtr context(materials);
			context->SetHeightClipMode(MATERIAL_HEIGHTCLIPMODE_RENDER_ABOVE_HEIGHT);// (MATERIAL_HEIGHTCLIPMODE_RENDER_BELOW_HEIGHT);
			context->SetHeightClipZ(GetAbsOrigin().z + oc_player_legs_clip_height.GetFloat());
		}
	}
...
{
	/*C_BaseAnimating *pBody = GetBaseAnimating();

	forward = pBody->GetLocalOrigin();
	forward.z = oc_player_legs_origin_shift.GetFloat();
	pBody->SetLocalOrigin(forward);*/

	//DevMsg("GetRenderOrigin(): %.2f, %.2f, %.2f \n", BaseClass::GetRenderOrigin().x, BaseClass::GetRenderOrigin().y, BaseClass::GetRenderOrigin().z);
	...
	forward = BaseClass::GetRenderOrigin(); // static because this method returns a reference
	AngleVectors(GetRenderAngles(), &forward);

	// Shift the render origin by a fixed amount
	forward *= oc_player_legs_origin_shift.GetFloat();
...
void C_BasePlayer::CalcInEyeCamView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	...
	if (pl && (pl->m_Local.m_bDucked || pl->m_Local.m_bDucking) && cvar->FindVar("oc_player_draw_body")->GetInt())
	{
		DevMsg("eyeAngles: %.2f, %.2f, %.2f \n", eyeAngles.x, eyeAngles.y, eyeAngles.z);

		if (eyeAngles.x > 70.0f && eyeAngles.x < 90.0f)
		{
			eyeAngles.x -= 0.7f;// *gpGlobals->frametime;
		}
...
// OVERCHARGED: 1st person deathcam
void C_BasePlayer::CalcFirstPersonDeathView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	if (m_lifeState != LIFE_ALIVE)
	{
		...
		// Sets the ragdoll.
		InitAsClientRagdollDeath(boneDelta0, boneDelta1, currentBones, boneDt);
		
		// Calculate origin of the ragdoll.
		Vector origin = EyePosition();
		QAngle Ang = EyeAngles();
		VectorAngles(origin, Ang);
		...
		if (pRagdoll)
		{
			...
			GetAttachment(m_iEyeAttachment, attToWorld);
			Vector local, tmp;
			local.Init(0.0f, 0.0f, 0.0f);
			VectorTransform(local, attToWorld, tmp);
			origin = tmp;
			MatrixAngles(attToWorld, Ang);
		...
		VectorNormalize(vForward);
		VectorMA(origin, -CHASE_CAM_DISTANCE_MIN, vForward, eyeOrigin);*/
...
// OVERCHARGED: 3rd person deathcam
...
/*static*/ bool C_BasePlayer::ShouldDrawLocalPlayer()
{
	if ( !UseVR() )
	{
		if (oc_player_draw_body.GetBool())
		{
		...
		return !LocalPlayerInFirstPersonView(); //|| ((cvar->FindVar("oc_playerview_draw_body")->GetInt() == 1) && this->IsAlive());		// Overcharged added last argument
	}

	C_BasePlayer bp;
	if (bp.IsAlive() && cvar->FindVar("oc_playerview_draw_body")->GetInt() == 1)
	{
		if (CurrentViewID() == VIEW_INTRO_CAMERA)
		{
			return false;
...
bool C_BasePlayer::ShouldDrawThisPlayer()
{
	...
	if (!UseVR() && oc_player_draw_body.GetBool()) //|| (cvar->FindVar("oc_playerview_draw_body")->GetInt() == 1)) )		// Overcharged added last argument
	{
		if (CurrentViewID() == VIEW_INTRO_CAMERA)
		{
			return false;
	...
	if ( !IsAlive() && cvar->FindVar("oc_playerview_draw_body")->GetInt() == 1)
	{
		if (CurrentViewID() == VIEW_INTRO_CAMERA)
		{
			return false;
...
void C_BasePlayer::OverrideView(CViewSetup *pSetup)
{
	...
	if (pPlayer->IsAlive() && input->CAM_IsFreeAiming())
	{
		if (cvar->FindVar("oc_state_InSecondFire")->GetInt() == 0 && cvar->FindVar("oc_state_IRsight_on")->GetInt() == 0)
		{
			//::input->GetCamViewangles(m_angViewAngle);
			QAngle angPunch = pPlayer->GetPunchAngle();
			QAngle viewAng, viewAng2;
			::input->GetCamViewangles(viewAng);

			if (!pPlayer->IsInAVehicle())
				pSetup->angles = viewAng + angPunch;

			//else if (pPlayer->IsInAVehicle())
			//pSetup->angles = m_vecVehicleViewAngles;
		}
	}

	Vector velocity;
	EstimateAbsVelocity(velocity);
	float speed = velocity.NormalizeInPlace();///Normalized
	float X = speed * (IsRunning() ? oc_viewbob_vol.GetFloat() : oc_viewbob_vol_walk.GetFloat());

	C_BaseCombatWeapon *pWeapon = GetActiveWeapon();

	if (pWeapon && pWeapon->IsScopeSighted())//cvar->FindVar("oc_state_InSecondFire")->GetInt() == 1)
		return;

	static float amt = 0.0f;
	static float amtSide = 0.0f;
	float amtGoal = RemapValClamped(X, 200.0f, 300.0f, 0, 1.5f);//OverCharged
	...
	if (amt != amtGoal)
		amt = Approach(amtGoal, amt, gpGlobals->frametime * 6.0f);//3.0

	if (amtSide != amtGoalSide)
		amtSide = Approach(amtGoalSide, amtSide, gpGlobals->frametime * 8.0f);//4.0
	...
	bool isInAttachment = false;

	if (cvar->FindVar("oc_player_allow_animated_camera")->GetInt() 
		  && ((!pPlayer->IsInAVehicle()
		  && pWeapon
		  && pWeapon->GetWpnData().AttachmentApply
		  //&& DoAttach
		  && !cvar->FindVar("oc_using_entity")->GetInt()
		  && pPlayer->AllowCamAttachment)))
	{
		isInAttachment = true;
		Vector pos, X, Y, Z;
		Vector posFinal;
		...
		if (m_pBobViewModel == NULL)
		{
			...
			if (pszName && *pszName)
			{
				m_pBobViewModel = new C_Cam_Attachment();
		...
		if (!m_pBobViewModel->IsInvalid())
		{
			...
			QAngle ang, ang2;
			m_pBobViewModel->GetDeltaTransforms(pos, ang);
		...
		{
			pSetup->angles += m_angLastBobAngle * oc_viewbob_model_scale_ang.GetFloat() * m_flBobModelAmount;
		}
...
RenderGroup_t C_BasePlayer::GetRenderGroup()
{
	Vector origin = EyePosition();
	...
	{
		origin.z -= 5.f;
		isOnWater = (enginetrace->GetPointContents(origin) & (CONTENTS_WATER | CONTENTS_SLIME)) || GetWaterLevel();
		if (isOnWater)
			break;
	}
	...
	{
		GetBaseAnimating()->RemoveAllDecals();
		clearDecals = true;
	}
	else
	{
	...
	if (isOnWater)
	{
		if (GetBaseAnimating())
		{
			GetBaseAnimating()->SetRenderColor(255, 255, 255, 255);
			GetBaseAnimating()->SetModelScale(0.5f);
		}
		return RENDER_GROUP_VIEW_MODEL_TRANSLUCENT;
	}


	if (/*GetWaterLevel() > 0 || */ SpecialBodyDraw || IsDucked() || IsDucking())
	{
		if (GetBaseAnimating())
		{
			GetBaseAnimating()->SetRenderColor(255,255,255,255);
	...
	{
		if (GetBaseAnimating())
			GetBaseAnimating()->SetModelScale(1.f);
	}
	return RENDER_GROUP_TRANSLUCENT_ENTITY;// RENDER_GROUP_TRANSLUCENT_ENTITY;// RENDER_GROUP_OPAQUE_ENTITY;//RENDER_GROUP_VIEW_MODEL_OPAQUE;
}
