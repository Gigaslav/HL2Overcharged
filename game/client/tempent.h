...
public:
	...
	/*DevMsg("%s \n", GetBaseAnimating()->m_iClassname);
	DevMsg("%s \n", C_BaseEntity::m_iClassname);
	if (!V_strcmp(C_BaseAnimating::C_BaseEntity::GetClassname(), "viewmodel"))
	{
	return true;
	}
	else
	...
	//DevMsg("%i \n", GetBaseAnimating()->IsClientPlayer());
