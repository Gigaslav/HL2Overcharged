...
PMaterialHandle g_Blood_Drops1 = NULL;
PMaterialHandle g_Blood_Drops2 = NULL;
PMaterialHandle g_Blood_Drops3 = NULL;
PMaterialHandle g_Blood_Drops4 = NULL;
...
void CTrailParticles::SimulateParticles( CParticleSimulateIterator *pIterator )
{
	...
	while ( pParticle )
	{
		...
		/*if (GetParticleSystemIndex("FX_BloodDrips"))
		{
			DevMsg("EffectName FX_BloodDrips\n");
			//m_ParticleCollision.MoveParticle(pParticle->m_Pos, pParticle->m_vecVelocity, NULL, timeDelta, &trace);
		}*/
...
void FX_LaserSparkMuzzle(const Vector &position, const Vector &direction, int iScale)
{
	VPROF_BUDGET("FX_LaserSparkMuzzle", VPROF_BUDGETGROUP_PARTICLE_RENDERING);

	Vector offset = position;

	CSmartPtr<CTrailParticles> sparkEmitter = CTrailParticles::Create("FX_LaserSparkMuzzle 1");
	...
	int	numSparks = random->RandomInt(1, 6) * (iScale * 2);
	numSparks = (int)(0.5f + (float)numSparks * g_pParticleSystemMgr->ParticleThrottleScaling());
	...
	for (int i = 0; i < numSparks; i++)
	{
		...
		float	spreadOfs = random->RandomFloat(0.0f, 5.0f);
		...
		pParticle->m_flWidth = random->RandomFloat(1.0f, 3.0f);
		pParticle->m_flLength = random->RandomFloat(length*0.35f, length);

		pParticle->m_vecVelocity = dir * random->RandomFloat((METAL_SPARK_MINSPEED*(2.0f - spreadOfs)), (METAL_SPARK_MAXSPEED*(2.0f - spreadOfs)));

		Color32Init(pParticle->m_color, 0, 213, 255, 255);
	}
...
void FX_LaserSpark(const Vector &position, const Vector &direction, const Vector &surfaceNormal, int iScale)
{
	VPROF_BUDGET("FX_LaserSpark", VPROF_BUDGETGROUP_PARTICLE_RENDERING);

	Vector offset = position + (surfaceNormal * 1.0f);

	CSmartPtr<CTrailParticles> sparkEmitter = CTrailParticles::Create("FX_LaserSpark 1");
...
void FX_BloodDrips(const Vector &position, const Vector &direction, const Vector &surfaceNormal, int iScale)
{
	VPROF_BUDGET("FX_BloodDrips", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	CSmartPtr<CTrailParticles> sparkEmitter = CTrailParticles::Create("FX_BloodDrips 1");
	...
	const float	timeDelta = pIterator->GetTimeDelta();
	trace_t trace;
	sparkEmitter->m_ParticleCollision.MoveParticle(pParticle->m_Pos, pParticle->m_vecVelocity, NULL, timeDelta, &trace);
	*/
	FXQuadData_t data;
...
void FX_BloodStreamSmall(Vector &position, Vector &normal, int bloodColor)
{
	/*Vector	offset = position + (normal * 1.0f);
	...
	sparkEmitter->m_ParticleCollision.m_bDoDecal = true;
	sparkEmitter->m_ParticleCollision.m_iBloodColor = bloodColor;
	...
	int count = random->RandomInt(2, 5);
	//Dump out sparks
	for (int i = 0; i < count; i++)
	{
		...
		float	spreadOfs = random->RandomFloat(1.0f, 3.0f);
		...
		pParticle->m_vecVelocity = dir * random->RandomFloat( (2*(2.0f-spreadOfs)), (54*(2.0f-spreadOfs)) );

		if (!bloodColor)
			Color32Init(pParticle->m_color, 55, 0, 0, 20);
		else
			Color32Init(pParticle->m_color, 240, 230, 140, 5);
	}*/
	...
	PMaterialHandle Blood_Drops = NULL;


	if (g_Blood_Drops1 == NULL)
	{
		g_Blood_Drops1 = ParticleMgr()->GetPMaterial("effects/blood_core");
	}
	if (g_Blood_Drops2 == NULL)
	{
		g_Blood_Drops2 = ParticleMgr()->GetPMaterial("effects/blood_gore");
	}
	if (g_Blood_Drops3 == NULL)
	{
		g_Blood_Drops3 = ParticleMgr()->GetPMaterial("effects/blood_drop");
	}
	if (g_Blood_Drops4 == NULL)
	{
		g_Blood_Drops4 = ParticleMgr()->GetPMaterial("effects/blood_puff");
	}
	...
	for (int i = 0; i < count; i++)
	{
		int randomTexture = random->RandomInt(1, 3);

		switch (randomTexture)
		{
		/*case 0:
			Blood_Drops = g_Blood_Drops1;
			break;*/
		case 1:
			Blood_Drops = g_Blood_Drops2;
			break;
		case 2:
			Blood_Drops = g_Blood_Drops3;
			break;
		case 3:
			Blood_Drops = g_Blood_Drops4;
			break;
		}
		//DevMsg("randomTexture: %i\n", randomTexture);
		
		//pParticle = (TrailParticle *)sparkEmitter->AddParticle(sizeof(TrailParticle), g_Material_Spark, offset);

		pParticle = (TrailParticle *)sparkEmitter->AddParticle(sizeof(TrailParticle), Blood_Drops, offset);
		...
		//offDir = offset + RandomVector(-1.0f, 1.0f);

		float	spreadOfs = random->RandomFloat(1.0f, 3.0f);

		offDir[0] = normal[0] + random->RandomFloat(-(METAL_SCRAPE_SPREAD*spreadOfs), (METAL_SCRAPE_SPREAD*spreadOfs));
		offDir[1] = normal[1] + random->RandomFloat(-(METAL_SCRAPE_SPREAD*spreadOfs), (METAL_SCRAPE_SPREAD*spreadOfs));
		offDir[2] = normal[2] + random->RandomFloat(-(METAL_SCRAPE_SPREAD*spreadOfs), (METAL_SCRAPE_SPREAD*spreadOfs));

		pParticle->m_vecVelocity = offDir * random->RandomFloat(84.0f, 168.0f);

		pParticle->m_flWidth = random->RandomFloat(0.9f, 3.0f);
		pParticle->m_flLength = random->RandomFloat(0.1f, 0.2f);
		pParticle->m_flDieTime = random->RandomFloat(0.25f, 0.5f);

		if (!bloodColor)
		{
			pParticle->m_color.r = 85.f;
			pParticle->m_color.g = 0.f;
			pParticle->m_color.b = 0.f;
			pParticle->m_color.a = 255.f;
		}
		else
		{
			pParticle->m_color.r = 135.f;
			pParticle->m_color.g = 115.f;
			pParticle->m_color.b = 67.f;
			pParticle->m_color.a = 255.f;
		}
		/*if (!bloodColor)
		{
			FloatToColor32(pParticle->m_color, 55.f, 0, 0, 1.0f);
			//Color32Init(pParticle->m_color, 55, 0, 0, 20);
		}
		else
		{
			FloatToColor32(pParticle->m_color, 240.f, 230.f, 140.f, 1.0f);
			//Color32Init(pParticle->m_color, 240, 230, 140, 5);
		}*/
...
void BloodDripsCallback(const CEffectData & data)
{
	Vector vecNormal;
	Vector vecPosition;
	QAngle angles;
	...
	vecNormal = data.m_vNormal;
	angles = data.m_vAngles;
	bloodColor = data.m_nAttachmentIndex;

	FX_BloodStreamSmall(vecPosition, vecNormal, bloodColor);
}

DECLARE_CLIENT_EFFECT("BloodDrips", BloodDripsCallback);