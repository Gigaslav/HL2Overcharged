...
/*void C_SoundscapeSystem::UpdateSoundScapes(void)
{
	if (int i = 0 ; i < m_soundscapes.Count(); i++)
	{
		m_soundscapes[i]->pitc;
	}
}*/

CON_COMMAND(oc_updateLooping_sounds, "Flushes the client side soundscapes")
{
	g_SoundscapeSystem.UpdateLoopingSounds(gpGlobals->frametime);
	g_SoundscapeSystem.UpdateRandomSounds(gpGlobals->curtime);
}
...
/*if ( !Q_strcasecmp( pKey->GetName(), "pitch" ) )
		{
...
void C_SoundscapeSystem::ProcessPlayLooping( KeyValues *pAmbient, const subsoundscapeparams_t &params )
{
	...
	if ( volume != 0 && pSoundName != NULL )
	{
		if ( positionIndex < 0 )
		{
			AddLoopingAmbient(pSoundName, volume, pitch/* * GP->newPitch*/);
		}
		else
		{
			...
			AddLoopingSound(pSoundName, false, volume, soundlevel, pitch/* * GP->newPitch*/, m_params.localSound[positionIndex]);
		}
	}
}
...
{
	return AddLoopingSound(pSoundName, true, volume, SNDLVL_NORM, pitch/* * GP->newPitch*/, vec3_origin);
}
...
int C_SoundscapeSystem::AddLoopingSound( const char *pSoundName, bool isAmbient, float volume, soundlevel_t soundlevel, int pitch, const Vector &position )
{
	...
	if ( soundSlot < 0 )
	{
		...
		if ( isAmbient )
		{
			...
			enginesound->EmitAmbientSound(pSoundName, 0, pitch/* * GP->newPitch*/);
...
{
	float newPitch = cvar->FindVar("host_timescale")->GetFloat();

	if ( loopSound.isAmbient )
	{
		enginesound->EmitAmbientSound(loopSound.pWaveName, loopSound.volumeCurrent, loopSound.pitch * newPitch, SND_CHANGE_VOL);
	}
	else
	{
		CLocalPlayerFilter filter;
		...
		ep.m_nPitch = loopSound.pitch * newPitch;
...
void C_SoundscapeSystem::PlayRandomSound( randomsound_t &sound )
{
	...
	if ( sound.isAmbient )
	{
		enginesound->EmitAmbientSound(pWaveName, sound.masterVolume * RandomInterval(sound.volume), (int)RandomInterval(sound.pitch)/* * GP->newPitch*/);
	}
	else
	{
		CLocalPlayerFilter filter;
		...
		ep.m_nPitch = (int)RandomInterval(sound.pitch)/* * GP->newPitch*/;
		if ( sound.isRandom )
		{
...
void C_SoundscapeSystem::UpdateRandomSounds( float gameTime )
{
	...
	{
		m_randomSounds[i].pitch.start *= newPitch;
		// time to play?
