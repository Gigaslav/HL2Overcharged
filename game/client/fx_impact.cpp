...
/*static void ReadDataFromFile(IFileSystem* filesystem)
{
	KeyValues *pkvProperties = new KeyValues("ImpactFx");
	DevMsg("ReadDataFromFile \n");
	char ss[256];
	Q_snprintf(ss, sizeof(ss), "scripts/%s.txt", "ImpactFxParticleEffects");

	if (pkvProperties->LoadFromFile((IBaseFileSystem *)filesystem, "scripts/ImpactFxParticleEffects.txt", "MOD"))
	{
		DevMsg("LoadFromFile \n");
		if (pkvProperties)
		{
			DevMsg("Loaded \n");
			KeyValues *pMain = pkvProperties->FindKey("Entities");
			if (pMain)
			{
...
static void PerformNewCustomEffects( const Vector &vecOrigin, trace_t &tr, const Vector &shotDir, int iMaterial, int iScale, int nFlags )
{
	...
	if (iMaterial == CHAR_TEX_METAL)
	{
		CPASFilter filter(vecOrigin);
		te->DynamicLight(filter, 0.0, &vecOrigin, RandomFloat(50, 100), RandomFloat(50, 100), RandomFloat(50, 100), 3, RandomFloat(12, 48), RandomFloat(0.05, 0.11), 0);//OverCharged
	}
	if (iMaterial == CHAR_TEX_COMPUTER)
	{
		CPASFilter filter(vecOrigin);
		te->DynamicLight(filter, 0.0, &vecOrigin, RandomFloat(50, 100), RandomFloat(50, 100), RandomFloat(50, 100), 3, RandomFloat(12, 48), RandomFloat(0.05, 0.11), 0);//OverCharged
	}
	if (iMaterial == CHAR_TEX_WARPSHIELD)
	{
		CPASFilter filter(vecOrigin);
		te->DynamicLight(filter, 0.0, &vecOrigin, 0, RandomFloat(50, 140), RandomFloat(50, 140), 3, RandomFloat(12, 48), RandomFloat(0.09, 0.11), 0);//OverCharged
	}
...
void PerformCustomEffects( const Vector &vecOrigin, trace_t &tr, const Vector &shotDir, int iMaterial, int iScale, int nFlags )
{
	...
	if (cvar->FindVar("oc_particle_impacts")->GetInt())
	{
	...
	else if ( ( iMaterial == CHAR_TEX_METAL ) || ( iMaterial == CHAR_TEX_VENT ) )
	{
		...
		CPASFilter filter(vecOrigin);
		te->DynamicLight(filter, 0.0, &vecOrigin, RandomFloat(50, 100), RandomFloat(50, 100), RandomFloat(50, 100), 1, RandomFloat(12, 48), RandomFloat(0.04, 0.11), 0);//OverCharged
	...
	else if ( iMaterial == CHAR_TEX_WARPSHIELD )
	{
		...
		CPASFilter filter(vecOrigin);
		te->DynamicLight(filter, 0.0, &vecOrigin, 0, RandomFloat(50, 100), RandomFloat(50, 100), 1, RandomFloat(12, 48), RandomFloat(0.04, 0.11), 0);//OverCharged
	}
}
...
void PlayImpactSound( CBaseEntity *pEntity, trace_t &tr, Vector &vecServerOrigin, int nServerSurfaceProp )
{
	...
	if ( pdata->sounds.bulletImpact )
	{
		...
		else
		{
			...
			//VectorAngles(-vecOrigin, vecAngles);
			//DispatchParticleEffect("blood_impact_red_01", vecOrigin, vecAngles);
		}
