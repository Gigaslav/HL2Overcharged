...
void C_BaseExplosionEffect::Create( const Vector &position, float force, float scale, int flags )
{
	...
	el->color.exponent = 10;

	el->radius = random->RandomInt(760, 920);
	el->decay = el->radius / 4.1f;
	el->die = gpGlobals->curtime + 20.1f;*/
}
...
void C_BaseExplosionEffect::CreateCore( void )
{
	...
	if (cvar->FindVar("oc_particle_explosions")->GetInt())
	{
		...
		{
			case 0:
				DispatchParticleEffect((cvar->FindVar("oc_fx_particle_main_explosion_effect1_name")->GetString()), m_vecOrigin, vec3_angle);		// overcharged BJ: Was Explosion_01 02 03 04 05
				break;
			case 1:
				DispatchParticleEffect((cvar->FindVar("oc_fx_particle_main_explosion_effect2_name")->GetString()), m_vecOrigin, vec3_angle);
				break;
			case 2:
				DispatchParticleEffect((cvar->FindVar("oc_fx_particle_main_explosion_effect3_name")->GetString()), m_vecOrigin, vec3_angle);
				break;
			case 3:
				DispatchParticleEffect((cvar->FindVar("oc_fx_particle_main_explosion_effect4_name")->GetString()), m_vecOrigin, vec3_angle);
				break;
			case 4:
				DispatchParticleEffect((cvar->FindVar("oc_fx_particle_main_explosion_effect5_name")->GetString()), m_vecOrigin, vec3_angle);
				break;
			default:
				break;
		}
...
void C_BaseExplosionEffect::CreateDynamicLight( void )
{
	...
	dl->decay	= 200;
	dl->radius = 255 * (engine->MapHasHDRLighting() ? 1 : 0.3);
	dl->color.r = 255;
	dl->color.g = 220;
	dl->color.b = 128;
